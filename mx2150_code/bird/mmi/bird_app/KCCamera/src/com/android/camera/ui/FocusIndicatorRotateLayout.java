...
import com.android.camera.manager.ModePicker;//gangyun tech add 
    ...
    public void showStart() {
        ...
        if (mState == STATE_IDLE) {
			...
			if(mContext.getCurrentMode() == ModePicker.MODE_BOKEH){
				...
				animate().withLayer().setDuration(SCALING_UP_TIME).scaleX(1f).scaleY(1f);*/
			}
			else{/* gangyun tech add end */
    ...
    public void showFail(boolean timeout) {
        ...
        if (mState == STATE_FOCUSING) {
			...
			if(mContext.getCurrentMode() == ModePicker.MODE_BOKEH){
                 ...
                 mState = STATE_IDLE;
				/*
	            setDrawable(R.drawable.gy_aperture_focusfail);
	            animate().withLayer().setDuration(SCALING_DOWN_TIME).scaleX(1f).scaleY(1f)
	                    .withEndAction(timeout ? mEndAction : null);*/
