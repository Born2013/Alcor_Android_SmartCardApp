...
public class ZoomManager extends ViewManager implements
    ...
    public void zoomUp() {
        ...
        int oldIndex = findZoomIndex(mLastZoomRatio);
        if(oldIndex == getMaxZoomIndex()) {
            Log.i(TAG, "zoomUp(), zoomIndex is already max!");
            return;
        }
        
        int zoomIndex = oldIndex + 1;
        performZoom(zoomIndex, true);
        Log.i(TAG, "zoomUp() zoomIndex:" + zoomIndex);
    }
    ...
    public void zoomDown() {
        ...
        int oldIndex = findZoomIndex(mLastZoomRatio);
        if(oldIndex <= 0) {
            Log.i(TAG, "zoomDown(), zoomIndex is already min!");
            return;
        }
        
        int zoomIndex = oldIndex - 1;
        performZoom(zoomIndex, true);
        Log.i(TAG, "zoomDown() zoomIndex:" + zoomIndex);
    }
    //[BIRD][BIRD_VOLUME_KEY_FEATURE_IN_CAMERA][相机设置中增加音量键功能选项][qianliliang][20151209] BEGIN
}
