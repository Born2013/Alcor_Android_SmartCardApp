...
public class PhotoActor extends CameraActor implements FocusManager.Listener,
    ...
    private boolean mIsShut = false;//[BIRD_ONBACKPRESS_TAKE_PHOTO][在锁屏亮屏界面长按back打开相机并拍照][hongzhihao][20170613]
    ...
    public void setShut(boolean sh) {
        mIsShut = sh;
    }
	...
	//[BIRD_ONBACKPRESS_TAKE_PHOTO][在锁屏亮屏界面长按back打开相机并拍照][hongzhihao][20170613]begin
    @Override
    public boolean onShutterButtonClick(ShutterButton button) {
        int cameraState = mCameraActivity.getCameraState();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        //[BIRD][BIRD_VOLUME_KEY_FEATURE_IN_CAMERA][相机设置中增加音量键功能选项][qianliliang][20151209]BEGIN
        case KeyEvent.KEYCODE_VOLUME_DOWN:
            if(FeatureSwitcher.BIRD_VOLUME_KEY_FEATURE_IN_CAMERA && mIsInitialized) {
                String volumeKeyFeature = mCameraActivity.getISettingCtrl().getSettingValue(SettingConstants.KEY_VOLUME_KEY_FEATURE);
                if("zoom".equals(volumeKeyFeature)) {
                     mCameraActivity.getCameraAppUI().getZoomManager().zoomDown();
                    return true;
                }
                if("shoot".equals(volumeKeyFeature)) {
                    return true;
                }
            }
            return false;
        case KeyEvent.KEYCODE_VOLUME_UP:
            if(FeatureSwitcher.BIRD_VOLUME_KEY_FEATURE_IN_CAMERA && mIsInitialized) {
                String volumeKeyFeature = mCameraActivity.getISettingCtrl().getSettingValue(SettingConstants.KEY_VOLUME_KEY_FEATURE);
                if("zoom".equals(volumeKeyFeature)) {
                    mCameraActivity.getCameraAppUI().getZoomManager().zoomUp();
                    return true;
                }
                if("shoot".equals(volumeKeyFeature)) {
                    return true;
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        case KeyEvent.KEYCODE_VOLUME_DOWN:
            if(FeatureSwitcher.BIRD_VOLUME_KEY_FEATURE_IN_CAMERA && mIsInitialized) {
                String volumeKeyFeature = mCameraActivity.getISettingCtrl().getSettingValue(SettingConstants.KEY_VOLUME_KEY_FEATURE);
                if("shoot".equals(volumeKeyFeature)) {
                    if (mIsInitialized && !mIsCameraKeyLongPressed
                            && event.getRepeatCount() == 0
    ...
    public void startPreview(boolean needStop) {//gangyun modif private to public//[BIRD][GANGYUN_BOKEH_SUPPORT][双摄][yangheng][20170215] 
    ...
    public void restartPreview(boolean needStop) {//gangyun modif private to public//[BIRD][GANGYUN_BOKEH_SUPPORT][双摄][yangheng][20170215] END
    ...
    private void overrideFocusMode(String focusMode) {
		...
		//[BIRD_ONBACKPRESS_TAKE_PHOTO][在锁屏亮屏界面长按back打开相机并拍照][hongzhihao][20170613]begin
		Log.i("xiaowo", "mIsShut = " + mIsShut);
        if (mIsShut && FeatureSwitcher.BIRD_ONBACKPRESS_TAKE_PHOTO) {
            if (onShutterButtonClick(null)) {
                mIsShut = false;
            }           
        }
		//[BIRD_ONBACKPRESS_TAKE_PHOTO][在锁屏亮屏界面长按back打开相机并拍照][hongzhihao][20170613]end
