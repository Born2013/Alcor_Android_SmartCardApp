...
public class GyPreviewSize implements ISettingRule {
    private static final String TAG = "GyPreviewSize";
    ...
    public GyPreviewSize(ICameraContext ct) {
        mICameraContext = ct;
    ...
    private void setVFBPreviewSize() {
        ...
        for (int i = 0; i < mSupportedPreviewSizes.size(); i++) {
            if (mSupportedPreviewSizes.get(i).width >= 1280
                    || mSupportedPreviewSizes.get(i).height > 1088) {
...
*/
		...
		boolean Screen_4_3 = false;
		for (int i = 0; i < mSupportedPreviewSizes.size(); i++) {
				if (mSupportedPreviewSizes.get(i).width == 960
						&& mSupportedPreviewSizes.get(i).height == 540) {					
					fullScreen = true;
				}else if(mSupportedPreviewSizes.get(i).width == 800
						&& mSupportedPreviewSizes.get(i).height == 600){
						Screen_4_3 = true;
        ...
        if(previewRatio != null && previewRatio.equals("1.7778") && fullScreen){
			mICameraDevice.setPreviewSize(960, 540);

		}
		else if (previewRatio != null && previewRatio.equals("1.3333") && Screen_4_3){
			mICameraDevice.setPreviewSize(800, 600);
		}
		else{
        ...
        Log.d(TAG, "[setVFBPreviewSize] will set preview width = " + optiomSize.width
                + ",height = " + optiomSize.height + " previewRatio:"+previewRatio);
