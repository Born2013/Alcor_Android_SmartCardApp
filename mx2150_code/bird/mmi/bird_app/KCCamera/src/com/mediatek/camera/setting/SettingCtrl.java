...
public class SettingCtrl implements ISettingCtrl {
    ...
    private void onSettingChanged(Parameters parameters, int currentCameraId, String key,
        ...
        if (!isExecutedByRule) {
            ...
            //[BIRD][BIRD_ADD_CAMERA_SHUTTERSOUND][拍照提示音控制菜单][qianliliang][20151210] BEGIN
            } else if (FeatureSwitcher.BIRD_ADD_CAMERA_SHUTTERSOUND && SettingConstants.KEY_CAMERA_SHUTTER_SOUND.equals(key)) {
                if ((mICameraContext.getActivity()) instanceof CameraActivity) {
                    //if (!((CameraActivity)mICameraContext.getActivity()).isVideoCaptureIntent()
					//	&& !((CameraActivity)mICameraContext.getActivity()).isVideoWallPaperIntent() ) {
                    //}
                    boolean mShutterSoundEnabled = "on".equals(value);
                    Log.i(TAG, "BIRD, mShutterSoundEnabled = " + mShutterSoundEnabled);
				    if (mShutterSoundEnabled) {
			            mICameraDeviceManager.getCameraDevice().enableShutterSound();
			        } else {
			            mICameraDeviceManager.getCameraDevice().disnableShutterSound();
			        }
                }
            //[BIRD][BIRD_ADD_CAMERA_SHUTTERSOUND][拍照提示音控制菜单][qianliliang][20151210] END
            } else {
