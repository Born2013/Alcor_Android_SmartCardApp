...
public class SettingGenerator {
    ...
    public void restoreSetting(int cameraId) {
        ...
        if (pictureRatioPref != null) {
			...
			if(FeatureSwitcher.BIRD_CAMERA_PICTURESIZE_STANDARD) {
            	pictureRatioPref.setValue("1.3333");//ratio
            	pictureRatioSetting.setValue("1.3333");//ratio
            	pictureRatioSetting.setDefaultValue("1.3333");//ratio
			} else {
    ...
    private boolean filterPreference(final ListPreference preference) {
        ...
        case SettingConstants.ROW_SETTING_ZSD:
            ...
            break;
	    //[BIRD][BIRD_VOLUME_KEY_FEATURE_IN_CAMERA][相机设置中增加音量键功能选项][qianliliang][20150921]BEGIN
        case SettingConstants.ROW_SETTING_CAMERA_VOLUME_KEY_FEATURE:
			if(!FeatureSwitcher.BIRD_VOLUME_KEY_FEATURE_IN_CAMERA){
				removePreference = true;
			}
			break;
		//[BIRD][BIRD_VOLUME_KEY_FEATURE_IN_CAMERA][相机设置中增加音量键功能选项][qianliliang][20150921]END
		...
		case SettingConstants.ROW_SETTING_CAMERA_SHUTTER_SOUND:
			if(FeatureSwitcher.BIRD_ADD_CAMERA_SHUTTERSOUND){
    ...
    private void limitPreferencesByIntent() {
        ...
        if (currentCameraId == mICameraDeviceManager.getFrontCameraId()) {
            ...
            //[BIRD][BUG #7987][使用前摄连拍，提示相机停止运行][qianliliang][20160528] BEGIN
            List<String> captureMode = mICameraDeviceManager.getCameraDevice(currentCameraId).getParameters().getSupportedCaptureMode();
            boolean isSupport = captureMode == null ? false : captureMode.indexOf("continuousshot") >= 0;
            Log.i(TAG, "BIRD, Continuous_shot_mode [isSupport?]isSupport = " + isSupport);
            if (isSupport) {
                unSupportedByFrontCamera = SettingConstants.UN_SUPPORT_BY_FRONT_CAMERA_EXCEPT_CONTINUOUS_SHOT;
            }
            //[BIRD][BUG #7987][使用前摄连拍，提示相机停止运行][qianliliang][20160528] END
