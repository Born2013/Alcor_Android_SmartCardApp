...
int FirewallController::clearFwChain(const char* chain) {
    ...
    if(chain != NULL){
        /*[BIRD_AIMEI_STUDENT][gaowei][20171219]begin*/
        char mvalue[PROPERTY_VALUE_MAX];
        property_get("ro.bd_aimei_student", mvalue, "-1");
        if (atoi(mvalue) == 1) {
            ALOGD("gaowei1219-mvalue=%s,MOBILE=%d,chain=%s",mvalue,MOBILE,chain);
            if(!strcmp(chain,"mobile")) {
                ALOGD("gaowei1219 clearFwChain mobile");
                blacklistUsers.clear();
            }
        }
        /*[BIRD_AIMEI_STUDENT][gaowei][20171219]end*/
        if(strlen(chain) > 0){
