...
#ifdef CONFIG_GTP_GESTURE_DOUBLE_CLICK_WAKEUP
static int double_click_flag=1;
#endif
...
*/
		...
		if ((doze_buf[0] == 'a') || (doze_buf[0] == 'b') || (doze_buf[0] == 'c') ||
			...
			(doze_buf[0] == 'w') || (doze_buf[0] == 'y') || (doze_buf[0] == 'z'))
	 	{
			if ('o' == doze_buf[0])
			{
				...
				input_report_key(dev, KEYCODE_CTPO, 1);
				input_sync(dev);
				input_report_key(dev, KEYCODE_CTPO, 0);
				input_sync(dev);
			...
			{
				printk("Gesture[%c] wake up.\n", doze_buf[0]);
				input_report_key(dev, KEYCODE_CTPC, 1);
				input_sync(dev);
				input_report_key(dev, KEYCODE_CTPC, 0);
				input_sync(dev);
			...
			{
				printk("Gesture[%c] wake up.\n", doze_buf[0]);			
				input_report_key(dev, KEYCODE_CTPM, 1);
				input_sync(dev);
				input_report_key(dev, KEYCODE_CTPM, 0);
				input_sync(dev);
			...
			{
				printk("Gesture[%c] wake up.\n", doze_buf[0]);				
				input_report_key(dev, KEYCODE_CTPW, 1);
				input_sync(dev);
				input_report_key(dev, KEYCODE_CTPW, 0);
				input_sync(dev);
			...
			{	
				printk("Gesture[%c] wake up.\n", doze_buf[0]);	
				input_report_key(dev, KEYCODE_CTPZ, 1);
				input_sync(dev);
				input_report_key(dev, KEYCODE_CTPZ, 0);
				input_sync(dev);
			...
			{
				printk("Gesture[%c] wake up.\n", doze_buf[0]);				
				input_report_key(dev, KEYCODE_CTPE, 1);
				input_sync(dev);
				input_report_key(dev, KEYCODE_CTPE, 0);
				input_sync(dev);
			...
			{
				printk("Gesture[%c] wake up.\n", doze_buf[0]);			
				input_report_key(dev, KEYCODE_CTPS, 1);
				input_sync(dev);
				input_report_key(dev, KEYCODE_CTPS, 0);
				input_sync(dev);
		...
		else if (doze_buf[0] == 0xCC)
		{
			if(double_click_flag==1) {
				...
				input_report_key(dev, KEYCODE_CTPUP, 1);
				input_sync(dev);
				input_report_key(dev, KEYCODE_CTPUP, 0);
				input_sync(dev);
				double_click_flag=0;
			} else {
				gesture_enter_doze();
