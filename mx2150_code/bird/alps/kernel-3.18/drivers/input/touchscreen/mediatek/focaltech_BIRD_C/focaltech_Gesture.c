...
int fts_Gesture_init(struct input_dev *input_dev)
{
	...
	input_set_capability(input_dev, EV_KEY, KEY_DOUBLE_CLICK);  
	...
	__set_bit(KEY_DOUBLE_CLICK, input_dev->keybit);
...
static void fts_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	...
	switch(gesture_id)
	{
		...
		case GESTURE_C:
	        ...
	        case GESTURE_DOUBLECLICK:
	                input_report_key(input_dev, KEY_DOUBLE_CLICK, 1);
	                input_sync(input_dev);
	                input_report_key(input_dev, KEY_DOUBLE_CLICK, 0);
	                input_sync(input_dev);
	                break;
	        case GESTURE_O:
...
int fts_read_Gestruedata(void)        //read data from fw.Drawdata[i] sent to mobile************************************************
{
	...
	printk("crystal_fts_read_Touchdata buf[0] = %d\n",buf[0]);
	printk("crystal_fts_read_Touchdata buf[1] = %d\n",buf[1]);
	...
	if (ret < 0)
	{
       ...
       {
    printk( "%d fts_updateinfo_curr.CHIP_ID.\n", fts_updateinfo_curr.CHIP_ID);
    pointnum = (short)(buf[1]) & 0xff;
    buf[0] = 0xd3;
    if((pointnum * 4 + 2+6)<128)
    {
    ret = fts_i2c_read(fts_i2c_client, buf, 1, buf, (pointnum * 4 + 2 + 6));
    }
    else
    {
        ...
        ret = fts_i2c_read(fts_i2c_client, buf, 0, buf+128, (pointnum * 4 + 2 +6)-128);
