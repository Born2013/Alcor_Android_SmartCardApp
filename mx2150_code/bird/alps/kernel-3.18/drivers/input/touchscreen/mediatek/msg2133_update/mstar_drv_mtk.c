...
#ifdef CONFIG_MSG_BIRD_HARDWARE_INFOMATION_DETAILE
...
//    const char *vcc_i2c_name = "vcc_i2c";
	...
	TPD_DMESG("META_BOOT! MSG tpd_probe return!\n");
	return -1;
    } 
	...
	if(nMajor == msgMajorXXXX)
	{
		strcpy(mtkfb_tp_name_moudle, tp_moudle0_name);
	}
	else if(nMajor == msgMajorYYYY)
	{
		strcpy(mtkfb_tp_name_moudle, tp_moudle1_name);
	}
	else
	{
		strcpy(mtkfb_tp_name_moudle, "SENSOR ID ERROR");
	}
		sprintf(version, "%02d.%03X",nMajor, nMinor);
		strcat(mtkfb_tp_name_moudle,"VERSION=");
		strcat(mtkfb_tp_name_moudle,version);
		strcpy(mtkfb_tp_name, mtkfb_tp_name_moudle); 
...
{  
    ...
    if (tpd_dts_data.use_tpd_button)
    {
        ...
        tpd_button_setting(tpd_dts_data.tpd_key_num, g_TpVirtualKey,tpd_dts_data.tpd_key_dim_local);
    #else
        tpd_button_setting(tpd_dts_data.tpd_key_num, tpd_dts_data.tpd_key_local,
        tpd_dts_data.tpd_key_dim_local);
    #endif  //CONFIG_TPD_COMPATIBILITY_REPORT_KEY
    }
#else
#ifdef CONFIG_TP_HAVE_KEY
#ifdef CONFIG_ENABLE_REPORT_KEY_WITH_COORDINATE     
