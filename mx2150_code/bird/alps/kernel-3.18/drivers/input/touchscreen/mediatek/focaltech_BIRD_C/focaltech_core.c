		...
		void* buff_out, int size_out, int* actualout)
{
	...
	printk("[proxi_fts]command = 0x%02X\n", command);		
...
#if defined(FTS_GESTRUE_EN)||defined(TPD_PROXIMITY)
	int ret = 0;
	u8 state = 0;
#endif
			...
			if ((cinfo.y[0] > TPD_RES_Y) || (pinfo.y[0] > TPD_RES_Y)) {
				if (finfo.y[0] > TPD_RES_Y) {
					if ((cinfo.p[0] == 0) || (cinfo.p[0] == 2)) {
							...
							tpd_down(cinfo.x[0], cinfo.y[0], 1,1);
					} else if ((cinfo.p[0] == 1) &&
						((pinfo.p[0] == 0) || (pinfo.p[0] == 2))) {
...
//wwm start//
#ifndef BIRD_VH2521_BIRD_X5S_NO_POWEROFF
#if 1
	#if 1//def TPD_POWER_VTP28_USE_VCAMA
            #ifdef BIRD_TPD_POWER_USER_VGP1
...
//wwm start//
	...
	#if 1//def TPD_POWER_VTP28_USE_VCAMA
            #ifdef BIRD_TPD_POWER_USER_VGP1
		#ifdef BIRD_VH2521_BIRD_X5S_NO_POWEROFF
...
//wwm//		g_hq_ctp_module_fw_version = ver;
	...
	if (ret < 0)
		...
		if ((err = hwmsen_attach(ID_PROXIMITY, &obj_ps)))
			pr_err("hwmsen attach fail, return:%d.", err);
#endif
