...
#ifdef TPD_HAVE_BUTTON //bird modify
...
void gt1x_power_switch(s32 state)
{
  ...
  #ifdef CONFIG_BIRD_TPD_POWER_USER_VGP1
      #if 1// !defined(CONFIG_GTP_GESTURE_WAKEUP)
	        int ret = 0; 
          //#else
          //int ret = 0;
      #endif
	...
	pmic_set_register_value(PMIC_RG_VCAMA_VOSEL,0x03);//2.8
	#endif
			//ret = regulator_enable(tpd->reg);	/*enable regulator*/
			//if (ret)
			//	GTP_ERROR("regulator_enable() failed!\n");
...
{
	s32 err = 0;
#ifdef GTP_HAVE_TOUCH_KEY //bird modify
	s32 idx = 0;
#endif
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(META_BOOT == get_boot_mode()) {
		GTP_INFO("META_BOOT! gt1x_reset_guitar return!\n");
		return -1;
	} 
...
void gt1x_touch_down(s32 x, s32 y, s32 size, s32 id)
{
	...
	tpd_history_x = x;
	tpd_history_y = y;
//#ifdef CONFIG_MTK_BOOT //bird modify
	if (tpd_dts_data.use_tpd_button) {
