...
/* add by dayin */
static int VALS = 0x40;
static int VALW = 0x20;
static int VALM = 0x10;
static int VALO = 0x08;
static int VALC = 0x04;
static int VALE = 0x02;
static int VALZ = 0x01;
/* add by dayin */
...
#if defined (TOUCH_GESTURE_SWITCH)
static volatile unsigned int gsl_gesture_flag = 0;//1; //tjf
#else
static volatile unsigned int gsl_gesture_flag = 1;//0; //tjf
#endif
...
#endif
#ifdef CONFIG_BIRD_GET_PHONE_STATE
#include "stk3x1x.h" //modify by qiuming
extern int Get_Phone_State_Flag; // 1 =phone call  open   0= phone  call close
static u8 halt_power_flag = 0;
#endif
#ifdef TPD_PROXIMITY
#include <hwmsensor.h> 
...
#define gsl_err(fmt, args...)   \
		do{                              \
		    printk("[tp-gsl][%s][%d]"fmt,__func__, __LINE__,##args);     \
		}while(0)
                               ...
                               unsigned long arg)
{
   ...
   // void __user *data;
    ...
    if (err)
    {
        gsl_err("tpd: access error: %08X, (%2d, %2d)\n", cmd, _IOC_DIR(cmd), _IOC_SIZE(cmd));
        return -EFAULT;
    }
    ...
    switch (cmd)
    {
        ...
        default:
            gsl_info("tpd: unknown IOCTL: 0x%08x\n", cmd);
            err = -ENOIOCTLCMD;
            break;
...
static ssize_t tpd_misc_write(struct file *file, const char __user *buf, size_t count, loff_t * ppos)
{
    int value;
    int ret;
    int valuenow = 0x00;
    ...
    if(ret)
    gsl_info("copy form user failed\n");
    valuenow=value;
    VALS = valuenow&0x40;
    VALW = valuenow&0x20;   
    VALM = valuenow&0x10;
...
/* add by wuchuang 20170213 end */
	...
	static void msg_dma_alloct(void)
	{
	    if (NULL == g_dma_buff_va)
		 ...
		 memset(g_dma_buff_va, 0, 255);
	    	...
	    	if(!g_dma_buff_va)
		{
	        	gsl_err("[DMA][Error] Allocate DMA I2C Buffer failed!\n");
	    	}
	}
	static void msg_dma_release(void){
		if(g_dma_buff_va)
		{
	        	...
	        	g_dma_buff_va = NULL;
	        	g_dma_buff_pa = 0;
			gsl_info("[DMA][release] Allocate DMA I2C Buffer release!\n");
	    	}
	}
#endif
...
s32 gsl_write_interface(struct i2c_client *client, u8 reg, u8 *buf, u32 num)
{
	...
	else if( 1 &&(NULL != g_dma_buff_va))
	{
		for (i = 0; i < writelen; i++)
		{
			...
			//gsl_info("[DMA][gsl_write_interface] g_dma_buff_va[%d]=%x\n",i,g_dma_buff_va[i]);
		}
				...
				unsigned int addr,unsigned int len)
{
	u8 buf[4];
	int i;
	gsl_info("tp-gsl-gesture %s\n",__func__);
	gsl_info("gsl_read_oneframe_data:::addr=%x,len=%x\n",addr,len);
	for(i=0;i<len/2;i++){
	...
	#if 1
	for(i=0;i<len;i++){
	gsl_info("gsl_read_oneframe_data =%x\n",data[i]);	
	//gsl_info("gsl_read_oneframe_data =%x\n",data[len-1]);
	}
	#endif
...
static int gsl_gpio_idt_tp(struct gsl_ts_data *ts)
{
	...
	td = (value & (0x1 << 1))>>1;
	gsl_info("[tpd_gsl][%s] [ru,rd]=[%d,%d]\n",__func__,ru,rd);
	gsl_info("[tpd_gsl][%s] [tu,td]=[%d,%d]\n",__func__,tu,td);
	if(ru == 0 && rd == 0)
		rstate = 0;
	else if(ru == 1 && rd == 1)
		rstate = 1;
	...
	gsl_info("[tpd-gsl][%s] [rstate,status]=[%d,%d]\n",__func__,rstate,tstate);
	return 1;
}
#endif
static int gsl_test_i2c(struct i2c_client *client)
{
	...
	for(i=0;i<5;i++)
	{
		...
		if(err>0)
		{
			gsl_info("[tp-gsl] i2c read 0xfc = 0x%02x%02x%02x%02x\n",
...
static int gsl_ts_read(struct i2c_client *client, u8 addr, u8 *pdata, unsigned int datalen)
{
	...
	if (ret < 0)
	{
		gsl_info("%s set data address fail!\n", __func__);
		return ret;
	}
...
static void gsl_start_core(struct i2c_client *client)
{
	...
	gsl_write_interface(client,0xe0,buf,4);
		...
		if(tp_identy==1)
			gsl_DataInit(gsl_config_data_id_mwd);
		else
			gsl_DataInit(gsl_config_data_id_xyd);
		#elif defined (MX2126E_WPF_D5027_HD720)||defined (MX2126E_WPF_D5027_FWVGA)
		if(tp_identy==1)
...
static void gsl_identify_tp_by_dac(struct i2c_client *client)
{
	...
	gsl_ts_write(client, 0xf0, writebuf, 4); 	
      msleep(50);
	ret =gsl_ts_read(client,0x14, readbuf, 4);
	if (ret < 0) 
		gsl_ts_read(client,0x14, readbuf, 4);

	gsl_info("======read 1fe1000,1c: %x %x %x %x======\n",readbuf[3], readbuf[2], readbuf[1], readbuf[0]);
	
	if(first_init_flag ==0)
    {   
	if(0 < readbuf[1] && readbuf[1] < BIRD_LOAD_FW_THRD)
	
      {
		 	gsl_info("======000000000000000000000000000000000000000000000000000====\n");
             tp_identy=0;
			 #if defined(MX2126E_BER_S5518_QHD) 
			 gsl_config_data_id=gsl_config_data_id_xyd;
			 #elif defined (MX2126E_WPF_D5027_HD720)||defined (MX2126E_WPF_D5027_FWVGA)
			 gsl_config_data_id=gsl_config_data_id_mwd;
			 #else
			 gsl_config_data_id=gsl_config_data_id_mwd;
			 #endif
	}
       ...
       {	
			 ...
			 gsl_info("======1111111111111111111111111111111111======\n");

			tp_identy=1;
			#if defined(MX2126E_BER_S5518_QHD) 
			gsl_config_data_id=gsl_config_data_id_mwd;
			gsl_reset_core(client);
			tmp2=ARRAY_SIZE(GSLX680_FW_MWD);		
			gsl_load_fw(client,GSLX680_FW_MWD,tmp2);
			 #elif defined (MX2126E_WPF_D5027_HD720)||defined (MX2126E_WPF_D5027_FWVGA)			
			gsl_config_data_id=gsl_config_data_id_hsd;
			gsl_reset_core(client);
			tmp2=ARRAY_SIZE(GSLX680_FW_HSD);		
			gsl_load_fw(client,GSLX680_FW_HSD,tmp2);
			#else		
			gsl_config_data_id=gsl_config_data_id_hsd;
			gsl_reset_core(client);
			tmp2=ARRAY_SIZE(GSLX680_FW_HSD);		
...
static void gsl_sw_init(struct i2c_client *client)
{
	...
	#ifdef GSL_IDENTY_TP_BY_DAC
		#if defined(MX2126E_BER_S5518_QHD) 
		temp1=ARRAY_SIZE(GSLX680_FW_XYD);
		gsl_load_fw(client,GSLX680_FW_XYD,temp1);
		#elif defined (MX2126E_WPF_D5027_HD720)	||defined (MX2126E_WPF_D5027_FWVGA)
		temp1=ARRAY_SIZE(GSLX680_FW_MWD);
		gsl_load_fw(client,GSLX680_FW_MWD,temp1);
		#else
		temp1=ARRAY_SIZE(GSLX680_FW_MWD);
		gsl_load_fw(client,GSLX680_FW_MWD,temp1);
		#endif
	...
	gsl_info("[gsl1680][%s] addr = (b6,7c); read_buf = %02x%02x%02x%02x\n",
...
static void check_mem_data(struct i2c_client *client)
{
	...
	gsl_info("[gsl1680][%s] addr = 0xb0; read_buf = %02x%02x%02x%02x\n",
		__func__, read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
...
static int  gsl_config_write_proc(struct file *file, const char __user  *buffer, size_t  count, loff_t *data)
{
	...
	if(count > 512)
	{
		gsl_info("size not match [%d:%d]\n", CONFIG_LEN, count);
        	return -EFAULT;
	}
	path_buf=kzalloc(count,GFP_KERNEL);
	if(!path_buf)
	{
		gsl_info("alloc path_buf memory error \n");
		return -1;
	}	
	if(copy_from_user(path_buf, buffer, count))
	{
		gsl_info("copy from user fail\n");
...
static void gsl_timer_check_func(struct work_struct *work)
{
	...
	{
		gsl_info("======int_1st: %x %x %x %x , int_2nd: %x %x %x %x ======\n",
	...
	for(i=0;i<5;i++){
		...
		if(read_buf[3] != 0x5a || read_buf[2] != 0x5a || 
			...
			gsl_info("gsl_monitor_worker 0xb0 = {0x%02x%02x%02x%02x};\n",
...
static int tpd_enable_ps(int enable)
{
	...
	gsl_err("tpd_enable_ps:gsl_ps_enable = %d\n",gsl_ps_enable);
	if (enable) {
		wake_lock(&ps_lock);
		buf[3] = 0x00;
		buf[2] = 0x00;
		...
		gsl_info("tpd-ps function is on\n");
	} else {
		...
		buf[0] = 0x00;
		gsl_write_interface(ddata->client, 0, buf, 4);
		gsl_info("tpd-ps function is off\n");
	}
	return 0;
}
        ...
        void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			{
				gsl_info("Set delay parameter error!\n");
				err = -EINVAL;
			}
			// Do nothing
			break;
		...
		case SENSOR_ENABLE:
			if((buff_in == NULL) || (size_in < sizeof(int)))
			{
				gsl_info("Enable sensor parameter error!\n");
				err = -EINVAL;
			}
			else
			{
		...
		case SENSOR_GET_DATA:
			...
			{
				gsl_info("get sensor data parameter error!\n");
				err = -EINVAL;
			}
			else
			{
		...
		default:
			gsl_info("proxmy sensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
...
static unsigned int gsl_sysfs_init(void)
{
	...
	{
		gsl_info("%s: subsystem_register failed\n", __func__);
		return -ENOMEM;
	}
	ret = sysfs_create_file(gsl_debug_kobj, &dev_attr_version.attr);
	if (ret)
	{
		gsl_info("%s: sysfs_create_version_file failed\n", __func__);
		return ret;
	}
}
*/
static void gsl_report_point(struct gsl_touch_info *ti)
{
	...
	else 
	{
		...
		{
			gsl_info("[lifan_gsl1680](x[%d],y[%d]) = (%d,%d);\n", 
...
static void gsl_report_work(void)
{
	...
	gsl_info("enter gsl_report_work\n");
	...
	if (tpd_proximity_flag == 1)
	{
		...
		{
			gsl_info("call hwmsen_get_interrupt_data fail = %d\n", err);
		}
	}
	/*end of added*/
#endif
...
*/
	...
	for(i = 0; i < (point_num < MAX_CONTACTS ? point_num : MAX_CONTACTS); i ++)
	{
		...
		gsl_info("tp-gsl-------------  before: x[%d] = %d, y[%d] = %d, id[%d] = %d \n",i,cinfo.x[i],i,cinfo.y[i],i,cinfo.id[i]);
	}
	//int tmp1 = 0;
	...
	if(tmp1>0&&tmp1<0xffffffff)
	{
		...
		gsl_info("tmp1=%08x,buf[0]=%02x,buf[1]=%02x,buf[2]=%02x,buf[3]=%02x\n",
...
#ifdef GSL_GESTURE
		gsl_info("gsl_gesture_status=%d,gsl_gesture_flag=%d\n",gsl_gesture_status,gsl_gesture_flag);
	
		if(GE_ENABLE == gsl_gesture_status && gsl_gesture_flag == 1){
			int tmp_c;
			u8 key_data = 0;
			tmp_c = gsl_obtain_gesture();
			gsl_info("gsl_obtain_gesture():tmp_c=0x%x\n",tmp_c);
			gsl_info("gsl_obtain_gesture():tmp_c=0x%x\n",tmp_c);
			switch(tmp_c){
			case (int)'C':
                                if ( VALC == 0x04)
...
//				break;	/* left */
			...
			do_gettimeofday(&time_last);
				//gsl_gesture_status = GE_WAKEUP; //zxw modify for mx1091_hyf_9300 先划无效的手势然后划有效的手势不起作用
				gsl_info("gsl_obtain_gesture():tmp_c=%c\n",gsl_gesture_c);
...
gsl_info("[tpd-gsl][%s] 6666666666666666666\n",__func__);
	do
	{
gsl_info("[tpd-gsl][%s] 7777777777777777777777777\n",__func__);
	#ifdef CONFIG_OF_TOUCH
...
identify_tp_repeat:
	...
	for(i=0;i<3;i++){
		...
		gsl_info("i = %d count0 the test 0xb4 = {0x%02x%02x%02x%02x}\n",i,buf[3],buf[2],buf[1],buf[0]);

		count0 = (buf[3]<<8)|buf[2];
		msleep(5);
		gsl_read_interface(ts->client,0xb4,buf,4);

		gsl_info("i = %d count1 the test 0xb4 = {0x%02x%02x%02x%02x}\n",i,buf[3],buf[2],buf[1],buf[0]);
...
static irqreturn_t tpd_eint_interrupt_handler(unsigned irq, struct irq_desc *desc)
{
	...
	gsl_info("[tpd-gsl][%s][%d] cccccccccccccccccccccccccccccc\n",__func__,__LINE__);
...
static void gsl_hw_init(void)
{
	...
	int ret = 0;
        ...
        if (ret)
            gsl_info("regulator_set_voltage() failed!\n");
        ret = regulator_enable(tpd->reg);   /* enable regulator */
        if (ret)
            gsl_info("regulator_enable() failed!\n");
...
static int  gsl_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		client->addr = 0x40;
		gsl_info("Crystal_shen:i2c_client_GSL->addr=%d\n",client->addr);
	}
	...
	if (!ddata) {
		gsl_info("alloc ddata memory error\n");
		return -ENOMEM;
	}
	mutex_init(&gsl_i2c_lock);
	ddata->client = client;
	gsl_info("ddata->client->addr = 0x%x \n",ddata->client->addr);
	gsl_hw_init();
	#ifdef CONFIG_OF_TOUCH
	//disable_irq(touch_irq);
	#else
	...
	gsl_info("[tpd-gsl][%s] gsl_cfg_index=%d\n",__func__,gsl_cfg_index);
#ifdef GSL_GESTURE
	gsl_FunIICRead(gsl_read_oneframe_data);
        ...
        if (err != 0)
	{
	  gsl_info("mtk_tpd: tpd_misc_device register failed\n");
	}
	...
	gsl_info("[tpd-gsl][%s] 111111111111111\n",__func__);
	thread = kthread_run(touch_event_handler, 0, TPD_DEVICE);
	if (IS_ERR(thread)) {
	gsl_info("[tpd-gsl][%s] 222222222222222\n",__func__);
		//err = PTR_ERR(thread);
		//TPD_DMESG(TPD_DEVICE " failed to create kernel thread: %d\n", PTR_ERR(thread));
	}
	gsl_info("[tpd-gsl][%s] 3333333333333333\n",__func__);
...
//				enable_irq(touch_irq);
...
#endif
	gsl_info("[tpd-gsl][%s] 555555555555555555\n",__func__);
#ifdef TPD_PROC_DEBUG
#if 0
	gsl_config_proc = create_proc_entry(GSL_CONFIG_PROC_FILE, 0666, NULL);
	if (gsl_config_proc == NULL)
	{
		gsl_info("create_proc_entry %s failed\n", GSL_CONFIG_PROC_FILE);
	}
	else
	{
	...
	//gsl_sysfs_init();
	#ifdef CONFIG_OF_TOUCH
	gsl_info("[tpd-gsl][%s][%d] bbbbbbbbbbbbbbbbbbbbbbbbbbbb\n",__func__,__LINE__);
	disable_irq(touch_irq);
	gsl_info("[tpd-gsl][%s][%d] dddddddddddddddddddddddddddd\n",__func__,__LINE__);
	enable_irq(touch_irq);
	#else
	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
	#endif
...
static int gsl_local_init(void)
{
	...
	gsl_info();
	#ifdef CONFIG_OF_TOUCH
		gsl_info("[wsl] gsl_local_init start \n");

	gsl_info("[wsl] regulator_get start \n");
    tpd->reg = regulator_get(tpd->tpd_dev, "vtouch");
    if (IS_ERR(tpd->reg))
	gsl_err("tpd->reg error\n");
	gsl_info("[wsl] regulator_get end \n");
	...
	gsl_info("boot_mode == %d \n", boot_mode);

	if (boot_mode == SW_REBOOT)
	boot_mode = NORMAL_BOOT;

#ifdef TPD_HAVE_BUTTON
	gsl_info("TPD_HAVE_BUTTON\n");
	...
	if (ret < 0) {
		gsl_err("unable to i2c_add_driver\n");
		return -ENODEV;
	}

	if (tpd_load_status == 0) 
	{
		gsl_info("tpd_load_status == 0, gsl_probe failed\n");
...
static void gsl_suspend(struct device *h)
{
	...
	gsl_info();
	gsl_info("gsl_suspend11111111111111111111\n");
	gsl_err("gsl_suspend:gsl_ps_enable = %d\n",gsl_ps_enable);
#ifdef CONFIG_BIRD_GET_PHONE_STATE
	gsl_info("gsl_suspend: get_phone_state_flag = %d\n",Get_Phone_State_Flag);
	if(Get_Phone_State_Flag ==1){
	return;
	}
	halt_power_flag =1;
#endif
	...
	gsl_halt_flag = 1;
	//version info
	gsl_info("[tp-gsl]the last time of debug:%x\n",TPD_DEBUG_TIME);
#ifdef GSL_ALG_ID
	tmp = gsl_version_id();	
	gsl_info("[tp-gsl]the version of alg_id:%x\n",tmp);
#endif
...
static void gsl_resume(struct device *h)
{
	gsl_info();		
#ifdef CONFIG_BIRD_GET_PHONE_STATE
	gsl_info("gsl_resume: get_phone_state_flag = %d, halt_power_flag = %d\n",Get_Phone_State_Flag, halt_power_flag);
	if((Get_Phone_State_Flag ==1)&&(halt_power_flag ==0)){
	return;
	}
	halt_power_flag = 0;
#endif
	...
	check_mem_data(ddata->client);
	#ifdef CONFIG_OF_TOUCH
	gsl_info("[tpd-gsl][%s][%d] aaaaaaaaaaaaaaaaaaaaaaaaaa\n",__func__,__LINE__);
		#ifdef GSL_GESTURE//crystal 0509  tjf
			if(gsl_gesture_flag == 0){
				enable_irq(touch_irq); 
			}
		#else
				enable_irq(touch_irq); 
...
static int __init gsl_driver_init(void)
{
	...
	gsl_err("gsl_driver init error, return num is  \n");
