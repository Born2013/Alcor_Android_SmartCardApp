						...
						u32 len, u8 const *data)
{
	...
	* dma_msg_buf_va = ICM206XX_REG_MEM_R_W;
	memcpy(dma_msg_buf_va + 1, data, len);
	/* write message */
	msgs[0].addr = client->addr;
	msgs[0].flags = 0;
	msgs[0].buf = bank;
	...
	msgs[2].addr = client->addr | I2C_DMA_FLAG;
	msgs[2].flags = 0;
	msgs[2].buf = (u8 *)dma_msg_buf_pa;
	msgs[2].len = len + 1;
...
static int icm206xx_SetSampleRate(struct i2c_client *client, int sensor_type, u64 delay_ns, bool force_1khz)
{
	...
	/* ---------------------------------------------------- */		
		sample_rate = (int)delay_ns / 1000 / 1000;
