...
#define XY_THR_N      (-70)
#define XY_THR_P      (70)
#define Z_THR_MIN_N   (-650)
#define Z_THR_MIN_P   (650)
#define Z_THR_MAX_N   (-1350)
#define Z_THR_MAX_P   (1350)
#define Z_DOT         (20)
...
static const struct of_device_id accel_of_match[] = {
	{.compatible = "mediatek,gsensor"},//加_1（gsensor_1）代表会做兼容，和dts文件里面对应
	{},
};
#endif
...
//  add by djq 2017-08-02 start	
	static s16 axis_z_off = Z_OFF_DEFAULT;//static s16 axis_z_off = 0;
	static u16 index = 0;
	static s16 z_value[Z_DOT] ={0};
	int flag_x = 0;
	int flag_y = 0;
	int flag_z = 0;
	int i = 0;
...
//  add by djq 2017-08-02 start	
        ...
        if(index == Z_DOT)	
        {
			...
			for(i = 0; i < Z_DOT; i++)
			{
				temp_value += z_value[i];
			}
			temp_value = temp_value / Z_DOT;
			
			if(temp_value > Z_THR_MAX_N && temp_value < Z_THR_MIN_N) 		
				axis_z_off = -1024 - temp_value;
			else 
				axis_z_off = 1024 - temp_value;	
...
static int sc7a20_i2c_remove(struct i2c_client *client)
{
	...
	if(err)
	{
        ...
        err = misc_deregister(&sc7a20_device);
	if(err)
	{
