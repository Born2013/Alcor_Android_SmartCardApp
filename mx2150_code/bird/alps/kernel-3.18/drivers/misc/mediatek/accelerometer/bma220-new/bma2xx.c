...
static struct i2c_board_info __initdata bma_i2c_info = {
	I2C_BOARD_INFO(BMA_DEV_NAME, 0x0A) //BMA_I2C_ADDRESS(BMA2X2, PIN_HIGH))
};
*/
		...
		enum BMA_POWERMODE_ENUM power_mode)
{
    ...
    {
                mutex_unlock(&obj->lock);//add by jcl
                return 0;
    }
 

	if (obj->sensor_type == BMA220_TYPE) {/* BMA220 */
		u8 expectation, retry = 10;
		...
		*/
		...
		} while ((data != expectation) && (--retry > 0));
		...
		char *buf, int bufsize)
{
	...
	//GSE_ERR("wxc.04---------------acc[obj->cvt.map[BMA_AXIS_X]] :%d,%d,%d,--%d\n",acc[obj->cvt.map[BMA_AXIS_X]] ,obj->cvt.sign[BMA_AXIS_X] ,databuf[BMA_AXIS_X],obj->sensitivity);
	obj->sensitivity = 16;
	#if 1
		/* convert: LSB -> m/s^2 */
		...
		//if (atomic_read(&obj->trace) & ADX_TRC_IOCTL)
			//GSE_ERR("gsensor data: %s GRAVITY_EARTH_1000:%d\n", buf, GRAVITY_EARTH_1000);
	}
...
#if 1
static int bma_local_init(void)
{
	bma_probe(NULL);
	return 0;
}
...
static int bma_local_remove(void)
{
	bma_remove(NULL);
	return 0;
}
...
//  obj->hw = get_cust_acc_hw();
	...
	printk("[bma2xx] obj->client->addr = 0x%x\n", hw->i2c_addr[0]);
...
static int __init bma_init(void)
{
	...
	const char *name = "mediatek,cust_bma2xx";
