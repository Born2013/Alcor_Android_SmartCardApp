...
#include "../../step_counter/icm206xx_sc_bird/icm206xx_sc.h"
...
static int icm206xx_accel_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (atomic_read(&obj->trace)) 
	{
        ...
        #if defined(CONFIG_MTK_ICM206XX_SC)
	res = read_sensor_data();
        #endif
	return ICM206XX_SUCCESS;
}
...
#if defined(MSG_DMA_MODE)
//vitrual address
u8 * dma_msg_buf_va = NULL;
//physical address
uintptr_t  dma_msg_buf_pa;


static int dma_malloc(void)
{
	dma_msg_buf_va = (u8 *)dma_alloc_coherent(NULL, 4096, (dma_addr_t *)&dma_msg_buf_pa, GFP_KERNEL);
	if (!dma_msg_buf_va)
	{
		printk("***delfino malloc dma address failed!***\n");
		return -1;
	}
	printk("###delfino malloc dma address success!###\n");
	return 0;
}
...
static int dma_release(void)
{
	if (dma_msg_buf_va)
	{
		dma_free_coherent(NULL, 4096, dma_msg_buf_va, dma_msg_buf_pa);
		dma_msg_buf_va = NULL;
		printk("###release dma resources!\n###");
	}
	return 0;
}
#endif
static int icm206xx_accel_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		goto exit_init_failed;
		printk("delfino error when init icm206xxx\n");
	}
