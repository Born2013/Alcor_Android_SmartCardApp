...
static int inv_i2c_single_write(struct i2c_client *client,
						...
						u32 len, u8 const *data)
{
	...
	while (len > 0) 
	{
    	...
    	//bank[1] = mem_addr >> 8;
		inv_i2c_single_write(i2c, ICM206XX_REG_MEM_BANK_SEL, (unsigned char)(mem_addr >> 8));
    	//addr[0] = MPUREG_MEM_START_ADDR;
    	//addr[1] = mem_addr & 0xFF;
		inv_i2c_single_write(i2c, ICM206XX_REG_MEM_START_ADDR, (unsigned char)(mem_addr & 0xFF));
		//buf[0] = MPUREG_MEM_R_W;
	    //memcpy(buf + 1, data, length);
		buffer[0] = ICM206XX_REG_MEM_R_W;
		memcpy(buffer+1, data, write_size);
		if(i2c_master_send(i2c, buffer, write_size + 1) <= 0)
		{
						...
						u32 len, u8 *data)
{
	...
	while (size > 0) {
		...
		if(i2c_master_recv(client, data, read_size) <= 0)
		{
			printk(KERN_INFO "%s: transfer failed.", __func__);
			mutex_unlock(&icm206xx_accel_i2c_mutex);
			return -EIO;
