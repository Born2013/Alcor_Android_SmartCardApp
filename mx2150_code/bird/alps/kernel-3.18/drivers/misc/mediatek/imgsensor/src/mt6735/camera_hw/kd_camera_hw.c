...
#include <mt-plat/mt_pwm.h>//add by Crystal Shen(BIRD_C) 20160401
#include <mt-plat/upmu_common.h>//add by Crystal Shen(BIRD_C) 20160401
/******************************************************************************
 * Debug configuration
******************************************************************************/
...
#define VOL_1000 1000000
	...
	if (DUAL_CAMERA_MAIN_SENSOR == SensorIdx)
	   {	pinSetIdx = 0;
                bird_imgsensor_flag = 0;
           }
	else if (DUAL_CAMERA_SUB_SENSOR == SensorIdx)
	    {    pinSetIdx = 1;
                 bird_imgsensor_flag = 1;
	    }
	...
	/* power ON */
	if (On) {
		mtkcam_gpio_set(pinSetIdx, CAMLDO, GPIO_OUT_ONE);//add by Crystal Shen(BIRD_C) 20160401
		...
		if ((currSensorName && (0 == strcmp(currSensorName, "imx135mipiraw"))) ||
			...
			if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMPDN]) {
                		if(mtkcam_gpio_set(1-pinSetIdx, CAMPDN, pinSet[1-pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON])) {
            		...
            		if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMRST]) {
                		...
                		if(mtkcam_gpio_set(1-pinSetIdx, CAMRST, pinSet[1-pinSetIdx][IDX_PS_CMRST + IDX_PS_OFF])) {
		...
		mtkcam_gpio_set(pinSetIdx, CAMLDO, GPIO_OUT_ZERO);//add by Crystal Shen(BIRD_C) 20160401
		if (pinSetIdx == 0)
			ISP_MCLK1_EN(0);
		else if (pinSetIdx == 1)
			ISP_MCLK1_EN(0);
