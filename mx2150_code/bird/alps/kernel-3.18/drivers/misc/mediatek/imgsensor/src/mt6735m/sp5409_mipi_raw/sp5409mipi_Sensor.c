...
static imgsensor_info_struct imgsensor_info = {
    ...
    .i2c_addr_table = {0x7a, 0xff}, //5409H---0x78 /5409---0x7a
    .i2c_speed = 200,
};
...
static imgsensor_struct imgsensor = {
    ...
    .i2c_write_id = 0x7a,//record current sensor's i2c write id //5409H---0x78 /5409---0x7a
};
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("2015-2-6 sp5409 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("2015-2-6 sp5409 Read sensor id 0x%x fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 set_test_pattern_mode ( kal_bool enable)
{
 //   LOG_INF("enable: %d\n", enable);
   ...
   if( enable) {  
		...
		write_cmos_sensor(0xfb,0x35);  //abl
		write_cmos_sensor(0x01,0x01);
    	}
    spin_lock(&imgsensor_drv_lock);
