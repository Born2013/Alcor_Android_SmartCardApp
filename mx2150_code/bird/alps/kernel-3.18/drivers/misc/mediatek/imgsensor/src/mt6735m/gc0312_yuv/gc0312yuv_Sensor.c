...
#define SENSORDB(x,...)
#endif
#define GC0312_BANDING_50HZ    0
#define GC0312_BANDING_60HZ    1
kal_bool GC0312_night_mode_enable = KAL_FALSE;
kal_uint16 GC0312_CurStatus_AWB = 0;
kal_bool GC0312_banding_state = GC0312_BANDING_50HZ;      // 0~50hz; 1~60hz
...
static kal_uint32 GC0312_g_fPV_PCLK = 30 * 1000000;
...
static void GC0312_set_AE_mode(kal_bool AE_enable)
{

	SENSORDB("[GC0312]enter GC0312_set_AE_mode function, AE_enable = %d\n ", AE_enable);
    if (AE_enable == KAL_TRUE)
    {
        // turn on AEC/AGC
			SENSORDB("[GC0312]enter GC0312_set_AE_mode function 1\n ");
    ...
    {
        // turn off AEC/AGC
			SENSORDB("[GC0312]enter GC0312_set_AE_mode function 2\n ");        
			GC0312_write_cmos_sensor(0xfe, 0x00);
			GC0312_write_cmos_sensor(0x4f, 0x00);
    }   
}
UINT32 GC0312_SetMaxFramerateByScenario(
  MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate)
{
	SENSORDB("scenarioId = %d\n", scenarioId);
	return 0;
}
void GC0312_GetDelayInfo(uintptr_t delayAddr)
{
...
UINT32 GC0312_GetDefaultFramerateByScenario(
...
void GC0312_SetMaxMinFps(UINT32 u2MinFrameRate, UINT32 u2MaxFrameRate)
{
	SENSORDB("GC0312_SetMaxMinFps+ :FrameRate= %d %d\n",u2MinFrameRate,u2MaxFrameRate);
	MINFramerate = u2MinFrameRate;
	MAXFramerate = u2MaxFrameRate;
	return;
}
void GC0312_3ACtrl(ACDK_SENSOR_3A_LOCK_ENUM action)
{
	SENSORDB("[GC0329]enter ACDK_SENSOR_3A_LOCK_ENUM function:action=%d\n",action);
   switch (action)
   {
      case SENSOR_3A_AE_LOCK:
          GC0312_set_AE_mode(KAL_FALSE);
      break;
      case SENSOR_3A_AE_UNLOCK:
          GC0312_set_AE_mode(KAL_TRUE);
      break;
      ...
      case SENSOR_3A_AWB_UNLOCK:
		   if (((AWB_MODE_OFF == GC0312_CurStatus_AWB) ||
        		(AWB_MODE_AUTO == GC0312_CurStatus_AWB)))
        	{	
...
void GC0312_GetExifInfo(uintptr_t exifAddr)
{
    SENSOR_EXIF_INFO_STRUCT* pExifInfo = (SENSOR_EXIF_INFO_STRUCT*)exifAddr;
    pExifInfo->FNumber = 28;
//    pExifInfo->AEISOSpeed = GC0312_Driver.isoSpeed;
//    pExifInfo->AWBMode = S5K4ECGX_Driver.awbMode;
//    pExifInfo->CapExposureTime = S5K4ECGX_Driver.capExposureTime;
//    pExifInfo->FlashLightTimeus = 0;
//   pExifInfo->RealISOValue = (S5K4ECGX_ReadGain()*57) >> 8;
...
void GC0312_Sensor_Init(void)
{
	...
	GC0312_write_cmos_sensor(0x03, 0x07);
	GC0312_write_cmos_sensor(0x04, 0x38);
...
*/
	...
	/*
		GC0312_write_cmos_sensor(0xfe, 0x00); 
		GC0312_write_cmos_sensor(0x05, 0x00);
		GC0312_write_cmos_sensor(0x06, 0x90); //HB
		...
		GC0312_write_cmos_sensor(0x26, 0xb3);	//anti-flicker step [7:0]
		
		GC0312_write_cmos_sensor(0x27, 0x02);	//exp level 0  20fps
		GC0312_write_cmos_sensor(0x28, 0x19); 
		GC0312_write_cmos_sensor(0x29, 0x05);	//exp level 1  12.50fps
		GC0312_write_cmos_sensor(0x2a, 0x98); 
		GC0312_write_cmos_sensor(0x2b, 0x06); //7.14fps
		GC0312_write_cmos_sensor(0x2c, 0x4b); 
		GC0312_write_cmos_sensor(0x2d, 0x06);	//exp level 3  5.55fps
		GC0312_write_cmos_sensor(0x2e, 0x4b); 
	*/


	GC0312_write_cmos_sensor(0xfe, 0x00); 
	GC0312_write_cmos_sensor(0x05, 0x01);
	GC0312_write_cmos_sensor(0x06, 0x18); //HB
	GC0312_write_cmos_sensor(0x07, 0x00);
	GC0312_write_cmos_sensor(0x08, 0x10); //VB
	
	GC0312_SET_PAGE1;
	GC0312_write_cmos_sensor(0x25, 0x00);	//anti-flicker step [11:8]
	GC0312_write_cmos_sensor(0x26, 0x9a);	//anti-flicker step [7:0]
	
	GC0312_write_cmos_sensor(0x27, 0x01);	//exp level 0  20fps
	GC0312_write_cmos_sensor(0x28, 0xce); 
	GC0312_write_cmos_sensor(0x29, 0x03);	//exp level 1  12.50fps
	GC0312_write_cmos_sensor(0x2a, 0x9c); 
	GC0312_write_cmos_sensor(0x2b, 0x05); //7.14fps
	GC0312_write_cmos_sensor(0x2c, 0x6a); 
	GC0312_write_cmos_sensor(0x2d, 0x05);	//exp level 3  5.55fps
	GC0312_write_cmos_sensor(0x2e, 0x6a); 
        ...
        MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    pSensorInfo->CaptureDelayFrame = 1; // 2  2 4 
        ...
        MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    {
    case MSDK_SCENARIO_ID_VIDEO_PREVIEW:   
    	SENSORDB("GC0312 Camera Video Preview!\n");
        GC0312_MPEG4_encode_mode = KAL_TRUE;
...
*/
			...
			GC0312_write_cmos_sensor(0x27, 0x01);   //exp level 0  20fps
			GC0312_write_cmos_sensor(0x28, 0xce); 
			GC0312_write_cmos_sensor(0x29, 0x01);   //exp level 1  12.50fps
			GC0312_write_cmos_sensor(0x2a, 0xce); 
			GC0312_write_cmos_sensor(0x2b, 0x01); //7.14fps
			GC0312_write_cmos_sensor(0x2c, 0xce); 
			GC0312_write_cmos_sensor(0x2d, 0x01);   //exp level 3  5.55fps
			GC0312_write_cmos_sensor(0x2e, 0xce); 
			GC0312_SET_PAGE0;
		...
		case AE_FLICKER_MODE_60HZ:
			GC0312_write_cmos_sensor(0xfe, 0x00); 
			GC0312_write_cmos_sensor(0x05, 0x00);	
			GC0312_write_cmos_sensor(0x06, 0xfd); 
			GC0312_write_cmos_sensor(0x07, 0x00);
			GC0312_write_cmos_sensor(0x08, 0x10);
			
			GC0312_SET_PAGE1;
			GC0312_write_cmos_sensor(0x25, 0x00);	//anti-flicker step [11:8]
			GC0312_write_cmos_sensor(0x26, 0x84);	//anti-flicker step [7:0]
			
			GC0312_write_cmos_sensor(0x27, 0x01);	//exp level 0  20fps
			GC0312_write_cmos_sensor(0x28, 0x8c); 
			GC0312_write_cmos_sensor(0x29, 0x01);	//exp level 1  13fps
			GC0312_write_cmos_sensor(0x2a, 0x8c); 
			GC0312_write_cmos_sensor(0x2b, 0x01);	//exp level 2  7.5fps
			GC0312_write_cmos_sensor(0x2c, 0x8c); 
			GC0312_write_cmos_sensor(0x2d, 0x01);	//exp level 3  5.45fps
			GC0312_write_cmos_sensor(0x2e, 0x8c); 
			GC0312_SET_PAGE0;
...
void set_fixframerate_by_banding(UINT16 u2FrameRate,  kal_bool bBanding)
{
    if ( 30 == u2FrameRate){
		...
		if ( GC0312_BANDING_50HZ == bBanding )
		{
		/*
			GC0312_write_cmos_sensor(0xfe, 0x00); 
			GC0312_write_cmos_sensor(0x05, 0x00);
			...
			GC0312_write_cmos_sensor(0x27, 0x02);   //exp level 0  20fps
			GC0312_write_cmos_sensor(0x28, 0x19); 
			GC0312_write_cmos_sensor(0x29, 0x02);   //exp level 1  12.50fps
			GC0312_write_cmos_sensor(0x2a, 0x19); 
			GC0312_write_cmos_sensor(0x2b, 0x02); //7.14fps
			GC0312_write_cmos_sensor(0x2c, 0x19); 
			GC0312_write_cmos_sensor(0x2d, 0x02);   //exp level 3  5.55fps
			GC0312_write_cmos_sensor(0x2e, 0x19); 
		...
		else if ( GC0312_BANDING_60HZ == bBanding  )
		{
		/*
			GC0312_write_cmos_sensor(0xfe, 0x00); 
			GC0312_write_cmos_sensor(0x05, 0x00);	
			GC0312_write_cmos_sensor(0x06, 0xae); 
			GC0312_write_cmos_sensor(0x07, 0x00);
			GC0312_write_cmos_sensor(0x08, 0x22);
			
			GC0312_SET_PAGE1;
			GC0312_write_cmos_sensor(0x25, 0x00);	//anti-flicker step [11:8]
			GC0312_write_cmos_sensor(0x26, 0x90);	//anti-flicker step [7:0]
			
			GC0312_write_cmos_sensor(0x27, 0x01);	//exp level 0  20fps
			GC0312_write_cmos_sensor(0x28, 0xb0); 
			GC0312_write_cmos_sensor(0x29, 0x01);	//exp level 1  13fps
			GC0312_write_cmos_sensor(0x2a, 0xb0); 
			GC0312_write_cmos_sensor(0x2b, 0x01);	//exp level 2  7.5fps
			GC0312_write_cmos_sensor(0x2c, 0xb0); 
			GC0312_write_cmos_sensor(0x2d, 0x01);	//exp level 3  5.45fps
			GC0312_write_cmos_sensor(0x2e, 0xb0); 
			*/
	...
	else if  ( 15 == u2FrameRate)
	{
		...
		/*
			...
			GC0312_write_cmos_sensor(0x27, 0x04);   //exp level 0  20fps
			GC0312_write_cmos_sensor(0x28, 0x32); 
			GC0312_write_cmos_sensor(0x29, 0x04);   //exp level 1  12.50fps
			GC0312_write_cmos_sensor(0x2a, 0x32); 
			GC0312_write_cmos_sensor(0x2b, 0x04); //7.14fps
			GC0312_write_cmos_sensor(0x2c, 0x32); 
			GC0312_write_cmos_sensor(0x2d, 0x04);   //exp level 3  5.55fps
			GC0312_write_cmos_sensor(0x2e, 0x32); 
			*/
		...
		GC0312_write_cmos_sensor(0x28, 0x9c); 
		GC0312_write_cmos_sensor(0x29, 0x03);	//exp level 1  12.50fps
		GC0312_write_cmos_sensor(0x2a, 0x9c); 
		GC0312_write_cmos_sensor(0x2b, 0x03); //7.14fps
		GC0312_write_cmos_sensor(0x2c, 0x9c); 
		GC0312_write_cmos_sensor(0x2d, 0x03);	//exp level 3  5.55fps
		...
		/*
			...
			GC0312_write_cmos_sensor(0x2d, 0x03);	//exp level 3  5.45fps
...
void set_freeframerate_by_banding(UINT16 u2minFrameRate, UINT16 u2maxFrameRate,  kal_bool bBanding)
{
	if ((5 == u2minFrameRate) && (30 == u2maxFrameRate))
	{
		...
		/*
			...
			GC0312_write_cmos_sensor(0x29, 0x05);   //exp level 1  12.50fps
			GC0312_write_cmos_sensor(0x2a, 0x98); 
			GC0312_write_cmos_sensor(0x2b, 0x08); //7.14fps
			GC0312_write_cmos_sensor(0x2c, 0x64); 
			GC0312_write_cmos_sensor(0x2d, 0x0c);   //exp level 3  5.55fps
			GC0312_write_cmos_sensor(0x2e, 0x96); 
			*/
		...
		GC0312_write_cmos_sensor(0x29, 0x03);	//exp level 1  12.50fps
		GC0312_write_cmos_sensor(0x2a, 0x9c); 
		GC0312_write_cmos_sensor(0x2b, 0x07); //7.14fps
		GC0312_write_cmos_sensor(0x2c, 0x38); 
		GC0312_write_cmos_sensor(0x2d, 0x0a);	//exp level 3  5.55fps
		GC0312_write_cmos_sensor(0x2e, 0x3a); 
		...
		/*
			...
			GC0312_write_cmos_sensor(0x27, 0x01);	//exp level 0  20fps
			GC0312_write_cmos_sensor(0x28, 0xb0); 
			GC0312_write_cmos_sensor(0x29, 0x04);	//exp level 1  13fps
			GC0312_write_cmos_sensor(0x2a, 0x80); 
			GC0312_write_cmos_sensor(0x2b, 0x06);	//exp level 2  7.5fps
			GC0312_write_cmos_sensor(0x2c, 0xc0); 
			GC0312_write_cmos_sensor(0x2d, 0x0b);	//exp level 3  5.45fps
			GC0312_write_cmos_sensor(0x2e, 0xd0); 
			*/
			...
			GC0312_write_cmos_sensor(0x29, 0x03);	//exp level 1  13fps
			GC0312_write_cmos_sensor(0x2a, 0x9c); 
			GC0312_write_cmos_sensor(0x2b, 0x05);	//exp level 2  7.5fps
			GC0312_write_cmos_sensor(0x2c, 0xac); 
			GC0312_write_cmos_sensor(0x2d, 0x0a);	//exp level 3  5.45fps
			GC0312_write_cmos_sensor(0x2e, 0xd4); 
	...
	else if ((10 == u2minFrameRate) && (30 == u2maxFrameRate))
	{
		...
		/*
			...
			GC0312_write_cmos_sensor(0x2b, 0x05);	//exp level 2  7.5fps
			GC0312_write_cmos_sensor(0x2c, 0xa0); 
			GC0312_write_cmos_sensor(0x2d, 0x05);	//exp level 3  5.45fps
			GC0312_write_cmos_sensor(0x2e, 0xa0); 
			...
			*/
			...
			GC0312_write_cmos_sensor(0x2a, 0x9c); 
			GC0312_write_cmos_sensor(0x2b, 0x05);	//exp level 2  7.5fps
			GC0312_write_cmos_sensor(0x2c, 0x28); 
			GC0312_write_cmos_sensor(0x2d, 0x05);	//exp level 3  5.45fps
			GC0312_write_cmos_sensor(0x2e, 0x28); 
...
UINT32 GC0312YUVSetVideoMode(UINT16 u2FrameRate)    // lanking add
{
	...
	set_fixframerate_by_banding(30, GC0312_banding_state);
		
   	}
    else if (u2FrameRate == 15)       
    	{
    	
       	set_fixframerate_by_banding(15, GC0312_banding_state);
...
void GC0312NightMode(kal_bool bEnable)
{
	// LOG_INF("GC0312Night, Modeenable(%d)\n", bEnable);
         // LOG_INF("GC0312_banding_state = %d\n", GC0312_banding_state);


	if(bEnable) // night mode for preview and video
	{
        // LOG_INF("set_night_mode for preview\n");
        set_freeframerate_by_banding(5, 30, GC0312_banding_state);
	...
	else  // normal mode for preview and video (not night)
	{
        ...
        set_freeframerate_by_banding(10, 30, GC0312_banding_state);
  ...
  //  UINT32 i;
	...
	*pFeatureParaLen=4;
	...
	case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
		 SENSORDB("[GC0312] F_SET_MAX_FRAME_RATE_BY_SCENARIO.\n");
		 GC0312_SetMaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*feature_data, *(feature_data+1));
		 break;
    case SENSOR_FEATURE_GET_DELAY_INFO:
        SENSORDB("[GC0312] F_GET_DELAY_INFO\n");
        GC0312_GetDelayInfo((uintptr_t)*feature_data);
    break;

    case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
         SENSORDB("[GC0312] F_GET_DEFAULT_FRAME_RATE_BY_SCENARIO\n");
         GC0312_GetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*feature_data, (MUINT32 *)(uintptr_t)(*(feature_data+1)));
    break;   
    
	case SENSOR_FEATURE_SET_YUV_3A_CMD:
		 SENSORDB("[GC0312] SENSOR_FEATURE_SET_YUV_3A_CMD ID:%d\n", *pFeatureData32);
		 GC0312_3ACtrl((ACDK_SENSOR_3A_LOCK_ENUM)*feature_data);
		 break;


	case SENSOR_FEATURE_GET_EXIF_INFO:
		 //SENSORDB("[4EC] F_GET_EXIF_INFO\n");
		 GC0312_GetExifInfo((uintptr_t)*feature_data);
		 break;

	case SENSOR_FEATURE_GET_AE_AWB_LOCK_INFO:
		 SENSORDB("[GC0310] F_GET_AE_AWB_LOCK_INFO\n");
		 //GC0312_get_AEAWB_lock((uintptr_t)(*feature_data), (uintptr_t)*(feature_data+1));
	break;    
	case SENSOR_FEATURE_SET_MIN_MAX_FPS:
		SENSORDB("SENSOR_FEATURE_SET_MIN_MAX_FPS:[%d,%d]\n",*pFeatureData32,*(pFeatureData32+1)); 
		GC0312_SetMaxMinFps((UINT32)*feature_data, (UINT32)*(feature_data+1));
	break; 
    default:
        break;
	}
