...
#include "gc5005sub_mipi_Sensor.h"

extern int bird_imgsensor_flag;

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC5005SUB_camera_sensor"
#define LOG_1 LOG_INF("GC5005SUB,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define GC5005SUBOTP_FOR_CUSTOMER

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC5005SubDuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC5005SUB_MIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#ifdef GC5005SUBOTP_FOR_CUSTOMER
...
static gc5005sub_otp gc5005sub_otp_info;
...
static kal_uint8 gc5005sub_read_otp(kal_uint8 addr)
{
	kal_uint8 value;
...
static void gc5005sub_select_page_otp(otp_page otp_select_page)
{
	kal_uint8 page;
...
static void gc5005sub_gcore_read_otp_info(void)
{
...
#endif	
	memset(&gc5005sub_otp_info,0,sizeof(gc5005sub_otp));

	/*TODO*/
	gc5005sub_select_page_otp(otp_page0);
	flag0 = gc5005sub_read_otp(0x00);
	total_number0 = gc5005sub_read_otp(0x01) + gc5005sub_read_otp(0x02);
	LOG_INF("GC5005SUB_OTP : flag0 = 0x%x , total_number0 = %d\n",flag0,total_number0);
	
	gc5005sub_select_page_otp(otp_page1);
	total_number1 = gc5005sub_read_otp(0x28) + gc5005sub_read_otp(0x29);
	LOG_INF("GC5005SUB_OTP : total_number1 = %d\n",total_number1);
/*
//For GC
	gc5005sub_otp_info.gc_flag = (flag0>>2)&0x03;
	LOG_INF("GC5005SUB_OTP : gc5005sub_otp_info.gc_flag = 0x%x \n",gc5005sub_otp_info.gc_flag);

	if(gc5005sub_otp_info.gc_flag==0x01) 
	{
		gc5005sub_otp_info.Vald1=0xb0;
		gc5005sub_otp_info.Val21=0x08;
		gc5005sub_otp_info.Val29=0x14;
	}
	else
	{
		gc5005sub_otp_info.Vald1=0xa0;
		gc5005sub_otp_info.Val21=0x0a;
		gc5005sub_otp_info.Val29=0x22;
	}
*/
//DD
	switch(flag0&0x03)
	{
	case 0x00:
		LOG_INF("GC5005SUB_OTP_DD is Empty !!\n");
		gc5005sub_otp_info.dd_flag = 0x00;
		break;
	case 0x01:	
		LOG_INF("GC5005SUB_OTP_DD is Valid!!\n");
		...
		LOG_INF("GC5005SUB_OTP_DD total_number=%d!!\n",total_number);
		...
		for(i=0; i<total_number; i++)
		{
			...
			check_dd_flag = gc5005sub_read_otp(dd_rom_start + 4*(i-offset) + 3);
				
			if(check_dd_flag&0x10)
			{//Read OTP
				type = check_dd_flag&0x0f;
		
				dd0 = gc5005sub_read_otp(dd_rom_start + 4*(i-offset));
				dd1 = gc5005sub_read_otp(dd_rom_start + 4*(i-offset) + 1); 	
				dd2 = gc5005sub_read_otp(dd_rom_start + 4*(i-offset) + 2);
				x = ((dd1&0x0f)<<8) + dd0;
				y = (dd2<<4) + ((dd1&0xf0)>>4);
				LOG_INF("GC5005SUB_OTP_DD : type = %d , x = %d , y = %d \n",type,x,y);
				LOG_INF("GC5005SUB_OTP_DD : dd0 = %d , dd1 = %d , dd2 = %d \n",dd0,dd1,dd2);
				
				if(type == 3)
				{
					for(j=0; j<4; j++)
					{
						gc5005sub_otp_info.dd_param_x[cnt] = x;
						gc5005sub_otp_info.dd_param_y[cnt] = y + j;
						gc5005sub_otp_info.dd_param_type[cnt++] = 2;
					}
				}
/*
				else if(type == 2)
...
*/				
				...
				{
					gc5005sub_otp_info.dd_param_x[cnt] = x;
					gc5005sub_otp_info.dd_param_y[cnt] = y;
					gc5005sub_otp_info.dd_param_type[cnt++] = type;
				}
			}
			else
			{
				LOG_INF("GC5005SUB_OTP_DD:check_id[%d] = %x,checkid error!!\n",i,check_dd_flag);
			}
		}
		gc5005sub_otp_info.dd_cnt = cnt;
		gc5005sub_otp_info.dd_flag = 0x01;
		break;
	case 0x02:
	case 0x03:	
		LOG_INF("GC5005SUB_OTP_DD is Invalid !!\n");
		gc5005sub_otp_info.dd_flag = 0x02;
		break;
	default :
		break;
	}
...
#ifdef GC5005SUBOTP_FOR_CUSTOMER	
	gc5005sub_select_page_otp(otp_page1);
	flag1 = gc5005sub_read_otp(0x00);
	flag_golden = gc5005sub_read_otp(0x13);
	LOG_INF("GC5005SUB_OTP : flag1 = 0x%x , flag_golden = 0x%x\n",flag1,flag_golden);
...
//INFO&WB
	for(index=0;index<2;index++)
	{
		...
		{
		case 0x00:
			LOG_INF("GC5005SUB_OTP_INFO group%d is Empty !!\n", index + 1);
			break;
		case 0x01:
			info_start_add = INFO_ROM_START + index * INFO_WIDTH;
			gc5005sub_otp_info.module_id = gc5005sub_read_otp(info_start_add);
			gc5005sub_otp_info.lens_id = gc5005sub_read_otp(info_start_add + 1); 
			gc5005sub_otp_info.vcm_driver_id = gc5005sub_read_otp(info_start_add + 2);
			gc5005sub_otp_info.vcm_id = gc5005sub_read_otp(info_start_add + 3);
			gc5005sub_otp_info.year = gc5005sub_read_otp(info_start_add + 4);
			gc5005sub_otp_info.month = gc5005sub_read_otp(info_start_add + 5);
			gc5005sub_otp_info.day = gc5005sub_read_otp(info_start_add + 6);
			break;
		case 0x02:
		case 0x03:	
			LOG_INF("GC5005SUB_OTP_INFO group%d is Invalid !!\n", index + 1);
			break;
		default :
			break;
		}
		...
		{
		case 0x00:
			LOG_INF("GC5005SUB_OTP_WB group%d is Empty !!\n", index + 1);
			gc5005sub_otp_info.wb_flag = gc5005sub_otp_info.wb_flag|0x00;
			break;
		case 0x01:
			LOG_INF("GC5005SUB_OTP_WB group%d is Valid !!\n", index + 1);						
			wb_start_add = WB_ROM_START + index * WB_WIDTH;
			gc5005sub_otp_info.rg_gain = gc5005sub_read_otp(wb_start_add);
			gc5005sub_otp_info.bg_gain = gc5005sub_read_otp(wb_start_add + 1); 

			if((0==gc5005sub_otp_info.rg_gain)||(0==gc5005sub_otp_info.bg_gain))
			{
				gc5005sub_otp_info.wb_flag = gc5005sub_otp_info.wb_flag|0x02;
				LOG_INF("GC5005SUB_OTP_WB group%d is Error ,wb rg or bg = 0!!\n", index + 1);							
			}
			else
			{
				gc5005sub_otp_info.wb_flag = gc5005sub_otp_info.wb_flag|0x01;
			}
			
			break;
		case 0x02:
		case 0x03:	
			LOG_INF("GC5005SUB_OTP_WB group%d is Invalid !!\n", index + 1);			
			gc5005sub_otp_info.wb_flag = gc5005sub_otp_info.wb_flag|0x02;
			break;
		default :
			break;
		...
		{
		case 0x00:
			LOG_INF("GC5005SUB_OTP_GOLDEN group%d is Empty !!\n", index + 1);
			gc5005sub_otp_info.golden_flag = gc5005sub_otp_info.golden_flag|0x00;					
			break;
		case 0x01:
			LOG_INF("GC5005SUB_OTP_GOLDEN group%d is Valid !!\n", index + 1);						
			golden_start_add = GOLDEN_ROM_START + index * GOLDEN_WIDTH;
			gc5005sub_otp_info.golden_rg= gc5005sub_read_otp(golden_start_add);
			gc5005sub_otp_info.golden_bg = gc5005sub_read_otp(golden_start_add + 1); 

			if((0==gc5005sub_otp_info.golden_rg)||(0==gc5005sub_otp_info.golden_bg))
			{
				gc5005sub_otp_info.golden_flag = gc5005sub_otp_info.golden_flag|0x02;
				LOG_INF("GC5005SUB_OTP_GOLDEN group%d is Error,golden rg or bg = 0!!\n", index + 1);							
			}
			else
			{
				gc5005sub_otp_info.golden_flag = gc5005sub_otp_info.golden_flag|0x01;						
			}

			break;
		case 0x02:
		case 0x03:	
			LOG_INF("GC5005SUB_OTP_GOLDEN group%d is Invalid !!\n", index + 1);	
			gc5005sub_otp_info.golden_flag = gc5005sub_otp_info.golden_flag|0x02;			
			break;
		default :
			break;
	...
	LOG_INF("GC5005SUB_OTP_INFO:module_id=0x%x\n",gc5005sub_otp_info.module_id);
	LOG_INF("GC5005SUB_OTP_INFO:lens_id=0x%x\n",gc5005sub_otp_info.lens_id);
	LOG_INF("GC5005SUB_OTP_INFO:vcm_id=0x%x\n",gc5005sub_otp_info.vcm_id);
	LOG_INF("GC5005SUB_OTP_INFO:vcm_driver_id=0x%x\n",gc5005sub_otp_info.vcm_driver_id);
	LOG_INF("GC5005SUB_OTP_INFO:data=%d-%d-%d\n",gc5005sub_otp_info.year,gc5005sub_otp_info.month,gc5005sub_otp_info.day);
	LOG_INF("GC5005SUB_OTP_WB:r/g=0x%x\n",gc5005sub_otp_info.rg_gain);
	LOG_INF("GC5005SUB_OTP_WB:b/g=0x%x\n",gc5005sub_otp_info.bg_gain);
	LOG_INF("GC5005SUB_OTP_GOLDEN:golden_rg=0x%x\n",gc5005sub_otp_info.golden_rg);
	LOG_INF("GC5005SUB_OTP_GOLDEN:golden_bg=0x%x\n",gc5005sub_otp_info.golden_bg);	
#endif	
}

static void gc5005sub_gcore_update_dd(void)
{
	...
	if(0x01 ==gc5005sub_otp_info.dd_flag)
	{
#if defined(IMAGE_NORMAL_MIRROR)
		for(i=0; i<gc5005sub_otp_info.dd_cnt; i++)
		{
			if(gc5005sub_otp_info.dd_param_type[i]==0)
			{	gc5005sub_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005sub_otp_info.dd_param_x[i] + 1;	}
			else if(gc5005sub_otp_info.dd_param_type[i]==1)
			{	gc5005sub_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005sub_otp_info.dd_param_x[i] - 1;	}
			else
			{	gc5005sub_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005sub_otp_info.dd_param_x[i];	}
		}
		...
		for(i=0; i<gc5005sub_otp_info.dd_cnt; i++)
		{
			...
			{	
				gc5005sub_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005sub_otp_info.dd_param_x[i]+1;
				gc5005sub_otp_info.dd_param_y[i]= WINDOW_HEIGHT - gc5005sub_otp_info.dd_param_y[i]+1;
			}
			else if(gc5005sub_otp_info.dd_param_type[i]==1)
			{
				gc5005sub_otp_info.dd_param_x[i]= WINDOW_WIDTH - gc5005sub_otp_info.dd_param_x[i]-1;
		...
		//y
		for(i=0 ; i< gc5005sub_otp_info.dd_cnt-1; i++) 
		{
			for(j = i+1; j < gc5005sub_otp_info.dd_cnt; j++) 
			{  
				if(gc5005sub_otp_info.dd_param_y[i] > gc5005sub_otp_info.dd_param_y[j])  
				{  
					temp_x = gc5005sub_otp_info.dd_param_x[i] ; gc5005sub_otp_info.dd_param_x[i] = gc5005sub_otp_info.dd_param_x[j] ;  gc5005sub_otp_info.dd_param_x[j] = temp_x;
					temp_y = gc5005sub_otp_info.dd_param_y[i] ; gc5005sub_otp_info.dd_param_y[i] = gc5005sub_otp_info.dd_param_y[j] ;  gc5005sub_otp_info.dd_param_y[j] = temp_y;
					temp_type = gc5005sub_otp_info.dd_param_type[i] ; gc5005sub_otp_info.dd_param_type[i] = gc5005sub_otp_info.dd_param_type[j]; gc5005sub_otp_info.dd_param_type[j]= temp_type;
				} 
			}
		...
		for(i=0; i<gc5005sub_otp_info.dd_cnt; i++)
		{
			if(gc5005sub_otp_info.dd_param_y[i]==gc5005sub_otp_info.dd_param_y[i+1])
			{
				s=i++;
				while((gc5005sub_otp_info.dd_param_y[s] == gc5005sub_otp_info.dd_param_y[i+1])&&(i<gc5005sub_otp_info.dd_cnt-1))
					i++;
				e=i;
				...
				for(n=s; n<e; n++)
				{
					for(m=n+1; m<e+1; m++)
					{
						if(gc5005sub_otp_info.dd_param_x[n] > gc5005sub_otp_info.dd_param_x[m])
						{
							temp_x = gc5005sub_otp_info.dd_param_x[n] ; gc5005sub_otp_info.dd_param_x[n] = gc5005sub_otp_info.dd_param_x[m] ;  gc5005sub_otp_info.dd_param_x[m] = temp_x;
							temp_y = gc5005sub_otp_info.dd_param_y[n] ; gc5005sub_otp_info.dd_param_y[n] = gc5005sub_otp_info.dd_param_y[m] ;  gc5005sub_otp_info.dd_param_y[m] = temp_y;
							temp_type = gc5005sub_otp_info.dd_param_type[n] ; gc5005sub_otp_info.dd_param_type[n] = gc5005sub_otp_info.dd_param_type[m]; gc5005sub_otp_info.dd_param_type[m]= temp_type;
						}
					}
				}
		...
		for(i=0; i<gc5005sub_otp_info.dd_cnt; i++)
		{
			temp_val0 = gc5005sub_otp_info.dd_param_x[i]& 0x00ff;
			temp_val1 = ((gc5005sub_otp_info.dd_param_y[i]<<4)& 0x00f0) + ((gc5005sub_otp_info.dd_param_x[i]>>8)&0X000f);
			temp_val2 = (gc5005sub_otp_info.dd_param_y[i]>>4) & 0xff;
			write_cmos_sensor(0xaa,i);
			write_cmos_sensor(0xac,temp_val0);
			write_cmos_sensor(0xac,temp_val1);
			write_cmos_sensor(0xac,temp_val2);
			while((i < gc5005sub_otp_info.dd_cnt - 1) && (gc5005sub_otp_info.dd_param_x[i]==gc5005sub_otp_info.dd_param_x[i+1]) && (gc5005sub_otp_info.dd_param_y[i]==gc5005sub_otp_info.dd_param_y[i+1]))
				{
					flag = 1;
					i++;
				}
			if(flag)
				...
				write_cmos_sensor(0xac,gc5005sub_otp_info.dd_param_type[i]);

			LOG_INF("GC5005SUB_OTP_GC val0 = 0x%x , val1 = 0x%x , val2 = 0x%x \n",temp_val0,temp_val1,temp_val2);
			LOG_INF("GC5005SUB_OTP_GC x = %d , y = %d \n",((temp_val1&0x0f)<<8) + temp_val0,(temp_val2<<4) + ((temp_val1&0xf0)>>4));	
		}
...
#ifdef GC5005SUBOTP_FOR_CUSTOMER
static void gc5005sub_gcore_update_awb(void)
{
	...
	if(0x01==(gc5005sub_otp_info.golden_flag&0x01))
	{
		rg_typical=gc5005sub_otp_info.golden_rg;
		bg_typical=gc5005sub_otp_info.golden_bg;
		LOG_INF("GC5005SUB_OTP_UPDATE_AWB:rg_typical = 0x%x , bg_typical = 0x%x\n",rg_typical,bg_typical);		
	}

	if(0x01==(gc5005sub_otp_info.wb_flag&0x01))
	{	
		r_gain_current = 256 * rg_typical/gc5005sub_otp_info.rg_gain;
		b_gain_current = 256 * bg_typical/gc5005sub_otp_info.bg_gain;
		g_gain_current = 256;
		...
		LOG_INF("GC5005SUB_OTP_UPDATE_AWB:r_gain_current = 0x%x , b_gain_current = 0x%x , base_gain = 0x%x \n",r_gain_current,b_gain_current,base_gain);

		r_gain = 0x80 * r_gain_current / base_gain;
		g_gain = 0x80 * g_gain_current / base_gain;
		b_gain = 0x80 * b_gain_current / base_gain;
		LOG_INF("GC5005SUB_OTP_UPDATE_AWB:r_gain = 0x%x , g_gain = 0x%x , b_gain = 0x%x \n",r_gain,g_gain,b_gain);
...
static void gc5005sub_gcore_update_otp(void)
{
	gc5005sub_gcore_update_dd();
#ifdef GC5005SUBOTP_FOR_CUSTOMER	
	gc5005sub_gcore_update_awb();
#endif
}


static void gc5005sub_gcore_enable_otp(otp_state state)
{
	...
	{ 
		...
		LOG_INF("GC5005SUB_OTP: Enable OTP!\n");		
	}
	else			
	{
		otp_en = otp_en & 0x7f;
		...
		LOG_INF("GC5005SUB_OTP: Disable OTP!\n");
	}

}

static void gc5005sub_gcore_identify_otp(void)
{	
	...
	gc5005sub_gcore_enable_otp(otp_open);
	gc5005sub_gcore_read_otp_info();
	gc5005sub_gcore_enable_otp(otp_close);
}
...
{	
	...
	{
		write_cmos_sensor(0xfe, 0x00);          
		write_cmos_sensor(0x21, 0x0f);//gc5005sub_otp_info.Val21
		write_cmos_sensor(0x29, 0x22);//gc5005sub_otp_info.Val29
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xd1, 0xa0);//gc5005sub_otp_info.Vald1                                                                                                         
...
static kal_uint32 open(void)
{
    ...
    /*Don't Remove!!*/
    gc5005sub_gcore_identify_otp();

    /* initail sequence write in  */
    sensor_init();

    /*Don't Remove!!*/
    gc5005sub_gcore_update_otp();
...
UINT32 GC5005SUB_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC5005SUBMIPI_RAW_SensorInit    */
