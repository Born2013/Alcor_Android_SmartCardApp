...
void GC2145MIPI_Read_Information(kal_uint16 *Shutter, kal_uint8 *Analog_gain, kal_uint8 *Yaverage)
{
    	kal_uint8 temp_reg1, temp_reg2;

	GC2145MIPI_SET_PAGE0;
	temp_reg1 = GC2145MIPI_read_cmos_sensor(0x04);
	temp_reg2 = GC2145MIPI_read_cmos_sensor(0x03);
	*Shutter = (temp_reg1 & 0xFF) | (temp_reg2 << 8);
	printk("[Crystal_GC2145MIPI_FUNCTION][%s][Shutterr =0x%x]\n",__func__,*Shutter);

	*Analog_gain = GC2145MIPI_read_cmos_sensor(0x48);
	printk("[Crystal_GC2145MIPI_FUNCTION][%s][Analog_gain=0x%x]\n",__func__,*Analog_gain);

	GC2145MIPI_SET_PAGE1;
	*Yaverage = GC2145MIPI_read_cmos_sensor(0x14);
	printk("[Crystal_GC2145MIPI_FUNCTION][%s][Yaverage=0x%x]\n",__func__,*Yaverage);
	GC2145MIPI_SET_PAGE0;
} /* GC2145MIPI_Read_Information */
...
kal_uint16 bird_sensor_id_gc2145_for_cover = 0;
EXPORT_SYMBOL(bird_sensor_id_gc2145_for_cover);
...
UINT32 GC2145MIPIOpen(void)
{
	...
	for(i=0;i<3;i++)
	{
		...
		bird_sensor_id_gc2145_for_cover	= sensor_id; 
		SENSORDB("GC2145MIPI_Open, sensor_id:%x \n",sensor_id);
		if (sensor_id != GC2145_SENSOR_ID){
		bird_sensor_id_gc2145_for_cover = 0;	
		return ERROR_SENSOR_CONNECT_FAIL;
	     }
	}
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
   ...
   // kal_uint16 AE_setting_delay = 0;
    ...
    shutter= GC2145MIPI_exposure_lines;
