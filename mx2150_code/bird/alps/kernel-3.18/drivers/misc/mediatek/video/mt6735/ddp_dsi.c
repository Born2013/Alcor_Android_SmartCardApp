...
#ifdef CONFIG_BIRD_ESD_CHECK_HSTX_CKLP_EN    //add by qiuming 20170301
	bool hstx_cklp_en = true; //#false;
#else
        bool hstx_cklp_en = false; //#false;
#endif
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	DSI_T0_INS t0;
#ifdef CONFIG_BIRD_ESD_CHECK_FOR_JD9261
	/*modify by Crystal Shen(BIRD_C) for JD9261 ESD begin*/
	DSI_T0_INS t1;
	struct DSI_RX_DATA_REG read_data0;
        struct DSI_RX_DATA_REG read_data1;
	struct DSI_RX_DATA_REG read_data2;
	...
	/*modify by Crystal Shen(BIRD_C) for JD9261 ESD end*/
#else
	struct DSI_RX_DATA_REG read_data0;
	...
	//		if (hSlot) {
			...
			DISPDBG("lsy--[DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			     read_data0.byte0, read_data0.byte1, read_data0.byte2,
			     read_data0.byte3);
			DISPDBG("lsy--[DSI]enter cmp read_data1 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			     read_data1.byte0, read_data1.byte1, read_data1.byte2,
			     read_data1.byte3);	
			DISPDBG("lsy--[DSI]enter cmp read_data2 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			     read_data2.byte0, read_data2.byte1, read_data2.byte2,
			     read_data2.byte3);	
			DISPDBG("lsy--[DSI]enter cmp read_data3 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			...
			if(packet_type == 0x1A || packet_type == 0x1C)
			   {
			    ...
			    {
			     DISPDBG("DSI read long packet data exceeds buffer size: %d\n", dsi_params->lcm_esd_check_table[i].count);
			...
			{		
				...
				//pr_err("lsy: esd pass-----packet_type == 0x1C-----\n");
				ret = 0; // esd pass
			}
			else
			{
				ret = 1; // esd fail
		//		pr_err("lsy: esd fail----packet_type == 0x1C------\n");
				break;
			}
		}
		else
	    ...
	    if(read_data0.byte1 == dsi_params->lcm_esd_check_table[i].para_list[0])
	    {
	     // clear rx data
	     // DSI_OUTREG32(NULL, &DSI_REG[dsi_i]->DSI_RX_DATA0,0);
	//	 pr_err("lsy: esd pass-----packet_type == 0x11-----\n");
	     ret = 0; // esd pass
	    }
	    else
	    {
	     ret = 1; // esd fail
	//	 pr_err("lsy: esd fail-----packet_type == 0x11-----\n");
	     break;
	    }
		 }
	  }
