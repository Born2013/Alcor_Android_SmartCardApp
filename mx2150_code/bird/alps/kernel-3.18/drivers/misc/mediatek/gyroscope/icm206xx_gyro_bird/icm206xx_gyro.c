...
static int icm206xx_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int res = 0;
        printk("icm206xx_gyro_i2c_probe is start\n");
	...
	if (0 != obj->hw->addr) {
        ...
        client->addr = 0x68;
        printk("icm206xx_gyro addr is %x\n",client->addr);
	obj_i2c_data = obj;
	obj->client = client;
	...
	if (res){
                printk("icm206xx_gyro_init_client is error\n");
...
static int icm206xx_gyro_local_init(struct platform_device *pdev)
{
        printk("icm206xx_gyro_local_init is start\n");
	...
	if (-1 == icm206xx_gyro_init_flag)		
		return -1;
        printk("icm206xx_gyro_local_init is end\n");
	return 0;
}
/*----------------------------------------------------------------------------*/
