...
struct pinctrl *flashlight_pinctrl;
struct pinctrl_state *flashlight_high;
struct pinctrl_state *flashlight_low;
#ifdef CONFIG_DUAL_REAL_FLASHLIGHT
struct pinctrl_state *flashstrobe_high;
struct pinctrl_state *flashstrobe_low;
#endif
...
int flashlight_gpio_init(struct platform_device *pdev)
{
	...
	flashlight_high = pinctrl_lookup_state(flashlight_pinctrl, "flashlight_high");
	if (IS_ERR(flashlight_high)) {
		ret = PTR_ERR(flashlight_high);
		pr_debug("%s : init err, flashlight_high\n", __func__);
	}

	flashlight_low = pinctrl_lookup_state(flashlight_pinctrl, "flashlight_low");
	if (IS_ERR(flashlight_low)) {
		ret = PTR_ERR(flashlight_low);
		pr_debug("%s : init err, flashlight_low\n", __func__);
	}

#ifdef CONFIG_DUAL_REAL_FLASHLIGHT
	flashstrobe_high = pinctrl_lookup_state(flashlight_pinctrl, "flashstrobe_high");
	if (IS_ERR(flashstrobe_high)) {
		ret = PTR_ERR(flashstrobe_high);
		pr_debug("%s : init err, flashstrobe_high\n", __func__);
	}

	flashstrobe_low = pinctrl_lookup_state(flashlight_pinctrl, "flashstrobe_low");
	if (IS_ERR(flashstrobe_low)) {
		ret = PTR_ERR(flashstrobe_low);
		pr_debug("%s : init err, flashstrobe_low\n", __func__);
	}
#endif

#ifdef CONFIG_CUSTOM_KERNEL_SUBFLASHLIGHT
	subflashlight_high = pinctrl_lookup_state(flashlight_pinctrl, "subflash_high");
	if (IS_ERR(subflashlight_high)) {
		ret = PTR_ERR(subflashlight_high);
		pr_debug("%s : init err, subflashlight_high\n", __func__);
	}

	subflashlight_low = pinctrl_lookup_state(flashlight_pinctrl, "suflash_low");
