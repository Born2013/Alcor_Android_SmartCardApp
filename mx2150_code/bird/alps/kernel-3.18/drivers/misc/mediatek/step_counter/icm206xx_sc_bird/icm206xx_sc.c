...
#include "../../accelerometer/icm206xx_acc_bird/icm206xx_register_20608D.h"
#include "../../accelerometer/icm206xx_acc_bird/icm206xx_share.h"
...
static int icm206xx_sc_WriteDMPImage(void)
{
	...
	res = icm206xx_share_i2c_write_memory(ICM206XX_DMP_CODE_START_LOC, 
					...
					&dmpMemory[0]);
	if (res < 0) {
		STEP_C_ERR("delfino memory write error!!\n");
...
static int icm206xx_sc_VerifyDMPImage(void)
{
	...
	for (i = 0; i < ICM206XX_DMP_CODE_SIZE; i++)
	{
		...
		{
			printk("diffrent number is :%d verify data is:%d,mem data is :%d\n", i, dmpVerify[i], dmpMemory[i]);
...
static int icm206xx_sc_EnableStepCounter(u8 en)
{
	...
	if (en == 1) 	databuf[0] = 0xF1;	// enable
	else            databuf[0] = 0xFF;	// disable

	res = icm206xx_share_i2c_write_memory(ICM206XX_KEY_CFG_PED_ENABLE, 1, &databuf[0]);
	printk("### enable step_counter success!\n");
	if (res < 0) {
...
static int icm206xx_sc_enable_step_detect(int en)
{
 ...
 int icm206xx_sc_get_data(uint32_t *value, int *status)
{
...
int read_sensor_data(void)
{
	int stat;
	uint32_t val;
	return icm206xx_sc_get_data(&val,&stat);
}
//EXPORT_SYMBOL(read_sensor_data);
/*----------------------------------------------------------------------------*/
