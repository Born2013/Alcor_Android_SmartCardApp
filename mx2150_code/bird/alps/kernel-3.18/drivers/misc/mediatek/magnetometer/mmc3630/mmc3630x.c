...
static int mmc3630x_self_test(void)
{
	...
	tmp[0] = last_data[0] - vec[0];
	tmp[1] = last_data[1] - vec[1];
	tmp[2] = last_data[2] - vec[2];

	tmp[0] = ABS(tmp[0]);
	tmp[1] = ABS(tmp[1]);
	tmp[2] = ABS(tmp[2]);

	if(tmp[1] > 100 || tmp[2] > 100 || tmp[3] > 100)
	{
		return 1;
	}
