...
static int g_IMX149AF_addr_switch = 0x0;
...
static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
    ...
    char pBuff[2];
		...
		printk("wxs moveAF s4AF_ReadReg g_IMX149AF_addr_switch:%d\n",g_IMX149AF_addr_switch);
		if (g_IMX149AF_addr_switch==0)
		{
			pBuff[0] = 0x03;
	   	...
	   	if (i4RetValue < 0) 
	  		{
//		    	LOG_INF("[IMX149AF] I2C write failed!! g_IMX149AF_addr_switch=%x\n",g_IMX149AF_addr_switch);
				return -1;
				}	
...
static int s4AF_WriteReg(u16 a_u2Data)
{
  int  i4RetValue = 0;
	...
	printk("wxs moveAF s4AF_WriteReg g_IMX149AF_addr_switch:%d\n",g_IMX149AF_addr_switch);
	if (g_IMX149AF_addr_switch==0)
		{
			puSendCmd[0] = 0x03;
			...
			if (i4RetValue < 0) 
			{
//				LOG_INF("[IMX149AF] I2C read failed!! g_IMX149AF_addr_switch=%x\n",g_IMX149AF_addr_switch);
				return -1;
			}
		}
	else
...
static int imx149af_enable(int enable)
{
	...
	//s4IMX149AF_ReadID();
    //LOG_INF("[IMX149AF] g_sr %d, write %d \n", g_sr, a_u2Data);
...
static inline int moveAF(unsigned long a_u4Position)
{
  ...
  unsigned short InitPos;
	...
	if(*g_pAF_Opened == 1)
		...
		iic_ret = imx149af_enable(1);
		printk("wxs moveAF iic_ret:%d\n",iic_ret);
		if(iic_ret == -1)
			g_IMX149AF_addr_switch = 1;  
		else
			g_IMX149AF_addr_switch = 0;
			
		if(g_IMX149AF_addr_switch ==1 )
			IMX149_AF_Open();
		}
