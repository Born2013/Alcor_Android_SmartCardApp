...
static struct pinctrl_state *pins_adc_en_high;
static struct pinctrl_state *pins_adc_en_low;

extern int IMM_GetOneChannelValue(int dwChannel, int dwdata[4], int *rawdata);//20160330
extern int IMM_IsAdcInitReady(void);
#define ADC_Chanel  12
...
int cm36652_setup_adc_en_gpio(struct i2c_client *client)
{
#ifdef CUSTOM_KERNEL_SENSORHUB
	int err = 0;
	...
	int ret;
	//u32 ints[2] = {0, 0};
	APS_LOG("cm36652_setup_adc_en_gpio\n");
...
/* gpio setting */
	...
	pins_adc_en_high = pinctrl_lookup_state(pinctrl, "pin_adc_en_high");
	if (IS_ERR(pins_adc_en_high)) {
		ret = PTR_ERR(pins_adc_en_high);
		APS_ERR("Cannot find alsps pinctrl pin_adc_en_high!\n");
	}

	pins_adc_en_low = pinctrl_lookup_state(pinctrl, "pin_adc_en_low");
	if (IS_ERR(pins_adc_en_low)) {
		ret = PTR_ERR(pins_adc_en_low);
		APS_ERR("Cannot find alsps pinctrl pin_adc_en_low!\n");
	}
...
static DEFINE_MUTEX(crystal_adc_en_gpio_mutex);
void adc_en_gpio_output(int level)
{
	APS_LOG("[tpd]adc_en_gpio_output  level = %d\n",level);
	mutex_lock(&crystal_adc_en_gpio_mutex);
	if (level){
			pinctrl_select_state(pinctrl, pins_adc_en_high);
		  }
		else{
			pinctrl_select_state(pinctrl, pins_adc_en_low);
	} 
	mutex_unlock(&crystal_adc_en_gpio_mutex);
}
...
int cm36652_enable_ps(struct i2c_client *client, int enable)
{
	...
	if (enable == 1) {
		APS_LOG("cm36652_enable_ps enable_ps\n");
			...
			{
				wake_lock(&ps_lock);
				APS_LOG("log3...enable: %d\n",enable);

			}

			printk("crystal als_ps enable ....\n");
			adc_en_gpio_output(1);
			mdelay(10); 
		...
		APS_LOG("cm36652_enable_ps disable_ps\n");
			{
				wake_unlock(&ps_lock);
				APS_LOG("log5...enable: %d\n",enable);
			}
			adc_en_gpio_output(0);
			mdelay(10); 
...
READ_PS_EXIT_ERR:
	...
	int data_temp[4], i, ret_value = 0, ret_temp = 0;
	int times = 1,Channel = ADC_Chanel;
	...
	if (IMM_IsAdcInitReady() == 0) {
		APS_DBG("crystal_ADC is not ready\n");
		return 0;
	}
	...
	i = times;
	while (i--) {
	ret_value = IMM_GetOneChannelValue(Channel, data_temp, &ret_temp);

	APS_DBG("get adc outputhigh  Channel =%d , data_temp[0] =%d , data_temp[1]=%d\n",Channel,data_temp[0],data_temp[1]); 
	}

	ps_value1 = data_temp[0]*100+data_temp[1];
	...
	APS_DBG("get adc  outpulow  Channel =%d , data_temp[0] =%d , data_temp[1]=%d\n",Channel,data_temp[0],data_temp[1]); 
	}

	ps_value2 = data_temp[0]*100+data_temp[1];

	if(ps_value1<ps_value2||ps_value2>80){

		ps_data =10;
	}
	else
	{
		ps_data =ps_value1-ps_value2;
	}	


	if(ps_data<0)
	{
		APS_ERR("cm36652_read_ps_adc fail\n");
		return -1;
	}

	*data = ps_data;

	APS_DBG("ps_data=%d, ps_value1:%d  ps_value2:%d\n", ps_data, ps_value1, ps_value2);
	return 0;
#endif
}
...
static int cm36652_get_ps_value(struct cm36652_priv *obj, u8 ps)
{
	...
	APS_LOG("CM36652_cm36652_get_ps_value\n");
	if (ps > atomic_read(&obj->ps_thd_val_high))
		{
		val = 0;  /*close*/
		APS_LOG("1111111111111111111111111111111111PS=%d,atomic_read(&obj->ps_thd_val_high)=%d\n",ps,atomic_read(&obj->ps_thd_val_high));
		}
	else if (ps < atomic_read(&obj->ps_thd_val_low))
		{
		val = 1;  /*far away*/
		APS_LOG("222222222222222222222222222222222PS=%d,atomic_read(&obj->ps_thd_val_high)=%d\n",ps,atomic_read(&obj->ps_thd_val_high));
		}
...
#endif
	int res = 0;
	res = cm36652_setup_adc_en_gpio(client);
	if (res != 0) {
		APS_ERR("setup adc_en_gpio: %d\n", res);
		return res;
	}
	APS_LOG("cm36652_init_client success!\n");
...
static int cm36652_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = cm36652_init_client(client); //modify crystal shen in function
	if (err)
...
static int  cm36652_local_init(void)
{
	...
	wake_lock_init(&ps_lock, WAKE_LOCK_SUSPEND, "ps wakelock");  // modify crystal 2016-4-20
...
{
	const char *name = "mediatek,scm_0147b";
