...
{
	#if defined(CONFIG_BIRD_NTC_NOSUPPORT)
	return 25;
	#else
...
void check_battery_exist(void)
{
	...
	if (baton_count >= 3) {
		if ((g_platform_boot_mode == META_BOOT) || (g_platform_boot_mode == ADVMETA_BOOT)
			...
			battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
//Modify By Crystal Shen for no battery bootup
	#if 0//!defined(CONFIG_BIRD_NTC_NOBATTERY_BOOT)	
...
int __batt_init_cust_data_from_cust_header(void)
{
	...
	/* charger error check */
#if defined(CONFIG_BAT_LOW_TEMP_PROTECT_ENABLE)
	batt_cust_data.bat_low_temp_protect_enable = 1;
#else				/* #if defined(CONFIG_BAT_LOW_TEMP_PROTECT_ENABLE) */
	batt_cust_data.bat_low_temp_protect_enable = 0;
#endif				/* #if defined(CONFIG_BAT_LOW_TEMP_PROTECT_ENABLE) */
