...
//[BIRD][chengshujiang]20170706 begin
import android.os.SystemProperties;
//[BIRD][chengshujiang]20170706 end
...
final class DefaultPermissionGrantPolicy {
    ...
    //[BIRD][BIRD_ADD_BROWSER_PERMISSIONS][浏览器默认赋予权限][yangheng][20170518] BEGIN
    private static final Set<String> BROWSER_PERMISSIONS = new ArraySet<>();
    static {
        BROWSER_PERMISSIONS.add(Manifest.permission.CAMERA);
        BROWSER_PERMISSIONS.add(Manifest.permission.ACCESS_FINE_LOCATION);
        BROWSER_PERMISSIONS.add(Manifest.permission.RECORD_AUDIO);
        BROWSER_PERMISSIONS.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
        BROWSER_PERMISSIONS.add(Manifest.permission.GET_ACCOUNTS);
    }
    //[BIRD][BIRD_ADD_BROWSER_PERMISSIONS][浏览器默认赋予权限][yangheng][20170518] END
    
    //[BIRD][BIRD_PROMISSION_TURBINNE][Turbinne默认权限打开也能过GTS][yangheng][20161223]BEGIN
    private static final Set<String> READ_PHONE_PERMISSIONS = new ArraySet<>();
    static {
        READ_PHONE_PERMISSIONS.add(Manifest.permission.READ_PHONE_STATE);
    }
    //[BIRD][BIRD_PROMISSION_TURBINNE][Turbinne默认权限打开也能过GTS][yangheng][20161223]END

    /*[BIRD][BIRD_GMAIL_ADD_PRIMISSION][gmail默认打开权限][zhangaman][20180131]begin*/
    public final static boolean BIRD_GMAIL_ADD_PRIMISSION = getValue("ro.bird_gmail_add_per");
    ...
    private static final Set<String> BIRD_GMAIL_PERMISSIONS = new ArraySet<>();
    static {
        BIRD_GMAIL_PERMISSIONS.add(Manifest.permission.READ_CALENDAR);
        BIRD_GMAIL_PERMISSIONS.add(Manifest.permission.WRITE_CALENDAR);
    }
    /*[BIRD][BIRD_GMAIL_ADD_PRIMISSION][gmail默认打开权限][zhangaman][20180131]end*/
    ...
    private void grantDefaultSystemHandlerPermissions(int userId) {
        ...
        synchronized (mService.mPackages) {
            ...
            //[BIRD][TASK-4144][jira-142 Mail帐号 问题][yangheng][20170411] BEGIN
            PackageParser.Package exchangeMobleSerPackage = getSystemPackageLPr("com.android.exchange");
            if (exchangeMobleSerPackage != null
                && doesPackageSupportRuntimePermissions(exchangeMobleSerPackage)) {
                Log.i(TAG, "Granting permissions to com.android.exchange " + userId);
                grantRuntimePermissionsLPw(exchangeMobleSerPackage, READ_PHONE_PERMISSIONS , userId);
                grantRuntimePermissionsLPw(exchangeMobleSerPackage, STORAGE_PERMISSIONS , userId);
            }
            //[BIRD][TASK-4144][jira-142 Mail帐号 问题][yangheng][20170411] END
            /*[BIRD][BIRD_NEUSOFT_ENABLE_PRIMISSION][NEUSOFT应用商店默认打开权限][pangmeizhou 20180404]begin*/
            if (SystemProperties.get("ro.bd_neusoft_permission").equals("1")) {
                PackageParser.Package pac = getPackageLPr("com.neusoft.saca.emm.mam");
                if (pac != null && doesPackageSupportRuntimePermissions(pac)) {
                    grantRuntimePermissionsLPw(pac, LOCATION_PERMISSIONS , userId);
                    grantRuntimePermissionsLPw(pac, CALENDAR_PERMISSIONS , userId);
                    grantRuntimePermissionsLPw(pac, MICROPHONE_PERMISSIONS , userId);
                    grantRuntimePermissionsLPw(pac, PHONE_PERMISSIONS , userId);
                    grantRuntimePermissionsLPw(pac, CAMERA_PERMISSIONS , userId);
                    grantRuntimePermissionsLPw(pac, SENSORS_PERMISSIONS , userId);
                    grantRuntimePermissionsLPw(pac, STORAGE_PERMISSIONS , userId);
                    grantRuntimePermissionsLPw(pac, CONTACTS_PERMISSIONS , userId);
                    grantRuntimePermissionsLPw(pac, SMS_PERMISSIONS , userId);
                }
            }
            /*[BIRD][BIRD_NEUSOFT_ENABLE_PRIMISSION][NEUSOFT应用商店默认打开权限][pangmeizhou 20180404]end*/
            //[BIRD][BIRD_ADD_BROWSER_PERMISSIONS][浏览器默认赋予权限][yangheng][20170518] BEGIN
            if (SystemProperties.get("ro.bd_browser_add_perm").equals("1")){
                Log.i("yangheng", "defaultpermission");
                PackageParser.Package mtkBrowserPackage = getSystemPackageLPr(
                        "com.android.browser");
                if (mtkBrowserPackage != null
                        && doesPackageSupportRuntimePermissions(mtkBrowserPackage)) {
                    grantRuntimePermissionsLPw(mtkBrowserPackage, BROWSER_PERMISSIONS, userId);
                }
            }
            //[BIRD][BIRD_ADD_BROWSER_PERMISSIONS][浏览器默认赋予权限][yangheng][20170518] BEGIN
            
            //[BIRD][BIRD_PROMISSION_TURBINNE][Turbinne默认权限打开也能过GTS][yangheng][20161223] BEGIN
            if (SystemProperties.get("ro.bird_permission_turbinne").equals("1")){
                PackageParser.Package MobleSerPackage = getSystemPackageLPr("com.dti.millicom");
                if (MobleSerPackage != null && doesPackageSupportRuntimePermissions(MobleSerPackage)) {
                    Log.i(TAG, "Granting permissions to com.dti.millicom " + userId);
                    grantRuntimePermissionsLPw(MobleSerPackage, READ_PHONE_PERMISSIONS, userId);
                }
            }
            //[BIRD][BIRD_PROMISSION_TURBINNE][Turbinne默认权限打开也能过GTS][yangheng][20161223] END
            ...
            //Turbo Battery
            if (com.android.server.FeatureOption.BIRD_ADD_TURBO_BATTERY) {
                Intent gplayIntent = new Intent();
                gplayIntent.setClassName("com.qianqi.mylook","com.qianqi.mylook.activity.MainActivity");
                PackageParser.Package gplayPackage = getDefaultSystemHandlerActivityPackageLPr(
                        gplayIntent, userId);
                if (gplayPackage != null
                        && doesPackageSupportRuntimePermissions(gplayPackage)) {
                    grantRuntimePermissionsLPw(gplayPackage, PHONE_PERMISSIONS, userId);
                    grantRuntimePermissionsLPw(gplayPackage, STORAGE_PERMISSIONS, userId);
                    grantRuntimePermissionsLPw(gplayPackage, LOCATION_PERMISSIONS, userId);
                }
            }
            //[BIRD][BIRD_ADD_TURBO_BATTERY][添加省電工具Turbo Battery][yangheng][20170812] END

			/* [BIRD][BIRD_LAUNCHER_LEFT_ADD_ADVERT][桌面左一页添加广告页][dingjiayuan][20180124] begin */    
			if(SystemProperties.get("ro.bird_left_add_advert").equals("1")) {
				PackageParser.Package LauncherPackage = getSystemPackageLPr("com.android.launcher3");
				Log.i("perm_dingjiayuan", "LauncherPackage = "+LauncherPackage);
                if (LauncherPackage != null && doesPackageSupportRuntimePermissions(LauncherPackage)) {
                    Log.i(TAG, "Granting permissions to com.android.launcher3 " + userId);
                    grantRuntimePermissionsLPw(LauncherPackage, READ_PHONE_PERMISSIONS, userId);
					grantRuntimePermissionsLPw(LauncherPackage, STORAGE_PERMISSIONS, userId);
                }
			}
			/* [BIRD][BIRD_LAUNCHER_LEFT_ADD_ADVERT][桌面左一页添加广告页][dingjiayuan][20180124] end */
            /*[BIRD][BIRD_GMAIL_ADD_PRIMISSION][gmail默认打开权限][zhangaman][20180131]begin*/
            PackageParser.Package pac = getSystemPackageLPr("com.google.android.gm");
            if (BIRD_GMAIL_ADD_PRIMISSION && pac != null
                && doesPackageSupportRuntimePermissions(pac)) {
                Log.i(TAG, "Granting permissions to com.google.android.gm " + userId);
                grantRuntimePermissionsLPw(pac, BIRD_GMAIL_PERMISSIONS , userId);
            }
            /*[BIRD][BIRD_GMAIL_ADD_PRIMISSION][gmail默认打开权限][zhangaman][20180131]end*/
			//[BIRD][BIRD_SMART_HARD_DM][集成移动的智能硬件DM][pangmeizhou][20180822]begin
			if("1".equals(SystemProperties.get("ro.bird_smart_hard_dm"))){
				PackageParser.Package LauncherPackage = getSystemPackageLPr("com.android.launcher3");
				Log.i("perm_dingjiayuan", "LauncherPackage = "+LauncherPackage);
	            if (LauncherPackage != null && doesPackageSupportRuntimePermissions(LauncherPackage)) {
	                Log.i(TAG, "Granting permissions to com.android.launcher3 " + userId);
			...
			//[BIRD][BIRD_SMART_HARD_DM][集成移动的智能硬件DM][pangmeizhou][20180822]end
        }
    }
    ...
    private void grantDefaultPermissionsToDefaultSystemSmsAppLPr(
        ...
        if (doesPackageSupportRuntimePermissions(smsPackage)) {
            ...
            //[BIRD][BIRD_KUSAI_MMS][chengshujiang][20170706] begin
            if ("1".equals(SystemProperties.get("ro.bird_kusai_mms"))) {
                grantRuntimePermissionsLPw(smsPackage, STORAGE_PERMISSIONS, userId);
            }
            //[BIRD][BIRD_KUSAI_MMS][chengshujiang][20170706] end
        }
    }
    ...
    public void grantDefaultPermissionsToDefaultSmsAppLPr(String packageName, int userId) {
        ...
        if (smsPackage != null && doesPackageSupportRuntimePermissions(smsPackage)) {
            ...
            //[BIRD][BIRD_KUSAI_MMS][chengshujiang][20170706]
            if ("1".equals(SystemProperties.get("ro.bird_kusai_mms"))) {
