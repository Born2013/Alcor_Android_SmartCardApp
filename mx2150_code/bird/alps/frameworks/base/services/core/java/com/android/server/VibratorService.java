...
public class VibratorService extends IVibratorService.Stub
    ...
    private class Vibration implements IBinder.DeathRecipient {
        ...
        //[BIRD][TASK-4844][工厂测试震动测试 震动(2秒)-->震动停止（1秒）-->震动（2秒）（持续循环），且要求能持续测试，不受屏幕提示、灭屏等影响][yangheng][20170526] BEGIN
        public boolean isVibrationLocked() {
            return "com.bird.mmi_test".equals(mOpPkg);
        }
        //[BIRD][TASK-4844][工厂测试震动测试 震动(2秒)-->震动停止（1秒）-->震动（2秒）（持续循环），且要求能持续测试，不受屏幕提示、灭屏等影响][yangheng][20170526] END
        //[BIRD][BIRD_CB_SAE_FEATURE_CUSTOM][小区广播SAE功能客制化][dingjiayuan][20161125]begin
        public boolean isCellBroadcastPackage() {
            return "com.mediatek.cellbroadcastreceiver".equals(mOpPkg);
        }
        //[BIRD][BIRD_CB_SAE_FEATURE_CUSTOM][小区广播SAE功能客制化][dingjiayuan][20161125]end	
            ...
            if (intent.getAction().equals(Intent.ACTION_SCREEN_OFF)) {
                synchronized (mVibrations) {
                    ...
                    if (mCurrentVibration != null
                        ...
                        //[BIRD][TASK-4844][工厂测试震动测试 震动(2秒)-->震动停止（1秒）-->震动（2秒）（持续循环），且要求能持续测试，不受屏幕提示、灭屏等影响][yangheng][20170526] BEGIN
                        if (mCurrentVibration.isVibrationLocked()
						    || (SystemProperties.get("ro.bd_sae_custom").equals("1") && mCurrentVibration.isCellBroadcastPackage())) {//[BIRD][BIRD_CB_SAE_FEATURE_CUSTOM][小区广播SAE功能客制化][dingjiayuan][20161125]
                            return;
                        }
                        //[BIRD][TASK-4844][工厂测试震动测试 震动(2秒)-->震动停止（1秒）-->震动（2秒）（持续循环），且要求能持续测试，不受屏幕提示、灭屏等影响][yangheng][20170526] END
                        doCancelVibrateLocked();
