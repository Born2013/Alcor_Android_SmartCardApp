...
public class NetworkStatsService extends INetworkStatsService.Stub {
   ...
   public static boolean USE_TRUESTED_TIME = false;
    ...
    private long mPersistThreshold = BIRD_TRAFFIC_MANAGE ? 0L : 2 * MB_IN_BYTES;
    //[BIRD][BIRD_TRAFFIC_MANAGE] caoyuangui add end add 20170904
            ...
            public NetworkStats getSummaryForAllUid(
                ...
                if (includeTags) {
                    ...
                    if (BIRD_TRAFFIC_MANAGE) {
                        SystemProperties.set("sys.bd.get_traffic", "1");
                    }
                    final NetworkStats tagStats = getUidTagComplete()
                            .getSummary(template, start, end, accessLevel);
                    if (BIRD_TRAFFIC_MANAGE) {
                        SystemProperties.set("sys.bd.get_traffic", "0");
                    }
    ...
    public void advisePersistThreshold(long thresholdBytes) {
        ...
        //[BIRD][BIRD_TRAFFIC_MANAGE] caoyuangui add begin add 20170904
        if (BIRD_TRAFFIC_MANAGE) {
            mPersistThreshold = MathUtils.constrain(thresholdBytes, 0L, 2 * MB_IN_BYTES);
        } else {
    ...
    private static class DefaultNetworkStatsSettings implements NetworkStatsSettings {
        ...
        public long getPollInterval() {
            //[BIRD][BIRD_TRAFFIC_MANAGE] caoyuangui add begin add 20170904
            if (BIRD_TRAFFIC_MANAGE) {
                return getGlobalLong(NETSTATS_POLL_INTERVAL, 30 * SECOND_IN_MILLIS);
            } else {
        ...
        public Config getXtConfig() {
          ...
          // caoyuangui test BirdTrafficManager
        @Override
        public Config getUidConfig() {
            //[BIRD][BIRD_TRAFFIC_MANAGE] caoyuangui add begin add 20170904
            if (BIRD_TRAFFIC_MANAGE) { 
                return new Config(getGlobalLong(NETSTATS_UID_BUCKET_DURATION, 30 * SECOND_IN_MILLIS),
        ...
        public Config getUidTagConfig() {
            //[BIRD][BIRD_TRAFFIC_MANAGE] caoyuangui add begin add 20170904
            if (BIRD_TRAFFIC_MANAGE) { 
                return new Config(getGlobalLong(NETSTATS_UID_TAG_BUCKET_DURATION, 30 * SECOND_IN_MILLIS),
