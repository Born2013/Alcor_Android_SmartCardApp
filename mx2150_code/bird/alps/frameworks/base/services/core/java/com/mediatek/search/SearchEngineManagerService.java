...
//[BIRD][BIRD_ADD_SEARCH_ENGINE_AS_DEFAULT][全局增加搜索引擎并设为默认][qianliliang][20160627] BEGIN
import android.os.SystemProperties;
//[BIRD][BIRD_ADD_SEARCH_ENGINE_AS_DEFAULT][全局增加搜索引擎并设为默认][qianliliang][20160627] END
/*[BIRD][BIRD_DEFAULT_SEARCH_ENGINE][默认搜索引擎可配置][zhangaman][20170320]begin*/
import android.text.TextUtils;
/*[BIRD][BIRD_DEFAULT_SEARCH_ENGINE][默认搜索引擎可配置][zhangaman][20170320]end*/
...
public class SearchEngineManagerService extends ISearchEngineManagerService.Stub {
    ...
    private void initSearchEngines() throws IllegalArgumentException {
        ...
        //[BIRD][BIRD_ADD_SEARCH_ENGINE_AS_DEFAULT][全局增加搜索引擎并设为默认][qianliliang][20160627] BEGIN
        if (SystemProperties.get("ro.bd_add_search_engine_def").equals("1")) {
            String[] birdSearchEngines = null;
            if(null == searchEngines || 1 > searchEngines.length) {
                birdSearchEngines = new String[2];
                birdSearchEngines[0] = res.getString(com.android.internal.R.string.zzzzz_bird_add_search_engine_sp);
                birdSearchEngines[1] = res.getString(com.android.internal.R.string.zzzzz_bird_add_search_engine_default);
            } else if(1 == searchEngines.length) {
                birdSearchEngines = new String[2];
                birdSearchEngines[0] = searchEngines[0];
                birdSearchEngines[1] = res.getString(com.android.internal.R.string.zzzzz_bird_add_search_engine_default);
            } else {
                birdSearchEngines = new String[searchEngines.length + 1];
                birdSearchEngines[0] = searchEngines[0];
                //arraycopy(被复制的数组, 从第几个元素开始复制, 要复制到的数组, 从第几个元素开始粘贴, 一共需要复制的元素个数);
                System.arraycopy(searchEngines, 1, birdSearchEngines, 2, searchEngines.length - 1);
                birdSearchEngines[1] = res.getString(com.android.internal.R.string.zzzzz_bird_add_search_engine_default);
            }

            if (null == birdSearchEngines || 1 >= birdSearchEngines.length) {
                // todo: throws an exception in this case.
                throw new IllegalArgumentException("No data found for ");
            }
            searchEngines = birdSearchEngines;

            String sp = searchEngines[0];
            for (int i = 1; i < searchEngines.length; i++) {
                String configInfo = searchEngines[i];
                android.util.Log.d("BIRD_ADD_SEARCH_ENGINE_AS_DEFAULT", "configInfo = " + configInfo);
                SearchEngine info = SearchEngine.parseFrom(configInfo, sp);
                /*[BIRD][BIRD_DEFAULT_SEARCH_ENGINE][默认搜索引擎可配置][zhangaman][20170320]begin*/
                String defaultEngine = SystemProperties.get("bd_default_search_engine","");
                if (!TextUtils.isEmpty(defaultEngine) && info.getFaviconUri().equals(defaultEngine)) {
                    mDefaultSearchEngine = info;
                }
                /*[BIRD][BIRD_DEFAULT_SEARCH_ENGINE][默认搜索引擎可配置][zhangaman][20170320]end*/
