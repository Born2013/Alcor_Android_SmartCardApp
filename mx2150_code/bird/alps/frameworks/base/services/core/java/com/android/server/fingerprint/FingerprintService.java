...
public class FingerprintService extends SystemService implements IBinder.DeathRecipient {
    ...
    private static final long CANCEL_TIMEOUT_LIMIT = android.os.SystemProperties.getInt("ro.bd_fp_cancel_timeout", 3000); // max wait for onCancel() from HAL,in ms
    /*[BIRD][microarray指纹][yangbo][20161220]END */
    /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]BEGIN */
    private BirdFingerprintDatabaseHelper mDbHelper;
    /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]END */
    ...
    public FingerprintService(Context context) {
        ...
        mContext = context;
        /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]BEGIN */
        if (isBirdFingerprintEx()) {
            mDbHelper = new BirdFingerprintDatabaseHelper(context);
        }
        /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]END */
    ...
    protected void handleEnrollResult(long deviceId, int fingerId, int groupId, int remaining) {
        ...
        if (client != null && client.onEnrollResult(fingerId, groupId, remaining)) {
            removeClient(client);
                    ...
                    if (isBirdFingerprintEx()) {
                        if (mDbHelper != null) {
                            mDbHelper.insert(fingerId, getEffectiveUserId(groupId));
                        }
                    }
                    /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]END */
        }
    ...
    private boolean inLockoutMode() {
        ...
        if (isBirdFingerprintEx()) { //不设置限制次数
            return false;
        } else {
    ...
    void startRemove(IBinder token, int fingerId, int groupId, int userId,
        ...
        if (isBirdFingerprintEx()) {
            if (mDbHelper != null) {
                mDbHelper.delete(fingerId, userId);
            }
        }
        /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]END */
    ...
    private final class FingerprintServiceWrapper extends IFingerprintService.Stub {
        ...
        public boolean isAnswerCallEnable(int fingerId, int groupId) {
            ...
            return FingerprintService.this.isAnswerCallEnable(fingerId, effectiveGroupId);
        }
        
        @Override
        public String getMakeCallNumber(int fingerId, int groupId) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.getMakeCallNumber(fingerId, effectiveGroupId);
        }
        
        @Override
        public String getQuickStartPackageName(int fingerId, int groupId) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.getQuickStartPackageName(fingerId, effectiveGroupId);
        }
        
        @Override
        public boolean isTakePhotoEnable(int fingerId, int groupId) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.isTakePhotoEnable(fingerId, effectiveGroupId);
        }
        
        @Override
        public boolean isCallRecordingEnable(int fingerId, int groupId) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.isCallRecordingEnable(fingerId, effectiveGroupId);
        }
        
        @Override
        public boolean setAnswerCallEnable(int fingerId, int groupId, boolean enable) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.setAnswerCallEnable(fingerId, effectiveGroupId, enable);
        }
        
        @Override
        public boolean setMakeCallNumber(int fingerId, int groupId, String number) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.setMakeCallNumber(fingerId, effectiveGroupId, number);
        }
        
        @Override
        public boolean setQuickStartPackageName(int fingerId, int groupId, String packageName) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.setQuickStartPackageName(fingerId, effectiveGroupId, packageName);
        }
        
        @Override
        public boolean setTakePhotoEnable(int fingerId, int groupId, boolean enable) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.setTakePhotoEnable(fingerId, effectiveGroupId, enable);
        }
        
        @Override
        public boolean setCallRecordingEnable(int fingerId, int groupId, boolean enable) {
            final int effectiveGroupId = getEffectiveUserId(groupId);
            return FingerprintService.this.setCallRecordingEnable(fingerId, effectiveGroupId, enable);
        }
        /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]END */
    }
    ...
    /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]BEGIN */
    private boolean isBirdFingerprintEx() {
        return SystemProperties.get("ro.bd_fingerprint_ex", "0").equals("1");
    }
    
    public boolean isAnswerCallEnable(int fingerId, int groupId) {
        return mDbHelper.isAnswerCallEnable(fingerId, groupId);
    }
    
    public String getMakeCallNumber(int fingerId, int groupId) {
        return mDbHelper.getMakeCallNumber(fingerId, groupId);
    }
    
    public String getQuickStartPackageName(int fingerId, int groupId) {
        return mDbHelper.getQuickStartPackageName(fingerId, groupId);
    }
    
    public boolean isTakePhotoEnable(int fingerId, int groupId) {
        return mDbHelper.isTakePhotoEnable(fingerId, groupId);
    }
    
    public boolean isCallRecordingEnable(int fingerId, int groupId) {
        return mDbHelper.isCallRecordingEnable(fingerId, groupId);
    }
    
    public boolean setAnswerCallEnable(int fingerId, int groupId, boolean enable) {
        return mDbHelper.setAnswerCallEnable(fingerId, groupId, enable);
    }
    
    public boolean setMakeCallNumber(int fingerId, int groupId, String number) {
        return mDbHelper.setMakeCallNumber(fingerId, groupId, number);
    }
    
    public boolean setQuickStartPackageName(int fingerId, int groupId, String packageName) {
        return mDbHelper.setQuickStartPackageName(fingerId, groupId, packageName);
    }
    
    public boolean setTakePhotoEnable(int fingerId, int groupId, boolean enable) {
        return mDbHelper.setTakePhotoEnable(fingerId, groupId, enable);
    }
    
    public boolean setCallRecordingEnable(int fingerId, int groupId, boolean enable) {
        return mDbHelper.setCallRecordingEnable(fingerId, groupId, enable);
    }
    /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]END */
