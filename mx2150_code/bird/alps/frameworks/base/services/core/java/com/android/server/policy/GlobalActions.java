...
class GlobalActions implements DialogInterface.OnDismissListener, DialogInterface.OnClickListener  {
    ...
    /*[BIRD][BIRD_AIRPLANE_MODE_MENU][电源键长按弹出框中增加飞行模式][yangbo][20170322]BEGIN */
    public static final boolean BIRD_AIRPLANE_MODE_MENU = getValue("ro.bird_airplane_mode_menu");
    
    private static boolean getValue(String key) {
        return SystemProperties.get(key).equals("1");
    }
    /*[BIRD][BIRD_AIRPLANE_MODE_MENU][电源键长按弹出框中增加飞行模式][yangbo][20170322]END */
    
    /*[BIRD][BIRD_SILENT_MODE_MENU][电源键长按弹出框中增加静音(包括静音、振动、响铃)][yangbo][20170322]BEGIN */
    public static final boolean BIRD_SILENT_MODE_MENU = getValue("ro.bird_silent_mode_menu");
    /*[BIRD][BIRD_SILENT_MODE_MENU][电源键长按弹出框中增加静音(包括静音、振动、响铃)][yangbo][20170322]END */
    ...
    private GlobalActionsDialog createDialog() {
        ...
        for (int i = 0; i < defaultActions.length; i++) {
            ...
            if (GLOBAL_ACTION_KEY_POWER.equals(actionKey)) {
                ...
                //[BIRD][BIRD_REBOOT_MENU][重新启动][yangheng][20161121] BEGIN
                if (SystemProperties.get("ro.bird_reboot_menu").equals("1")) {
	                mItems.add(new RebootAction());
                }
                //[BIRD][BIRD_REBOOT_MENU][重新启动][yangheng][20161121] END
            } else if (GLOBAL_ACTION_KEY_AIRPLANE.equals(actionKey)) {
                mItems.add(mAirplaneModeOn);
            } else if (GLOBAL_ACTION_KEY_BUGREPORT.equals(actionKey)) {
                //[BIRD][bug-15007]songwei 20170919 begin
                //if (Settings.Global.getInt(mContext.getContentResolver(),
                //        Settings.Global.BUGREPORT_IN_POWER_MENU, 0) != 0 && isCurrentUserOwner()) {
                //    mItems.add(new BugReportAction());
                //}
                //[BIRD][bug-15007]songwei 20170919 end
        ...
        /*[BIRD][BIRD_AIRPLANE_MODE_MENU][电源键长按弹出框中增加飞行模式][yangbo][20170322]BEGIN */
        if (BIRD_AIRPLANE_MODE_MENU) {
            if (!addedKeys.contains(GLOBAL_ACTION_KEY_AIRPLANE)) {
                mItems.add(mAirplaneModeOn);
            }
        }
        /*[BIRD][BIRD_AIRPLANE_MODE_MENU][电源键长按弹出框中增加飞行模式][yangbo][20170322]END */
        ...
        /*[BIRD][BIRD_SILENT_MODE_MENU][电源键长按弹出框中增加静音(包括静音、振动、响铃)][yangbo][20170322]BEGIN */
        if (BIRD_SILENT_MODE_MENU) {
            if (!addedKeys.contains(GLOBAL_ACTION_KEY_SILENT)) {
                mItems.add(mSilentModeAction);
            }
        }
        /*[BIRD][BIRD_SILENT_MODE_MENU][电源键长按弹出框中增加静音(包括静音、振动、响铃)][yangbo][20170322]END */
        ...
        /*[BIRD][BIRD_GLOBALACTIONS_ADD_SCREENSHOT][长按电源键弹出的对话框中增加"截屏"][yangbo][20180106]BEGIN */
        if (FeatureOption.BIRD_GLOBALACTIONS_ADD_SCREENSHOT) {
    ...
    private final class RebootAction extends SinglePressAction implements LongPressAction {
        private RebootAction() {
            super(com.android.internal.R.drawable.zzzzz_ic_lock_power_reboot,
                R.string.zzzzz_power_reboot);
        }
    ...
    /*[BIRD][BIRD_GLOBALACTIONS_ADD_SCREENSHOT][长按电源键弹出的对话框中增加"截屏"][yangbo][20180106]BEGIN */
    private static abstract class LongTimeAction implements Action {
        ...
        // prefs
        protected int mNormalIconResId;
        protected int mDoingIconResId;
        protected int mMessageResId;
        protected int mDoingStatusMessageResId;

        public LongTimeAction(int normalIconResId, int doingIconResId, int message, int doingStatusMessageResId, State state) {
            mNormalIconResId = normalIconResId;
            mDoingIconResId = doingIconResId;
            mMessageResId = message;
            mDoingStatusMessageResId = doingStatusMessageResId;
            mState = state;
        }
        ...
        public View create(Context context, View convertView, ViewGroup parent,
            ...
            boolean doing = (mState == State.Doing);
            if (icon != null) {
                icon.setImageDrawable(context.getDrawable((doing ? mDoingIconResId : mNormalIconResId)));
                icon.setEnabled(enabled);
            }
            ...
            if (statusView != null) {
                if (doing) {
                    statusView.setVisibility(View.VISIBLE);
                    statusView.setText(mDoingStatusMessageResId);
                } else {
                    statusView.setVisibility(View.GONE);
                    statusView.setText("");
                }
    ...
    private LongTimeAction mScreenshotAction;
    
    private Action getScreenshotAction() {
        mScreenshotAction = new LongTimeAction(
                com.android.internal.R.drawable.zzzzz_ic_qs_screen_shot,
                com.android.internal.R.drawable.zzzzz_ic_qs_screen_shot,
                R.string.zzzzz_screenshot,
                R.string.zzzzz_screenshot_saving_ticker,
                mScreenshotState) {

            void doInBackground() {
                mScreenshotState = LongTimeAction.State.Doing;
                mHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
            ...
            void onPostExecute() {
                mScreenshotState = LongTimeAction.State.Normal;
                if (mAdapter != null) {
                    mAdapter.notifyDataSetChanged();
                }
            }
    ...
    private LongTimeAction.State mScreenshotState = LongTimeAction.State.Normal;
