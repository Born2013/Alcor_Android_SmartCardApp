...
import com.android.server.policy.IWakeUpService;
/*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] end*/

//[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能][20150722]BEGIN
import android.hardware.Sensor;
import android.hardware.SensorEvent;
import android.hardware.SensorEventListener;
import android.hardware.SensorManager;
//[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能][20150722]END
import com.bird.clam.ClamNative;/*[BIRD_SMALL_VIEW_WINDOW] mandy.wy 20160612 */
/*[BIRD][BIRD_SYSTEM_ENCRYPT][防刷机]xujing 20170628 begin */
import android.os.AsyncTask;
import com.android.server.utils.SystemEncryptUtils;
...
import com.android.server.FeatureOption;
/*[BIRD][BIRD_ZX_SHOUFUBAO_APPS][中兴守护宝][yangbo][20170803]END */

/*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170629]BEGIN */
import com.android.server.BirdContainerManagerService;
/*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170629]END */
//[BIRD][TASK #14664][亮屏界面屏蔽Menu键][chenguangxiang][20170906] BEGIN
import android.app.KeyguardManager;
//[BIRD][TASK #14664][亮屏界面屏蔽Menu键][chenguangxiang][20170906] END
/*[BIRD][一键静音][Task6105] chengci 20171015 begin */
import com.bird.flashlight.BirdFlashlightNative;
/*[BIRD][一键静音][Task6105] chengci 20171015 end */
//[BIRD][BIRD_KEYCODE_F5_CAMERA][点击F5按键进入相机，再点击进行拍照][pangmeizhou][20180508]begin
import android.app.ActivityManager.RunningTaskInfo;
//[BIRD][BIRD_KEYCODE_F5_CAMERA][点击F5按键进入相机，再点击进行拍照][pangmeizhou][20180508]end
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static final int DOUBLE_TAP_HOME_GO_TO_SLEEP = 2;//[BIRD][BIRD_DOUBLE_TAP_HOME_SLEEP][双击home键灭屏功能][qianliliang][20150926]
    ...
    /*[BIRD][BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150609 begin */
    public static final String ACTION_CLAMCLOSE = "bird.intent.action.CLAMCLOSE";
    public static final String ACTION_CLAMOPEN = "bird.intent.action.CLAMOPEN";  
    public static final boolean BIRD_TEST_HALL_TEST = SystemProperties.get("ro.bird_hall_small_window").equals("1");
    /*[BIRD][BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150609 end */
    
    /*[BIRD][BIRD_HALL_BASIC_FEATURES][霍尔器件基本功能：合上熄灭屏幕，打开点亮屏幕][yangbo][20170318]BEGIN */
    private static final boolean BIRD_HALL_BASIC_FEATURES = SystemProperties.get("ro.bd_hall_basic_features").equals("1");
    /*[BIRD][BIRD_HALL_BASIC_FEATURES][霍尔器件基本功能：合上熄灭屏幕，打开点亮屏幕][yangbo][20170318]END */
    ...
    /*[BIRD][BIRD_AIMEI_STUDENT][gaowei][20170617]begin*/
    private long mFirstDownTime;
    private long mSecondDownTime;
    private long mThirdDownTime;
    private int mPowerDownCount = 0;
    private static final int MAX_SPACE_TIME = 420;
    // Time to volume and power must be pressed within this interval of each other.
    private static final long SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS = SystemProperties.get("ro.bd_aimei_student").equals("1") ? MAX_SPACE_TIME+150 : 150;
    // Increase the chord delay when taking a screenshot from the keyguard
    /*[BIRD][BIRD_AIMEI_STUDENT][gaowei][20170617]end*/
    ...
    //[BIRD][BIRD_MENU_VOLUME_UP_BRIGHTNESS_MAX][音量加键加menu键亮度最大][yangheng][20170613] BEGIN
    private long mMaxBrightnessVolumeUpKeyTime;
    private long mMaxBrightnessMenuKeyTime;
    //BIRD][BIRD_MENU_VOLUME_UP_BRIGHTNESS_MAX][音量加键加menu键亮度最大][yangheng][20170613] END
    ...
    /*[BIRD][BIRD_POWER_KEY_LONG_PRESS_REBOOT][长按POWER键10秒重启][zhangaman][20170527]begin*/
    private static final int MSG_REBOOT = 30;
    /*[BIRD][BIRD_POWER_KEY_LONG_PRESS_REBOOT][长按POWER键10秒重启][zhangaman][20170527]end*/

    private static final int MSG_REQUEST_TRANSIENT_BARS_ARG_STATUS = 0;
    private static final int MSG_REQUEST_TRANSIENT_BARS_ARG_NAVIGATION = 1;
    
    /*[BIRD][BIRD_SYSTEM_ENCRYPT][防刷机]xujing 20170628 begin */
    private static boolean hasSystemEncryptRunned = false;
    /*[BIRD][BIRD_SYSTEM_ENCRYPT][防刷机]xujing 20170628 end */

    /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] begin*/
    public static PowerManager.WakeLock mLetterWakeLock;
    private LockPatternUtils mLockPatternUtils;
    private IWakeUpService mService = null;
    /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] end*/
    
    //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能][dengyang][20160526]BEGIN
    private class BirdSensorEventListener implements SensorEventListener {
		
		@Override
		public void onSensorChanged(SensorEvent event) {
			if (event.values != null && event.values.length > 0) { 
                Log.d(TAG, "event.values[0]:" + event.values[0]);
                Log.d(TAG, "getMaximumRange:" + mProximitySensor.getMaximumRange());
                if (event.values[0] < mProximitySensor.getMaximumRange()) {
                    mProximitySensorCovered = true;
                    Log.d(TAG, "setmProximitySensorCovered:" + mProximitySensorCovered);
                } else {
	...
	private BirdSensorEventListener mSensorListener;
    private SensorManager mSensorManager;
    private Sensor mProximitySensor;
    private boolean mProximitySensorCovered = false;
    //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能][dengyang][20160526]END
    //private static boolean isMpEnabled = false; //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能 优化][20150804]add
    private static boolean mProximitySensorRegisted = false;//[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能 优化][20150804]add
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能 优化][20150804]BEGIN
            if(BIRD_SMART_WAKE_UP_SCREEN && BIRD_SMART_WAKE_MISOPERATION_PREVENT) {
	            resolver.registerContentObserver(Settings.System.getUriFor(
	                    Settings.System.BIRD_SMART_WAKE_CTP_MP), false, this,
	                    UserHandle.USER_ALL);
	            resolver.registerContentObserver(Settings.System.getUriFor(
	                    Settings.System.BIRD_SMART_WAKE_CTP), false, this,
	                    UserHandle.USER_ALL);
            }
            //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能 优化][20150804]END
    ...
    private void interceptPowerKeyDown(KeyEvent event, boolean interactive) {
        ...
        if (!SystemProperties.get("ro.bd_aimei_student").equals("1")) {
    ...
    private boolean mMaxBrightnessWork = false;
    
    private void interceptMaxBrightness() {
        if (Settings.System.getInt(mContext.getContentResolver(), Settings.System.BIRD_SUPER_SAVING_STATUS, 0) != 1) {//[BIRD][Bug 12150][音量加键加menu键亮度最大 超级省电下屏蔽本功能][yangheng][20170616]
            final long now = SystemClock.uptimeMillis();
            if (now <= mMaxBrightnessVolumeUpKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS
                    && now <= mMaxBrightnessMenuKeyTime
                            + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS) {
                mMaxBrightnessWork = true;
    ...
    private class ScreenshotRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (FeatureOption.BIRD_ENCRYPT_SPACE) {
                BirdContainerManagerService cms = (BirdContainerManagerService)ServiceManager.getService(Context.BIRD_CONTAINER_SERVICE);
                if (cms != null && cms.isCurrentContainerUser()) {
                    return;
                }
            }
            /*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170629]END */
    ...
    void showGlobalActionsInternal() {
        /*[BIRD][BIRD_SMALL_VIEW_WINDOW][皮套功能][皮套合盖时按power键不弹出关机弹出框][zhangaman][20170601]begin*/
        if (SystemProperties.get("ro.bd_hall_leather").equals("1") && readHallStatus()) {
            Log.d(TAG,"SMALL view show ,don't show global action dialog");
            return;
        }
        /*[BIRD][BIRD_SMALL_VIEW_WINDOW][皮套功能][皮套合盖时按power键不弹出关机弹出框][zhangaman][20170601]end*/
        sendCloseSystemWindows(SYSTEM_DIALOG_REASON_GLOBAL_ACTIONS);
    ...
    private void handleDoubleTapOnHome() {
        if (mDoubleTapOnHomeBehavior == DOUBLE_TAP_HOME_RECENT_SYSTEM_UI) {
            mHomeConsumed = true;
            toggleRecentApps();
        //[BIRD][BIRD_DOUBLE_TAP_HOME_SLEEP][双击home键灭屏功能][qianliliang][20150926] BEGIN
        } else if (SystemProperties.get("ro.bird_double_tap_home_sleep").equals("1") && isSupportSmartWakeUp() && 
				mDoubleTapOnHomeBehavior == DOUBLE_TAP_HOME_GO_TO_SLEEP && isSystemSettingsSupportHomeKey()) {
			try {
				mPowerManager.goToSleep(SystemClock.uptimeMillis(), PowerManager.GO_TO_SLEEP_REASON_POWER_BUTTON, 0);
			} catch (Exception e) {
				Log.i("TAG","BIRD, handleDoubleTapOnHome(), goToSleepBird() is Error !"+e);
			}
		}
		//[BIRD][BIRD_DOUBLE_TAP_HOME_SLEEP][双击home键灭屏功能][qianliliang][20150926] END
    }
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        if(BIRD_SMART_WAKE_UP_SCREEN && BIRD_SMART_WAKE_MISOPERATION_PREVENT) {
            ...
            mSensorListener = new BirdSensorEventListener();
        }
        //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能 优化][20150804]END
        ...
        /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526]begin*/
        mLetterWakeLock = mPowerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK , "PhoneWindowManager.mLetterWakeLock");
        /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526]end*/
        ...
        if(BIRD_SMART_WAKE_UP_SCREEN){//(FeatureOption.BIRD_SMART_WAKE_UP_SCREEN){
            IntentFilter samrtFilter = new IntentFilter("android.wakeup.letter.wakelockrelease");
            context.registerReceiver(smartReceiver, samrtFilter);
        }
        /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] end*/
        
        /*[BIRD][BIRD_SYSTEM_ENCRYPT][防刷机]xujing 20170628 begin */
        //if (SystemProperties.get("ro.bird_system_encrypt").equals("1")) {
        //    IntentFilter systemEncryptReceiverFilter = new IntentFilter("android.intent.action.BOOT_COMPLETED");
        //    context.registerReceiver(systemEncryptReceiver, systemEncryptReceiverFilter);
        //}
        /*[BIRD][BIRD_SYSTEM_ENCRYPT][防刷机]xujing 20170628 end */
    ...
    private void readConfigurationDependentBehaviors() {
        ...
        //[BIRD][BIRD_DOUBLE_TAP_HOME_SLEEP][双击home键灭屏功能][qianliliang][20150926] BEGIN
		if (SystemProperties.get("ro.bird_double_tap_home_sleep").equals("1") && isSupportSmartWakeUp() && isSystemSettingsSupportHomeKey()) {
			mDoubleTapOnHomeBehavior = DOUBLE_TAP_HOME_GO_TO_SLEEP;
		}
		//[BIRD][BIRD_DOUBLE_TAP_HOME_SLEEP][双击home键灭屏功能][qianliliang][20150926] END
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
           ...
           if(BIRD_SMART_WAKE_UP_SCREEN && BIRD_SMART_WAKE_MISOPERATION_PREVENT) {
                ...
                boolean mpEnabled =  Settings.System.getIntForUser(mContext.getContentResolver(), 
                                            Settings.System.BIRD_SMART_WAKE_CTP_MP, 0, 
                                            UserHandle.USER_CURRENT) == 1 ? true : false;
                boolean smartUpEnabled =  Settings.System.getIntForUser(mContext.getContentResolver(), 
                                            Settings.System.BIRD_SMART_WAKE_CTP, 0, 
                                            UserHandle.USER_CURRENT) == 1 ? true : false;

                if(mSystemReady) {
                    Log.i("pangmeizhou","aaaa mpEnabled = "+mpEnabled+" smartUpEnabled = "+smartUpEnabled+" mProximitySensorRegisted = "+mProximitySensorRegisted);
                    if(mpEnabled && smartUpEnabled) {
                        if (mSensorManager != null && mProximitySensor == null && !mProximitySensorRegisted) {
                            if ((mProximitySensor = mSensorManager.getDefaultSensor(Sensor.TYPE_PROXIMITY)) != null) {
                                mSensorManager.registerListener(mSensorListener, mProximitySensor, SensorManager.SENSOR_DELAY_NORMAL);
                                mProximitySensorRegisted = true;
                                Log.i("pangmeizhou","aaaa registerListener mProximitySensor");
                            }
                        }
                    } else {
                        if (mProximitySensorRegisted == true && mSensorManager != null && mProximitySensor != null && mSensorListener != null) {
                            //if ((mProximitySensor = mSensorManager.getDefaultSensor(Sensor.TYPE_PROXIMITY)) != null) {
                            mSensorManager.unregisterListener(mSensorListener);
                            //}
                            mProximitySensor = null;
                            mProximitySensorRegisted = false;
                            Log.i("pangmeizhou","aaaa unregisterListener mProximitySensor");
                        }
                    }
                }

                /*if(!mpEnabled && mSystemReady && isMpEnabled != mpEnabled) {
                    isMpEnabled = mpEnabled;
                    if (mSensorManager != null && mProximitySensor != null && mSensorListener != null) {
                        //if ((mProximitySensor = mSensorManager.getDefaultSensor(Sensor.TYPE_PROXIMITY)) != null) {
                        mSensorManager.unregisterListener(mSensorListener);
                        //}
                        mProximitySensor = null;
                    ...
                    //android.util.Log.d("aaa", " mpEnabled= "+mpEnabled +" mSensorListener= "+mSensorListener + " mProximitySensor= "+mProximitySensor);
            
                } else if(mpEnabled && mSystemReady && isMpEnabled != mpEnabled) {
                    isMpEnabled = mpEnabled;
                    ...
                    //android.util.Log.d("aaa", " registerListener  mpEnabled= "+mpEnabled+ " mSensorListener= "+mSensorListener+ " mProximitySensor= "+mProximitySensor);
                }*/
           }
           //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能 优化][20150804]END
        }
    ...
    //[BIRD][BIRD_MENU_CUSTOMER][menu键客制化需求][dingjiayuan][20160612] BEGIN
    private boolean mMenuLongPressed = false;
    //[BIRD][BIRD_MENU_CUSTOMER][menu键客制化需求][dingjiayuan][20160612] END
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        //[BIRD][BIRD_LEFT_KEY_REBOOT][手机左下角键短按重启][chenguangxiang][20171213] BEGIN
        if (keyCode == KeyEvent.KEYCODE_F6 && !down && SystemProperties.get("ro.bird_left_key_reboot").equals("1")) {
            // caoyuangui add 20170129 begin
            if (win != null && win.getAttrs() != null && "com.bird.mmi_test/com.bird.mmi_test.MMITestKeyBoard".equals(win.getAttrs().getTitle())) {
                return 0; 
            }
            // caoyuangui add 20170129 end
        ...
        //[BIRD][BIRD_LEFT_KEY_REBOOT][手机左下角键短按重启][chenguangxiang][20171213] END

        //[BIRD][BIRD_MENU_VOLUME_UP_BRIGHTNESS_MAX][音量加键加menu键亮度最大][yangheng][20170613] BEGIN
        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP
                && mMaxBrightnessWork
                && SystemProperties.get("ro.bd_menu_up_brightness_max").equals("1")) {
            if (!down) {
                mMaxBrightnessWork = false;
                Log.i("yangheng", "not down");
            }
            return -1;
        }
        //[BIRD][BIRD_MENU_VOLUME_UP_BRIGHTNESS_MAX][音量加键加menu键亮度最大][yangheng][20170613] END
        ...
        /*[工厂测试]huangzhangbin 20180412 begin*/
        if (keyCode == KeyEvent.KEYCODE_F5 || keyCode == KeyEvent.KEYCODE_F6 || keyCode == KeyEvent.KEYCODE_F7) {
            if(win != null && win.getAttrs() != null 
                    && (win.getOwningPackage().equals("com.bird.mmi_test") || win.getOwningPackage().equals("com.bird.smt_sw"))) {
                return 0;// return 0就会丢给app处理了。
            }
            //[PPT键进对讲机]xujing 20180614 begin
            if (SystemProperties.get("ro.bird_intent_ppt_lunch").equals("1") && keyCode == KeyEvent.KEYCODE_F7 && down) {
                ActivityManager manager = (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
                RunningTaskInfo info = manager.getRunningTasks(1).get(0);
                String packageName = info.topActivity.getPackageName(); 
                Log.i("PhoneWindowManager","keyCode == KeyEvent.KEYCODE_F5 packageName = "+packageName);
                if(!"com.zed3.sipua".equals(packageName)) {
                    // launch the camera activity
                    Intent cameraIntent = new Intent();
                    final ComponentName cameraComponent = new ComponentName("com.zed3.sipua",
                    "com.zed3.sipua.ui.splash.SplashActivity");
                    cameraIntent.setComponent(cameraComponent);
                    cameraIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					//[BIRD][BIRD_PTT_SOS_SUPPORT][如果没有预置集群通则PTT键拨打112][pangmeizhou][20180706]begin
                    Log.i("PhoneWindowManager","keyCode == KeyEvent.KEYCODE_F5 packageName = "+packageName);
					if(SystemProperties.get("ro.bird_ptt_sos").equals("1")){
						Intent intent = new Intent(Intent.ACTION_CALL,Uri.parse("tel:112"));  
						mContext.startActivity(intent);
                        return -1; 
					} else {
                        try {
                            startActivityAsUser(cameraIntent, UserHandle.CURRENT);
                            return -1;
                        } catch (ActivityNotFoundException e) {
                            Slog.w(TAG, "No activity to handle assist long press action.", e);
                        }
                    }
					//[BIRD][BIRD_PTT_SOS_SUPPORT][如果没有预置集群通则PTT键拨打112][pangmeizhou][20180706]end
                }
            }
            //[PPT键进对讲机]xujing 20180614 end
        }
        /*[工厂测试]huangzhangbin 20180412 end*/
        // First we always handle the home key here, so applications
        // can never break it, although if keyguard is on, we do let
        // it handle it, because that gives us the correct 5 second
        // timeout.
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            /*[BIRD][BIRD_POWER_AND_HOME_SCREEN_SHOT_SUPPORT][POWER键＋HOME键截屏][zhangaman][20170309]begin*/
            if (isPowerAndHomeScreenshotSupport()) {
                if (mScreenshotChordVolumeDownKeyConsumed) {
                    return 0;
                }
            }
            /*[BIRD][BIRD_POWER_AND_HOME_SCREEN_SHOT_SUPPORT][POWER键＋HOME键截屏][zhangaman][20170309]end*/
            //[BIRD][BIRD_FACTORY_ASSEMBLE_TEST_COMMAND][*#15963#*指令功能][dengyang][]BEGIN
			if(win != null && win.getAttrs() != null&&win.getOwningPackage().equals("com.bird.mmi_test") || win.getOwningPackage().equals("com.bird.smt_sw")){//BIRD_SMT_SW][贴片软件 屏蔽home按键][dengyang][20160128]
                return 0;// return 0就会丢给app处理了。
            }
            //[BIRD][BIRD_FACTORY_ASSEMBLE_TEST_COMMAND][*#15963#*指令功能][dengyang][]END
            //[BIRD][BIRD_POWER_MANAGER_V2][省电管理V2.0][zhangaman][20160112]begin
            if (SystemProperties.get("ro.bird_power_manager_v2").equals("1") 
                && win != null && win.getAttrs() != null 
                && win.getOwningPackage().equals("com.bird.powermanager")
                && isSuperSavingMode()) {
                return 0;
            }
            //[BIRD][BIRD_POWER_MANAGER_V2][省电管理V2.0][zhangaman][20160112]end
            
            /*[BIRD][BIRD_ZX_SHOUFUBAO_APPS][中兴守护宝][yangbo][20170803]BEGIN */
            if (FeatureOption.BIRD_ZX_SHOUFUBAO_APPS) {
                if (win != null && "com.njzx.lock".equals(win.getOwningPackage())) {
                    return 0;
                }
            }
            /*[BIRD][BIRD_ZX_SHOUFUBAO_APPS][中兴守护宝][yangbo][20170803]END */
            ...
            if (attrs != null) {
                ...
                if (type == WindowManager.LayoutParams.TYPE_KEYGUARD_SCRIM
                    ...
                    //[BIRD][BIRD_DOUBLE_TAP_HOME_SLEEP][双击home键灭屏功能][锁屏界面可以灭屏][qianliliang][20150926] BEGIN
				    if (SystemProperties.get("ro.bird_double_tap_home_sleep").equals("1") && repeatCount == 0 
                        && isSupportSmartWakeUp() && isSystemSettingsSupportHomeKey()) {
					    mHomePressed = true;
					    if (mHomeDoubleTapPending) {
					        mHomeDoubleTapPending = false;
					...
					//[BIRD][BIRD_DOUBLE_TAP_HOME_SLEEP][双击home键灭屏功能][锁屏界面可以灭屏][qianliliang][20150926] END
                    return 0;
                }
            ...
            //[BIRD][BIRD_MENU_VOLUME_UP_BRIGHTNESS_MAX][音量加键加menu键亮度最大][yangheng][20170613] BEGIN
            if (down && SystemProperties.get("ro.bd_menu_up_brightness_max").equals("1")) {
                mMaxBrightnessMenuKeyTime = SystemClock.uptimeMillis();
                interceptMaxBrightness();
            }
            if (mMaxBrightnessWork) {
                return 0;
            }
            //[BIRD][BIRD_MENU_VOLUME_UP_BRIGHTNESS_MAX][音量加键加menu键亮度最大][yangheng][20170613] END
            ...
            Log.i("chenguangxiang","ifOpen:"+ifOpen+"   flag:"+flag);
            //[BIRD][TASK #14664][亮屏界面屏蔽Menu键][chenguangxiang][20170906] END
            
            //[BIRD][BIRD_MENU_CUSTOMER][menu键客制化需求][dingjiayuan][20160612] BEGIN
            if(SystemProperties.get("ro.bird_menu_customer").equals("1")) {
                Log.i("xiaowo","mMenuLongPressed = "+mMenuLongPressed+" down = "+down);
                if(!down){
                    if(!mMenuLongPressed) {
                        //[BIRD][BIRD_FACTORY_ASSEMBLE_TEST_COMMAND][工厂装配测试][menu键处理][qianliliang][20160323] BEGIN
                        if (win != null && win.getOwningPackage() != null &&
                            (win.getOwningPackage().equals("com.bird.smt_sw") || win.getOwningPackage().equals("com.bird.mmi_test"))){
                            mMenuLongPressed = false;
                            return 0;
                        }
                        //[BIRD][BIRD_FACTORY_ASSEMBLE_TEST_COMMAND][工厂装配测试][menu键处理][qianliliang][20160323] END
                        //[BIRD][BIRD_POWER_MANAGER_V2][省电管理V2.0][zhangaman][20160112]begin
                        if (SystemProperties.get("ro.bird_power_manager_v2").equals("1") 
                            && win != null && win.getAttrs() != null 
                            && win.getOwningPackage().equals("com.bird.powermanager")
                        ...
                        //[BIRD][TASK #14664][亮屏界面屏蔽Menu键][chenguangxiang][20170906] BEGIN
                        if (flag == true) {
                            Log.i("chenguangxiang","   flag1:"+flag);
                            return 0;
                        }
                        //[BIRD][TASK #14664][亮屏界面屏蔽Menu键][chenguangxiang][20170906] END
                        toggleRecentApps();
            ...
            //[BIRD][BIRD_SHORT_MENU_RECENT_APP][短按menu进入recent app][hongzhihao][20160725]begin
            //[BIRD][TASK #14664][亮屏界面屏蔽Menu键][chenguangxiang][20170906]
            if(SystemProperties.get("ro.bird_short_menu_recent_app").equals("1")){
                if (win != null && win.getOwningPackage() != null && (win.getOwningPackage().equals("com.bird.smt_sw") || win.getOwningPackage().equals("com.bird.mmi_test")) || (flag == true)){
                    return 0;
                }
			...
			//[BIRD][BIRD_KEYCODE_F5_CAMERA][点击F5按键进入相机，再点击进行拍照][pangmeizhou][20180508]begin
			if(SystemProperties.get("ro.bird_keyf5_camera").equals("1")){
				if(!down) {
					Log.i("PhoneWindowManager","keyCode == KeyEvent.KEYCODE_F5...");
					ActivityManager manager = (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
					RunningTaskInfo info = manager.getRunningTasks(1).get(0);
					...
					if(!"com.mediatek.camera".equals(packageName)) {
						// launch the camera activity
						Intent cameraIntent = new Intent();
						final ComponentName cameraComponent = new ComponentName("com.mediatek.camera",
			...
			//[BIRD][BIRD_KEYCODE_F6_GPS][点击F6按键打开GPS并进入百度地图][pangmeizhou][20180508]begin
			if(SystemProperties.get("ro.bird_keyf6_gps").equals("1")){
				if(!down) {
					Log.i("PhoneWindowManager","keyCode == KeyEvent.KEYCODE_F6...");
					Log.i("PhoneWindowManager","keyCode == KeyEvent.KEYCODE_F6 before isOpen = "+isOpen());
					if(!isOpen()){
						openGPS(true);
					}
					Log.i("PhoneWindowManager","keyCode == KeyEvent.KEYCODE_F6 after isOpen = "+isOpen());
					Intent mapIntent = new Intent();
					final ComponentName mapComponent = new ComponentName("com.baidu.BaiduMap",
						    "com.baidu.baidumaps.WelcomeScreen");
					mapIntent.setComponent(mapComponent);
					mapIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					try {
						startActivityAsUser(mapIntent, UserHandle.CURRENT);
					} catch (ActivityNotFoundException e) {
						Slog.w(TAG, "No activity to handle assist long press action.", e);
					}
					return -1;					
			...
			//[BIRD][BIRD_KEYCODE_F6_GPS][点击F6按键打开GPS并进入百度地图][pangmeizhou][20180508]end
		}
    ...
    private void launchAssistAction(String hint, int deviceId) {
        ...
        //[BIRD][BIRD_LONG_PRESS_HOME_GOOGLE_NOW][长按home键弹出google_now][pangmeizhou][20161128]begin
        if ((mContext.getResources().getConfiguration().uiMode
                & Configuration.UI_MODE_TYPE_MASK) == Configuration.UI_MODE_TYPE_TELEVISION
                || SystemProperties.get("ro.bd_long_home_google_now").equals("1")) {
        //[BIRD][BIRD_LONG_PRESS_HOME_GOOGLE_NOW][长按home键弹出google_now][pangmeizhou][20161128]end
    ...
    public void applyPostLayoutPolicyLw(WindowState win, WindowManager.LayoutParams attrs,
        ...
        if (mTopFullscreenOpaqueWindowState == null &&
            ...
            /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] begin*/
             if(attrs.getTitle().toString().equals("LockScreenWate")){
               if (DEBUG_LAYOUT) Log.v(TAG, "applyPostLayoutPolicyLw LockScreenWate");
               appWindow = true;
             }
            /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] end*/
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        Log.d("Bird_oneStep","enter PhoneWindowManager interceptKeyBeforeQueueing");
        if (!mSystemBooted) {
        ...
        /// @}
        /*[BIRD_SMALL_VIEW_WINDOW][合上皮套，重按手机下方，home键会起作用] mandy.wu 20150625 begin */
        if(SystemProperties.get("ro.bd_hall_leather").equals("1")) {
            if (readHallStatus() && ((KeyEvent.KEYCODE_MENU == event.getKeyCode())
            ||(KeyEvent.KEYCODE_HOME ==  event.getKeyCode())
            ||(KeyEvent.KEYCODE_BACK ==  event.getKeyCode()))) {
                return 0;
            }
        } 
        /*[BIRD_SMALL_VIEW_WINDOW][合上皮套，重按手机下方，home键会起作用] mandy.wu 20150625 end */
        /// M: IPO migration @{
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN && !isPowerAndHomeScreenshotSupport()) {/*[BIRD][BIRD_POWER_AND_HOME_SCREEN_SHOT_SUPPORT][POWER键＋HOME键截屏][zhangaman][20170309]*/
                    if (down) {
                ...
                if (keyCode == KeyEvent.KEYCODE_VOLUME_UP 
                        && SystemProperties.get("ro.bd_menu_up_brightness_max").equals("1")) {
                    if (down) {
                        mMaxBrightnessVolumeUpKeyTime = event.getDownTime();
                        mMaxBrightnessWork = false;
                        interceptMaxBrightness();
                    }
                //[BIRD][BIRD_MENU_VOLUME_UP_BRIGHTNESS_MAX][音量加键加menu键亮度最大][yangheng][20170613] END
            ...
            /*[BIRD][BIRD_INTENT_PPT][gaowei][20171019]begin*/
            case KeyEvent.KEYCODE_F5: {
                if (down) {
                    Log.i("testppt","KEYCODE_F5  down");
                    if(SystemProperties.get("ro.bird_intent_ppt").equals("1")) {
                        //[task-6961]songwei 20180410 begin
                        String downkey = SystemProperties.get("ro.bird_ppt_down", "android.intent.action.PTT.down");
                        Intent intent = new Intent(downkey);
                        //[task-6961]songwei 20180410 end
                        mContext.sendBroadcast(intent);
                        Log.i("testppt","down");
                    }
                } else {
                    Log.i("testppt","KEYCODE_F5  up");
                    if(SystemProperties.get("ro.bird_intent_ppt").equals("1")) {
                        //[task-6961]songwei 20180410 begin
                        String upkey = SystemProperties.get("ro.bird_ppt_up", "android.intent.action.PTT.up");
                        Intent intent = new Intent(upkey);
                        //[task-6961]songwei 20180410 end
                        mContext.sendBroadcast(intent);
                        Log.i("testppt","up");
                    }
            ...
            /*[BIRD][BIRD_INTENT_PPT][gaowei][20171019]end*/
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                /*[BIRD][BIRD_AIMEI_STUDENT][gaowei][20170617]begin*/
                if (SystemProperties.get("ro.bd_aimei_student").equals("1")) {
                    aiMeiPowerClick(down, event, interactive, canceled);
                } else {
                    if (down) {
                        interceptPowerKeyDown(event, interactive);
                        /*[BIRD][BIRD_POWER_KEY_LONG_PRESS_REBOOT][长按POWER键10秒重启][zhangaman][20170527]begin*/
                        if (SystemProperties.get("ro.bird_powerkey_reboot").equals("1")) {
                            mHandler.removeMessages(MSG_REBOOT);
                            mHandler.sendEmptyMessageDelayed(MSG_REBOOT, 10000);
                        }
                        /*[BIRD][BIRD_POWER_KEY_LONG_PRESS_REBOOT][长按POWER键10秒重启][zhangaman][20170527]end*/
                    } else {
            ...
            //[BIRD][BIRD_HALL_HOLSTER_SUPPORT][霍尔皮套功能][luqin][20150817] BEGIN
            case KeyEvent.KEYCODE_SHIFT_LEFT: {

                if (down && BIRD_TEST_HALL_TEST) {
                    if (readHallStatus()) {
                        Log.i(TAG+"wrf", "---close---");
                        mContext.sendBroadcast(new Intent(ACTION_CLAMCLOSE));
                        mContext.sendBroadcast(new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS));
                    } else {
                        Log.i(TAG+"wrf", "---open---");
                        mContext.sendBroadcast(new Intent(ACTION_CLAMOPEN));
                        mContext.sendBroadcast(new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS));
                    }
                    /*[BIRD] [BIRD_LEATHER_SUPPORT] xujing 20150921 begin*/
                    if (SystemProperties.get("ro.bird_hall_small_window").equals("1")) {
                        Intent leatherIntent = new Intent("android.intent.action.leatherService").setPackage("com.bird.leather");
                        leatherIntent.putExtra("keycode", keyCode);
                        mContext.startService(leatherIntent);
                    }
                    /*[BIRD] [BIRD_LEATHER_SUPPORT] xujing 20150921 end*/
                }

                /*[BIRD][BIRD_HALL_BASIC_FEATURES][霍尔器件基本功能：合上熄灭屏幕，打开点亮屏幕][yangbo][20170318]BEGIN */
                if (BIRD_HALL_BASIC_FEATURES) {
                    //sleep
                    result &= ~ACTION_PASS_TO_USER;
                    if (!mPowerManager.isInteractive()) {
                        useHapticFeedback = false; // suppress feedback if already non-interactive
				...
				/*[BIRD][BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150612 begin */
				if(SystemProperties.get("ro.bd_hall_leather").equals("1")) {
					if(down){
						Log.d("wuxuelei","KEYCODE_SHIFT_LEFT");
						mContext.sendBroadcast(new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS));
						mKeyguardDelegate.showSmallViewWindow(true);
						mContext.sendBroadcast(new Intent(ACTION_CLAMCLOSE));
						Log.d("wuxuelei","sendBroadcast");
					}
					return 0;/*拨号盘输入号码后合上皮套，打开皮套时拨号盘下方多出一栏无用栏:mandy.wu 20150627*/
				}
				/*[BIRD][BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150612 end */
            }
                break;
            case KeyEvent.KEYCODE_SHIFT_RIGHT: {
                ...
                if (BIRD_HALL_BASIC_FEATURES) {
                    if (mPowerManager.isInteractive()) {
                        mPowerManager.userActivity(SystemClock.uptimeMillis(), false); //当屏幕半亮时，点亮它
                    }
                    //wakeup
                    result &= ~ACTION_PASS_TO_USER;
                    isWakeKey = true;
				...
				if(SystemProperties.get("ro.bd_hall_leather").equals("1")) {
					if(down){
						Log.d("wuxuelei","KEYCODE_SHIFT_RIGHT");
						mContext.sendBroadcast(new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS));
						mKeyguardDelegate.showSmallViewWindow(false);
						mContext.sendBroadcast(new Intent(ACTION_CLAMOPEN));
						Log.d("wuxuelei","sendBroadcast");
					}
					return 0;/*拨号盘输入号码后合上皮套，打开皮套时拨号盘下方多出一栏无用栏:mandy.wu 20150627*/
            ...
            //[BIRD][BIRD_HALL_HOLSTER_SUPPORT][霍尔皮套功能][luqin][20150817] END
            ...
            case KeyEvent.KEYCODE_F7: {
                ...
                switch (keyCode) {
                    case KeyEvent.KEYCODE_F6:
                        Log.d("Bird_oneStep", "enter PhoneWindowManager KEYCODE_F6");
                        Settings.System.putInt(mContext.getContentResolver(), "bird_onestep_volume_mute", 0); 
                        direction = AudioManager.ADJUST_UNMUTE;
                        break;
                    case KeyEvent.KEYCODE_F7:
                        Log.d("Bird_oneStep", "enter PhoneWindowManager KEYCODE_F7");
                        Settings.System.putInt(mContext.getContentResolver(), "bird_onestep_volume_mute", 1); 
                ...
                if (getAudioManager().isMusicActive()) {
                    Log.d("Bird_oneStep", "enter PhoneWindowManager 222");
                    MediaSessionLegacyHelper.getHelper(mContext).sendAdjustVolumeBy(
                    ...
                    if (telecomManager != null && telecomManager.isInCall()) {
                        Log.d("Bird_oneStep", "enter PhoneWindowManager 444");
                        flags = AudioManager.FLAG_VIBRATE | AudioManager.FLAG_FROM_KEY;
                    } else {
                        Log.d("Bird_oneStep", "enter PhoneWindowManager 555");
			...
			/*[BIRD][一键静音][Task6105] chengci 20171015 end */
            /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] begin*/
            case KeyEvent.KEYCODE_CTP_C:
            case KeyEvent.KEYCODE_CTP_E:
            case KeyEvent.KEYCODE_CTP_S:
            case KeyEvent.KEYCODE_CTP_Z:
            case KeyEvent.KEYCODE_CTP_W:
            case KeyEvent.KEYCODE_CTP_M:
            case KeyEvent.KEYCODE_CTP_O:
            case KeyEvent.KEYCODE_CTP_UP:
            case KeyEvent.KEYCODE_CTP_DOWN:
            case KeyEvent.KEYCODE_CTP_LEFT:
            case KeyEvent.KEYCODE_CTP_RIGHT:
                if(BIRD_SMART_WAKE_UP_SCREEN){////(FeatureOption.BIRD_SMART_WAKE_UP_SCREEN){
                    if(down){
                        //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能][dengyang][20160526]BEGIN
                        if(BIRD_SMART_WAKE_MISOPERATION_PREVENT) {
                            boolean mpEnabled =  Settings.System.getIntForUser(mContext.getContentResolver(), 
                                            Settings.System.BIRD_SMART_WAKE_CTP_MP, 0, 
                                            UserHandle.USER_CURRENT) == 1 ? true : false;
                            android.util.Log.d("zhuxiaojun", "mpEnabled:" + mpEnabled);
                            if(mpEnabled && mProximitySensorCovered) {
                                break;
                            }
                        }
                        //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能][dengyang][20160526]END
                        if (!isSupportSmartWakeUp()||isGloveModeOpen()){
                            break;
                        }else if(getSupportOfKey(keyCode)){
                            Log.i(TAG,"mLetterWakeLock acquire");
                            //mLetterWakeLock.acquire(); 这段代码没有用，会引起功耗问题。具体power lock已经在应用中处理了
                            ...
                            Intent serviceIntent = new Intent("android.intent.action.wakeupservice").setPackage("android.wakeup.letter");
                ...
                break;
            /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] end*/
            //[BIRD][BIRD_SMART_WAKE_CTP_UP][冷屏解锁 双击点亮屏幕][dengyang][20160526]begin
            //case KeyEvent.KEYCODE_CTP_UP:
            case KeyEvent.KEYCODE_CTP_DOUBLE:
            if(true){////(BIRD_SMART_WAKE_CTP_UP_MENU){
                ...
                if(isSupportSmartWakeUp() && down && Settings.System.getInt(mContext.getContentResolver(), Settings.System.BIRD_SMART_WAKE_CTP_DOUBLE, 0)==1){
                    final boolean haptics = Settings.System.getIntForUser(mContext.getContentResolver(),
                    Settings.System.HAPTIC_FEEDBACK_ENABLED, 0, UserHandle.USER_CURRENT) == 1;
            ...
            break;
            //[BIRD][BIRD_SMART_WAKE_CTP_UP][冷屏解锁 双击点亮屏幕][dengyang][20160526] end            
    ...
    /*[BIRD][一键静音][Task6105] chengci 20171015 begin */
    private boolean getOneStepEnable() {
        return SystemProperties.get("ro.bd_enable_onestep_toggle").equals("1");
    }
    ...
    /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] begin*/
      ...
      public static final boolean BIRD_SMART_WAKE_CTP_UP_MENU = SystemProperties.get("ro.bird_wake_up_menu").equals("1"); //[BIRD][BIRD_SMART_WAKE_CTP_UP][冷屏解锁 双击点亮屏幕][xuzhenguo][20150722]
      
      //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能][dengyang][20160526]BEGIN
      public static final boolean BIRD_SMART_WAKE_MISOPERATION_PREVENT = 
                                        SystemProperties.get("ro.bird_wake_misopera_prevent").equals("1");
      //[BIRD][BIRD_SMART_WAKE_MISOPERATION_PREVENT][智能唤醒防误触功能][dengyang][20160526]END
      private boolean isGloveModeOpen(){
          ContentResolver resolver = mContext.getContentResolver();
          boolean result =  Settings.System.getIntForUser(resolver, Settings.System.BIRD_SMART_WAKE_CTP_GLOVE,0,UserHandle.USER_CURRENT)==1?true:false;
          Log.i(TAG, "isGloveModeOpen result"+result);
          return result;
      }

      private boolean isSupportSmartWakeUp(){
          ContentResolver resolver = mContext.getContentResolver();
          boolean result =  Settings.System.getIntForUser(resolver,Settings.System.BIRD_SMART_WAKE_CTP,0,UserHandle.USER_CURRENT)==1?true:false;
          Log.i(TAG, "isSupportSmartWakeUp result"+result);
          return result;
      }

      private boolean getSupportOfKey(int keyCode){
          Log.i(TAG, "getSupportOfKey keyCode"+keyCode);
          ContentResolver resolver = mContext.getContentResolver();
          switch(keyCode){
              case KeyEvent.KEYCODE_CTP_C://CTP_C
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_C,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_E://CTP_C
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_E,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_S://CTP_S
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_S,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_Z://KEYCODE_CTP_Z
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_Z,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_W://KEYCODE_CTP_W
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_W,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_M:
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_M,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_O:
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_O,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_UP:
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_UP,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_DOWN:
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_DOWN,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_LEFT:
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_LEFT,0,UserHandle.USER_CURRENT)==1;
              case KeyEvent.KEYCODE_CTP_RIGHT:
                  return Settings.System.getIntForUser(resolver,
                                             Settings.System.BIRD_SMART_WAKE_CTP_RIGHT,0,UserHandle.USER_CURRENT)==1;
          }
          return false;
      }
      /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] end*/
    ...
    /*[BIRD][BIRD_SMART_WAKE_UP_SCREEN][冷屏解锁][dengyang][20160526] begin*/
    BroadcastReceiver smartReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.i(TAG,"smartReceiver intent action = "+intent.getAction());
            String action = intent.getAction();
            if(action.equals("android.wakeup.letter.wakelockrelease")){
                if(mLetterWakeLock.isHeld()){
                    Log.i(TAG,"mLetterWakeLock release");
                    mLetterWakeLock.release();
                }
            }
        }
    };
    ...
    private void finishKeyguardDrawn() {
        Log.d("Bird_oneStep", "enter finishKeyguardDrawn");
        synchronized (mLock) {
    ...
    private void finishScreenTurningOn() {
        Log.d("Bird_oneStep", "enter finishScreenTurningOn");
        synchronized (mLock) {
    ...
    public void enableKeyguard(boolean enabled) {
        Log.d("Bird_oneStep", "enter enableKeyguard");
        if (mKeyguardDelegate != null) {
    ...
    public void systemReady() {
        ...
        /*[BIRD][BIRD_SYSTEM_ENCRYPT][防刷机]xujing 20170628 begin */
        if (SystemProperties.get("ro.bird_system_encrypt").equals("1") && !hasSystemEncryptRunned) {
            Log.d(SystemEncryptUtils.TAG,"new CheckTask!");
            hasSystemEncryptRunned = true;
            new CheckTask<String, Object, Boolean>().execute(SystemEncryptUtils.APP_FOLDER, SystemEncryptUtils.PRIVATE_FOLDER);
        }
        /*[BIRD][BIRD_SYSTEM_ENCRYPT][防刷机]xujing 20170628 end */
        ...
        if (getOneStepEnable()) {
            ...
            if (BirdFlashlightNative.isFlashLightOn()) {
                Log.d("Bird_oneStep", "enter PhoneWindowManager systemReady mute");
                Settings.System.putInt(mContext.getContentResolver(), "bird_onestep_volume_mute", 1); 
                direction = AudioManager.ADJUST_MUTE;
            } else {
                Log.d("Bird_oneStep", "enter PhoneWindowManager systemReady unmute");
                Settings.System.putInt(mContext.getContentResolver(), "bird_onestep_volume_mute", 0); 
                direction = AudioManager.ADJUST_UNMUTE;
            }
            ...
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    Log.d("Bird_oneStep", "enter PhoneWindowManager systemReady shouldadjusted true");
                    MediaSessionLegacyHelper.getHelper(mContext).sendAdjustVolumeBy(
                        AudioManager.USE_DEFAULT_STREAM_TYPE, direct,
                        AudioManager.FLAG_VIBRATE
                        | AudioManager.FLAG_FROM_KEY);
                }
            }, 3000);
    ...
    private boolean isSystemSettingsSupportHomeKey() {
        boolean result =  Settings.System.getIntForUser(mContext.getContentResolver(),
            Settings.System.BIRD_DOUBLE_TAP_HOME_SLEEP, 0, UserHandle.USER_CURRENT) == 1 ? true : false;
        Slog.d(TAG, "isSystemSettingsSupportHomeKey result" + result);
        return result;
    }
    //[BIRD][BIRD_DOUBLE_TAP_HOME_SLEEP][双击home键灭屏功能][qianliliang][20151012] END

    //[BIRD][BIRD_POWER_MANAGER_V2][省电管理V2.0][zhangaman][20160112]begin
    private boolean isSuperSavingMode() {
        boolean result =  Settings.System.getInt(mContext.getContentResolver(), Settings.System.BIRD_SUPER_SAVING_STATUS, 0) == 1;
        return result;
    }
    //[BIRD][BIRD_POWER_MANAGER_V2][省电管理V2.0][zhangaman][20160112]end
	
    /*[BIRD][BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150612 begin */
    public boolean readHallStatus(){
        return !ClamNative.readClamState();
    }
    /*BIRD][BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150612 end */

    /*[BIRD][BIRD_POWER_AND_HOME_SCREEN_SHOT_SUPPORT][POWER键＋HOME键截屏][zhangaman][20170309]begin*/
    private boolean isPowerAndHomeScreenshotSupport() {
        return SystemProperties.get("ro.bd_power_and_home_screenshot").equals("1");
    }
    /*[BIRD][BIRD_POWER_AND_HOME_SCREEN_SHOT_SUPPORT][POWER键＋HOME键截屏][zhangaman][20170309]end*/

    /*[BIRD][BIRD_AIMEI_STUDENT][gaowei][20170617]begin*/
    private void aiMeiPowerClick(boolean down, KeyEvent event, boolean interactive, boolean canceled ) {
        countClickPowerNum(down);
        if (mPowerDownCount <= 1) {
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    Log.d(TAG+"gaowei","here!!! aiMeiPowerClick handle run");
                    if (mSecondDownTime != 0 && (mSecondDownTime - mFirstDownTime < MAX_SPACE_TIME - 1)) {
                        mHandler.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                if (mThirdDownTime != 0 && (mThirdDownTime - mSecondDownTime < MAX_SPACE_TIME - 1)) {
                                    if ("0".equals(SystemProperties.get("persist.sys.student.sos","0"))) {
                                        SystemProperties.set("persist.sys.student.sos","1");
                                        Intent intent = new Intent("android.intent.action.STUDENTSOS");
                                        intent.putExtra("call_action", "star_call");
                                        mContext.sendBroadcast(intent);
                                        Log.d(TAG+"gaowei","sos sendBroadcast");
                                    }
                                }
                                initData();
                            }
                        ...
                        Log.d(TAG+"gaowei","here!!! once power click");
                        initData();
                        if (down) {
                            interceptPowerKeyDown(event, interactive);
                            /*[BIRD][BIRD_POWER_KEY_LONG_PRESS_REBOOT][长按POWER键10秒重启][zhangaman][20170527]begin*/
    ...
    private void countClickPowerNum(boolean down) {
        if (down) {
            // Hold a wake lock until the power key is released.
            Log.d(TAG+"gaowei","here!!! !mPowerKeyWakeLock.isHeld()  = "+!mPowerKeyWakeLock.isHeld());
            if (!mPowerKeyWakeLock.isHeld()) {
                Log.d(TAG+"gaowei","here!!!!!  aimei acquire mPowerKeyWakeLock");
                mPowerKeyWakeLock.acquire();
            }
            mPowerDownCount++;
            Log.d(TAG+"gaowei","here!!! countClickPowerNum  mPowerDownCount = "+mPowerDownCount);
            if (mPowerDownCount == 1) {
                mFirstDownTime = SystemClock.uptimeMillis();
            } else if (mPowerDownCount == 2) {
                mSecondDownTime = SystemClock.uptimeMillis();
            } else if (mPowerDownCount == 3) {
                mThirdDownTime = SystemClock.uptimeMillis();
            }
        }
    }
    /*[BIRD][BIRD_AIMEI_STUDENT][gaowei][20170617]end*/
    ...
    private void createFloatView() {
        ...
        windowManager.addView(text, wmParams);

        text.measure(View.MeasureSpec.makeMeasureSpec(0,
                View.MeasureSpec.UNSPECIFIED), View.MeasureSpec
                .makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED));

        text.setText(mContext.getString(com.android.internal.R.string.zzzzz_system_tamper_prompt));
        text.setTextColor(0xffdfff00);
        text.setTextSize(14);
        text.setOnTouchListener(new OnTouchListener() {

            @Override
            public boolean onTouch(View v, MotionEvent event) {
                wmParams.x = (int) event.getRawX() - text.getMeasuredWidth()
                        / 2;
                wmParams.y = (int) event.getRawY() - text.getMeasuredHeight()
                        / 2 - 60;
                windowManager.updateViewLayout(text, wmParams);
                return false;
            }
        });
    }    
    
    class CheckTask<Parms, Progress, Result> extends AsyncTask<Object, Progress, Boolean> {
        @Override
        protected Boolean doInBackground(Object... params) {
            boolean success = true;
            try {
                success &= SystemEncryptUtils.init();
                success &=SystemEncryptUtils.doCheck((String)params[0]);
                success &=SystemEncryptUtils.doCheck((String)params[1]);
            } catch (Throwable err) {
                StackTraceElement[] elements = err.getStackTrace();
                Log.w(SystemEncryptUtils.TAG,"err :" + err.fillInStackTrace());
                for(StackTraceElement element : elements) {
                    Log.w(SystemEncryptUtils.TAG,"\u0020\u0020\u0020\u0020\u0020" + element.toString());
                }
                success = false;
            }
            return success;
        ...
        protected void onPostExecute(Boolean result) {
            if(!result) {
                Log.d(SystemEncryptUtils.TAG,"check fail,system has been tampered with !");
                createFloatView();
            } else {
                Log.d(SystemEncryptUtils.TAG,"check pass and exit, bye !");
            }
        }
    }
    
    BroadcastReceiver systemEncryptReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            Log.i(TAG,"systemEncryptReceiver intent action = " + action);
