...
public class UsbDeviceManager {
    ...
    //[BIRD][BIRD_USB_DEFAULT_FUNCTION_SETTING][USB连接电脑时默认功能][qianliliang][20160630] BEGIN
    private static final boolean BIRD_USB_DEFAULT_FUNCTION_SETTING = SystemProperties.get("ro.bd_usb_func_addition").equals("1");
    //[BIRD][BIRD_USB_DEFAULT_FUNCTION_SETTING][USB连接电脑时默认功能][qianliliang][20160630] END
...
functions = addFunction(functions, UsbManager.USB_FUNCTION_ACM);
   ...
   String tmp = ( (port_num == 2) ? UsbManager.USB_FUNCTION_DUAL_ACM : UsbManager.USB_FUNCTION_ACM);
        ...
        private boolean setUsbConfig(String config) {
            ...
            //[BIRD][BIRD_USB_DEFAULT_FUNCTION_SETTING][USB连接电脑时默认功能][qianliliang][20160630] BEGIN
            String oldConfigProperty = SystemProperties.get(USB_PERSISTENT_CONFIG_PROPERTY);
            if (BIRD_USB_DEFAULT_FUNCTION_SETTING && !config.equals(oldConfig)
                && (config != null && config.length() >= 1 && config.indexOf("accessory") == -1)) {
                //cts (config != null && config.length() >= 1 && config.indexOf("accessory") == -1)
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_UPDATE_STATE:
                    ...
                    //[BIRD][BIRD_USB_DEFAULT_FUNCTION_SETTING][USB连接电脑时默认功能][qianliliang][20160630] BEGIN
                    } else if (BIRD_USB_DEFAULT_FUNCTION_SETTING) {
                        if (!SystemProperties.get("persist.sys.usb.flag").equals("1")) {
                            mUsbDataUnlocked = true;
                            Slog.i(TAG, "BIRD_USB_DEFAULT_FUNCTION_SETTING mUsbDataUnlocked = true");
                        }
                        Slog.i(TAG, "FATAL Fatal BIRD_USB_DEFAULT_FUNCTION_SETTING");
                    }
                    //[BIRD][BIRD_USB_DEFAULT_FUNCTION_SETTING][USB连接电脑时默认功能][qianliliang][20160630] END
                    
                    /*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170817]BEGIN */
                    if (FeatureOption.BIRD_ENCRYPT_SPACE) {
                        if (mCurrentUser == ContainerManager.SECURITY_USER_ID) {
                            mUsbDataUnlocked = false;
                        }
                    }
                    /*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170817]END */
                ...
                case MSG_USER_SWITCHED: {
                    /*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170817]BEGIN */
                    if (FeatureOption.BIRD_ENCRYPT_SPACE) {
                        if (msg.arg1 == ContainerManager.SECURITY_USER_ID) {
    ...
    public void setCurrentFunctions(String functions) {
        ...
        //[BIRD][BIRD_USB_DEFAULT_FUNCTION_SETTING][USB连接电脑时默认功能][qianliliang][20160630] BEGIN
        if (BIRD_USB_DEFAULT_FUNCTION_SETTING) {
            if (functions == null || (functions != null && functions.length() >= 1 && functions.indexOf("accessory") != -1)) {
                //cts
                //(functions != null && functions.length >= 1 && functions.indexOf("o") != -1)
                SystemProperties.set("persist.sys.usb.flag", "1");
                if (UsbManager.containsFunction(SystemProperties.get(USB_PERSISTENT_CONFIG_PROPERTY),
                        UsbManager.USB_FUNCTION_ADB)) {
                    SystemProperties.set(USB_PERSISTENT_CONFIG_PROPERTY, "mtp,adb");
                } else {
                    SystemProperties.set(USB_PERSISTENT_CONFIG_PROPERTY, "mtp");
                }
            } else {
                SystemProperties.set("persist.sys.usb.flag", "0");
            }
        }
        //[BIRD][BIRD_USB_DEFAULT_FUNCTION_SETTING][USB连接电脑时默认功能][qianliliang][20160630] END
