...
/*[BIRD][BIRD_SML_UNLOCK_CONFIG][根据IMEI码计算锁网解锁密码][yangbo][20150920]BEGIN */
import android.telephony.TelephonyManager;
/*[BIRD][BIRD_SML_UNLOCK_CONFIG][根据IMEI码计算锁网解锁密码][yangbo][20150920]END */

//[BIRD][西班牙语翻译][yangheng][20161227] BEGIN
import java.util.Locale; 
//[BIRD][西班牙语翻译][yangheng][20161227] END
...
public class KeyguardSimPinPukMeView extends KeyguardPinBasedInputView {
    ...
    private String[] strLockName;//[BIRD][西班牙语翻译][yangheng][20161227]
    ...
    /*[BIRD][BIRD_SML_UNLOCK_CONFIG][根据IMEI码计算锁网解锁密码][yangbo][20150920]BEGIN */
    private String mSMLUnlockPassWord= "12345678";
    /*[BIRD][BIRD_SML_UNLOCK_CONFIG][根据IMEI码计算锁网解锁密码][yangbo][20150920]END */
                ...
                if (IccCardConstants.State.READY == simState) {
                    ...
                    if (!KeyguardUtils.isMediatekSimMeLockSupport()) {
                        ...
                        if (com.android.keyguard.FeatureOptions.BIRD_FREEZE_PHONE_WHEN_SLOT_PERMANENTLY_LOCKED) {
                            ...
                            prefs.edit().putBoolean(com.android.keyguard.FeatureOptions.BIRD_PREF_PHONE_FREEZE, true).commit();//卡槽被完全锁死了
                        } else {
                            mCallback.dismiss(true);
                        }
                        /*[BIRD][BIRD_FREEZE_PHONE_WHEN_SLOT_PERMANENTLY_LOCKED][当卡槽被完全锁死(锁网剩余次数为0)时，手机将无法使用][yangbo][20170506]END */
    ...
    public KeyguardSimPinPukMeView(Context context, AttributeSet attrs) {
        ...
        //[BIRD][西班牙语翻译][yangheng][20161227] BEGIN
        if(isSpanish()) {
            strLockName = new String[] {" NP ", " NSP ", " SP ", " CP ", " SIMP "};
        } else {
            strLockName = new String[] {" [NP]", " [NSP]", " [SP]", " [CP]", " [SIMP]"};
        }
        //[BIRD][西班牙语翻译][yangheng][20161227] END

        /*[BIRD][BIRD_REMOVE_SIMLOCK_NAME][去掉锁屏界面上的锁网类型名称(NP、NSP等等)][yangbo][20170418]BEGIN */
        if (SystemProperties.get("ro.bd_remove_simlock_name").equals("1")) {
            strLockName = new String[] {" ", " ", " ", " ", " "};
        }
        /*[BIRD][BIRD_REMOVE_SIMLOCK_NAME][去掉锁屏界面上的锁网类型名称(NP、NSP等等)][yangbo][20170418]END */
    ...
    public void resetState(boolean forceReload) {
        ...
        //[BIRD][BIRD_SHOW_NETWORK_LOCK_TIP][锁网界面显示“请在slot1插入Seatel SIM卡”][pangmeizhou][20161225]begin
        View networkLockTip = findViewById(R.id.network_lock_message);
        boolean showNetworkLockTip = false;
        if(SystemProperties.get("ro.db.show_netlock_tip").equals("1")){
            networkLockTip.setVisibility(View.VISIBLE);
        }
        //[BIRD][BIRD_SHOW_NETWORK_LOCK_TIP][锁网界面显示“请在slot1插入Seatel SIM卡”][pangmeizhou][20161225]end
        //Update Sim Icon
        ...
        if (count < 2) {
            ...
            /*[BIRD][TASK-5427][双卡PIN码西班牙文翻译][chenguangxiang][20170720]begin*/
            Log.i("chenguangxiang", "displayName:"+displayName);
            if (info != null) {
                String language = Locale.getDefault().getLanguage();
                if (language.endsWith("es") && (displayName.equals("Card 1") || displayName.equals("CARD 1"))) {
                    displayName = "SIM 1";
                } else if (language.endsWith("es") && (displayName.equals("Card 2") || displayName.equals("CARD 2"))){
                    displayName = "SIM 2";
                }
            }
            /*[BIRD][TASK-5427][双卡PIN码西班牙文翻译][chenguangxiang][20170720]end*/
    ...
    protected void onFinishInflate() {
        ...
        if (SystemProperties.get("ro.bird_mulit_simlock_viettel").equals("1")) {
    ...
    private String getRetryMeString(final int phoneId) {
        ...
        /*[BIRD][BIRD_REMOVE_ME_RETRY_COUNT][不显示剩余锁网密码次数][pangmeizhou][20150920]BEGIN */
        if (SystemProperties.get("ro.bird_rm_me_count").equals("1")){
            return "";
        } else {
            return "(" + mContext.getString(R.string.retries_left, meRetryCount) + ")";
        }
        /*[BIRD][BIRD_REMOVE_ME_RETRY_COUNT][不显示剩余锁网密码次数][pangmeizhou][20150920]END */
    }
    ...
    private abstract class CheckSimMe extends Thread {
        ...
        protected CheckSimMe(String passwd, int phoneId) {
            /*[BIRD][BIRD_SML_UNLOCK_CONFIG][根据IMEI码计算锁网解锁密码][yangbo][20150920]BEGIN */
            if (SystemProperties.get("ro.bird_sml_unlock_cfg").equals("1")){
                TelephonyManager tempTelephonyManager = (TelephonyManager) getContext().getSystemService(Context.TELEPHONY_SERVICE);
                String imei = tempTelephonyManager.getDeviceId();//get imei
                Log.d(TAG, "CheckSimMe [IMEI]+" + imei);
                String imeipasswd = getImeiPassword(imei);//依照客户的算法要求，根据IMEI来获得一个解锁密码
                Log.d(TAG, "CheckSimMe [Single Card][imeipasswd]" + imeipasswd);
                //用户输入
                if((null!=passwd) && (null!=imeipasswd) && (null!=imei) && (passwd.compareTo(imeipasswd) == 0)) { //跟用户输入比较，若相等，则解开
                    mPasswd = getSMLPassword(); //赋予正确的密码   
                } else {
                    String tempPassWd = new String(getSMLPassword());
                    int length = tempPassWd.length();
                    char[] data = tempPassWd.toCharArray();
                    //0~8,add 1,else -1,because data always is digit
                    if('0'<= data[length-1] && data[length-1]<'9') {
                        data[length-1] += 1;
                    } else if(data[length-1]=='9') {
                        data[length-1] -= 1;
                    }
                    Log.d(TAG, "[CheckSimMe][data[length-1]]:" + data[length-1]);  
                    String errorPassWd = new String(data);     
                    Log.d(TAG, "[CheckSimMe][errorPassWd]" + errorPassWd);  
                    mPasswd = errorPassWd; //赋予错误的密码    
                }
            } else {
                mPasswd = passwd;
            }
   ...
   @Override
    ...
    /*[BIRD][BIRD_SML_UNLOCK_CONFIG][根据IMEI码计算锁网解锁密码][yangbo][20150920]BEGIN */
    /* Description 根据IMEI来获得一个解锁密码
    * 客户要求算法是：
    * 自动产生的密码算法（从左到右）：
    * 1.第1位，IMEI码前8位数相加取其的个位数；
    * 2.第2位，IMEI码第2位到第9位数相加取其的个位数；
    * 3.第3位，IMEI码第3位到第10位数相加取其的个位数；
    * 这里先算出前八位的和的个位数(从左到右)，要算下一个八位值，需要前一个八位值和加上最后一位的值减去前一个八位值和的第一位，这样就省去很多重复计算，从而提高效率
    * input : imei
    * return password
    */
    private String getImeiPassword(String imei) {
        if(imei == null) {
            return null;
        }
        StringBuilder passwdNumber = new StringBuilder(8);
        char[] buf = new char[8];
        int digit = 0;
        int prior_eight = 0;
        for(int i = 0; i < 8; i++) {
            prior_eight += (int)(imei.charAt(i) - '0');          
        }
        digit = prior_eight % 10;
        passwdNumber.append(digit);  
        Log.d(TAG, "[getImeiPassword][first]"+digit);
        for(int j = 8; j < imei.length(); j++) {
            digit = prior_eight + (int)(imei.charAt(j) - '0') - (int)(imei.charAt(j-8) - '0');
            prior_eight = digit;
            digit %= 10;
            passwdNumber.append(digit);
        } 
        return passwdNumber.toString();
    }

    private void setSMLPassword(String passwd) {
        mSMLUnlockPassWord = passwd;
    }
    
    private String getSMLPassword() {
        return mSMLUnlockPassWord;
    }
    /*[BIRD][BIRD_SML_UNLOCK_CONFIG][根据IMEI码计算锁网解锁密码][yangbo][20150920]END */
}
