...
//[BIRD][BIRD_LOCK_LEFT_NAME_BIG][在锁屏界面左上角的运营商名称修改][yangheng][20170629] BEGIN
import android.os.SystemProperties;
//[BIRD][BIRD_LOCK_LEFT_NAME_BIG][在锁屏界面左上角的运营商名称修改][yangheng][20170629] END
/*[BIRD][BIRD_MULTI_SIMLOCK_VIETTEL_IMPROVE][Viettel双卡锁网需求改进][字符串部分][yangbo][20171221]BEGIN */
import com.android.keyguard.FeatureOptions;
...
public class CarrierText extends TextView {
    ...
    public CarrierText(Context context, AttributeSet attrs) {
        ...
        try {
            //[BIRD][BIRD_LOCK_LEFT_NAME_BIG][在锁屏界面左上角的运营商名称修改][yangheng][20170629] BEGIN
            if (SystemProperties.get("ro.bd_lock_left_name_big").equals("1")) {
                useAllCaps = true;
            } else {
        ...
        for (int i = 0 ; i < mNumOfPhone ; i++) {
            ...
            if (mCarrierNeedToShow[i] && (mCarrier[i] != null)) {
                ...
                if (FeatureOption.BIRD_KUSAI_KEYGUARD) {
                    if (mCarrier[i].equals("CHN-UNICOM")) {
                        mCarrier[i] = "CU";
                    }
                    if (isZh()) {
                        if (mCarrier[i].equals("cmcc") || mCarrier[i].equals("CMCC") || mCarrier[i].equals("Cmcc")) {
                            mCarrier[i]  = "中国移动";
                        } else if (mCarrier[i].equals("CU")) {
                            mCarrier[i]  = "中国联通";
                        }
                    }
                }
    ...
    private CharSequence getCarrierTextForSimState(int phoneId, IccCardConstants.State simState,
        ...
        Log.i("chenguangxiang", "status:" + status);
        switch (status) {
            case Normal:
                carrierText = text;
                /*[BIRD][BIRD_MULTI_SIMLOCK_VIETTEL_IMPROVE][Viettel双卡锁网需求改进][字符串部分][yangbo][20171221]BEGIN */
                if (FeatureOptions.BIRD_MULTI_SIMLOCK_VIETTEL_IMPROVE) {
                    if (getContext().getText(com.android.internal.R.string.emergency_calls_only).equals(text)) {
                        carrierText = getContext().getText(R.string.bird_invalid_sim);
                    }
                }
                /*[BIRD][BIRD_MULTI_SIMLOCK_VIETTEL_IMPROVE][Viettel双卡锁网需求改进][字符串部分][yangbo][20171221]END */
                break;
            ...
            case SimMissing:
                ...
                */
                ...
                /*[BIRD][BIRD_MULTI_SIMLOCK_VIETTEL_IMPROVE][Viettel双卡锁网需求改进][字符串部分][yangbo][20171221]BEGIN */
                if (FeatureOptions.BIRD_MULTI_SIMLOCK_VIETTEL_IMPROVE) {
                    carrierText = getContext().getText(R.string.bird_insert_sim);
                }
                /*[BIRD][BIRD_MULTI_SIMLOCK_VIETTEL_IMPROVE][Viettel双卡锁网需求改进][字符串部分][yangbo][20171221]END */
