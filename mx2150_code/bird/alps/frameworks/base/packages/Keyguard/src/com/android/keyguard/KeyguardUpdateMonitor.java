...
//[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][指纹识别失败，亮屏提示][dingjiayuan][20161031]BEGIN
import android.os.PowerManager; 
//[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][指纹识别失败，亮屏提示][dingjiayuan][20161031]END
/**
...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
	...
	private static final String KEYGUARD_NOTIFICATION_MANAGER_LOCK= "com.bird.captainmanager.lock";
	private static final String KEYGUARD_NOTIFICATION_MANAGER_UNLOCK= "com.bird.captainmanager.unlock";
    ...
    /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160826]BEGIN */
    private void startQuickCallOrQuickStart(Fingerprint fp) {
        if (fp == null ) {
            return;
        }
        String number = mFpm.getMakeCallNumber(fp.getFingerId(), fp.getGroupId());
	    if (!TextUtils.isEmpty(number)) {
	        ...
	        String quickStart = mFpm.getQuickStartPackageName(fp.getFingerId(), fp.getGroupId());
	        if (!TextUtils.isEmpty(quickStart)) {
	            String[] s = quickStart.split("\\|");
	            try {
		            Intent intent = new Intent();
				    intent.setAction("android.intent.action.MAIN");
				    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				    intent.putExtra("from_fingerprint_quickstart", true);
				    intent.setClassName(s[0], s[1]);
				    if (mContext.getPackageManager().resolveActivity(intent, 0) != null) {
				        saveToUnlockedAppsList(s[0]);
				        mContext.startActivity(intent);
				    }
	...
	public void saveToUnlockedAppsList(String pkgName) {
        if (TextUtils.isEmpty(pkgName)) {
            return;
        }
		String appsListString = Settings.System.getString(mContext.getContentResolver(), Settings.System.BIRD_ALREADY_UNLOCKED_PACKAGESNAME);
		boolean needSave = true;
		if (!TextUtils.isEmpty(appsListString)) {
			String[] apps = appsListString.split("\\|");
			boolean has = false;
			for (int i = 0; i < apps.length; i++) {
			    if (pkgName.equals(apps[i])) {
			        needSave = false;
			        break;
			    }
		    }
		...
		if (needSave) {
		    if ("com.android.dialer".equals(pkgName) || "com.android.contacts".equals(pkgName)) {
		        Settings.System.putString(mContext.getContentResolver(), Settings.System.BIRD_ALREADY_UNLOCKED_PACKAGESNAME, appsListString + "com.android.dialer|com.android.contacts|");
		    } else {
		        Settings.System.putString(mContext.getContentResolver(), Settings.System.BIRD_ALREADY_UNLOCKED_PACKAGESNAME, appsListString + pkgName + "|");
		    }
		}
	}

    private void onFingerprintAuthenticated(int userId, Fingerprint fp) {
    ...
    private void handleFingerprintAuthFailed() {
        //[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][指纹识别失败，点亮屏幕][dingjiayuan][20161031]BEGIN
        if (android.os.SystemProperties.get("ro.bd_fingerprint_ex").equals("1")) {
            mPM = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
            mPM.wakeUp(SystemClock.uptimeMillis());
            Log.i(TAG+"-dingjiayuan","handleFingerprintAuthFailed()");
	    }
		//[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][指纹识别失败，点亮屏幕][dingjiayuan][20161031]END
        ...
        //[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][指纹识别失败，点亮屏幕][dingjiayuan][20161031]BEGIN
        if (android.os.SystemProperties.get("ro.bd_fingerprint_ex").equals("1")) {
            handleFingerprintHelp(-1, mContext.getString(R.string.fingerprint_failed));
        } else {
    ...
    private void handleFingerprintAuthenticated(int authUserId,Fingerprint fp) {
    /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160930]END */
        try {
            final int userId;
            try {
            ...
            /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160930]BEGIN */
            onFingerprintAuthenticated(userId, fp);
            /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160930]END */
        } finally {
    ...
    private void proceedToHandleSimStateChanged(SimData simArgs) {
        if ((IccCardConstants.State.NETWORK_LOCKED == simArgs.simState) &&
            ...
            int birdsecret = Settings.System.getInt(mContext.getContentResolver(), "birdunlocksecret", 0);
            int state = TelephonyManager.from(mContext).getSimState(PhoneConstants.SIM_ID_1);
            if (SystemProperties.get("ro.bird_mulit_simlock_viettel").equals("1") && birdsecret == 1) {
                new simMeCheckThread("12345678", simArgs.phoneId).start();
            } else if((simArgs.phoneId == PhoneConstants.SIM_ID_2) && android.os.SystemProperties.get("ro.bird_mulit_simlock_viettel").equals("1")) {
                Log.i("chenguangxiang", "state:" + state);
                Log.i("chenguangxiang", "isExpiredSIM:" + isExpiredSIM(PhoneConstants.SIM_ID_1));
                if((false == isExpiredSIM(PhoneConstants.SIM_ID_1)) && (state == TelephonyManager.SIM_STATE_READY)) {
                    //参考FAQ12121 [SEC]在app端创建线程来解锁SIM ME lock
                    //mPasswd为modem配置的解锁密码.
                    //过期卡依赖关系，需要屏蔽该句，在GsmCdmaPhone.java实现解锁
                    //new simMeCheckThread("12345678", simArgs.phoneId).start();
                } else {
                    //走正常解锁流程
                ...
                int birdtranslock = Settings.System.getInt(mContext.getContentResolver(), "birdtranslock", 0);
                Log.i("chenguangxiang", "birdtranslock:" + birdtranslock);
                if (birdtranslock == 1) {
                    new simMeCheckThread("12345678", simArgs.phoneId).start();
                }
            } else if (android.os.SystemProperties.get("ro.bird_mulit_simlock_viettel").equals("1")) {
                //new simMeStatusQueryThread(simArgs).start();
            } else {
                new simMeStatusQueryThread(simArgs).start();
            }
    ...
    private class simMeCheckThread extends Thread {
        ...
        protected simMeCheckThread (String passwd, int slotId) {
        ...
        public void run() {
            ...
            try {
                ...
                Log.d(TAG, "CheckMe, " + "mSlotId = " + mSlotId + ", subId = " + subIds[0]);
    ...
    public static boolean isExpiredSIM (int simId) {
        ...
        if (ss != null) {
            Log.i("chenguangxiang", "ServiceState getVoiceRegState:" + ss.getVoiceRegState());
    ...
    private FingerprintManager.AuthenticationCallback mAuthenticationCallback
        ...
        public void onAuthenticationSucceeded(AuthenticationResult result) {
            /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160930]BEGIN */
            handleFingerprintAuthenticated(result.getUserId(),result.getFingerprint());
            /*[BIRD][BIRD_FINGER_PRINT][指纹功能拓展][yangbo][20160930]END */
        }
...
/**    private void handleSimStateChange(int subId, int slotId, State state) {
    ...
    public void onKeyguardVisibilityChanged(boolean showing) {
        ...
        updateFingerprintListeningState();
		/*[BIRD][BIRD_KEYGUARD_NOTIFICATION] [船员项目锁屏通知控制功能] yeyunfeng 20171226 begin*/
		if (android.os.SystemProperties.get("ro.bd_keyguard_notification").equals("1")) {
			if(showing){
			//com.bird.captainmanager.lock
				Intent intent=new Intent();
				intent.setAction(KEYGUARD_NOTIFICATION_MANAGER_LOCK);
				mContext.sendBroadcast(intent);	
			Settings.System.putInt(mContext.getContentResolver(), Settings.System.BIRD_KEYGUARD_NOTIFICATION_ENABLE,1);								
			}else{
				//com.bird.captainmanager.unlock
				Intent intent=new Intent();
				intent.setAction(KEYGUARD_NOTIFICATION_MANAGER_UNLOCK);
				mContext.sendBroadcast(intent);		
				Settings.System.putInt(mContext.getContentResolver(), Settings.System.BIRD_KEYGUARD_NOTIFICATION_ENABLE,0);		
			}
		}
		/*[BIRD][BIRD_KEYGUARD_NOTIFICATION] [船员项目锁屏通知控制功能] yeyunfeng 20171226 end*/
    }
    ...
    private boolean refreshSimState(int subId, int slotId) {
        ...
        try {
            ...
            //[BIRD][BIRD_MULTI_SIMLOCK_VIETTEL_IMPROVE][Viettel双卡锁网需求改进][chenguangxiang][20171221] BEGIN
            if (android.os.SystemProperties.get("ro.bird_mulit_simlock_viettel").equals("1")) {
