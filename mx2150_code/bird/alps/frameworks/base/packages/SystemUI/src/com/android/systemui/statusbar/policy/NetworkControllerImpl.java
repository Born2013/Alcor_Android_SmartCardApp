...
public class NetworkControllerImpl extends BroadcastReceiver
    ...
    boolean mNeedSendNotification = false;
    boolean mIsNotificationSended = false;
    int current4GSubId = -100;
    //[BIRD][BIRD_SEATEL_4G_NOTIFICATION][Seatel运营商卡处于4G状态时通知栏中显示图片，其他运营商不显示][yangheng][20170228] END
    ...
    public NetworkControllerImpl(Context context, Looper bgLooper) {
        ...
        //[BIRD][BIRD_SEATEL_4G_NOTIFICATION][Seatel运营商卡处于4G状态时通知栏中显示图片，其他运营商不显示][yangheng][20170228] BEGIN
        if(FeatureOption.BIRD_SEATEL_4G_NOTIFICATION) {
            mNotificationManager = (NotificationManager)context.getSystemService(Context.NOTIFICATION_SERVICE);
            mRemoteView = new RemoteViews(context.getPackageName(),R.layout.custom_notification_view);
            mBuilder = new Notification.Builder(context);
            mBuilder.setSmallIcon(R.drawable.ic_sim_seatel_icon);
    ...
    //[BIRD][BIRD_SEATEL_4G_NOTIFICATION][Seatel运营商卡处于4G状态时通知栏中显示图片，其他运营商不显示][yangheng][20170228] BEGIN
    void updateSeatelNotification(int subId, int dataNetType){
        if(!FeatureOption.BIRD_SEATEL_4G_NOTIFICATION) {
            return;
        }
        Log.i(TAG,"updateSeatelNotification mSlotCount = "+mSlotCount);
        SubscriptionInfo subinfo= (SubscriptionInfo)SIMHelper.getSubInfoById(mContext, subId);
        Log.i(TAG,"updateSeatelNotification subinfo = "+subinfo);
        if(subinfo != null) {
            int mcc = subinfo.getMcc();
            int mnc = subinfo.getMnc();
            Log.i(TAG,"updateSeatelNotification mcc = "+mcc+" mnc = "+mnc);
            if(mcc == 456 && mnc == 11) {
                if(dataNetType == TelephonyManager.NETWORK_TYPE_LTE) {
                    mNeedSendNotification = true;
                    current4GSubId = subId;
                } else {
                    mNeedSendNotification = false;
                }
            }
        }
        Log.i(TAG,"updateSeatelNotification subId = "+subId +" current4GSubId = "+current4GSubId);
        Log.i(TAG,"updateSeatelNotification mNeedSendNotification = "+mNeedSendNotification
                +" mIsNotificationSended = "+mIsNotificationSended);
        if(current4GSubId == subId && mNeedSendNotification && mIsNotificationSended == false) {//当前卡处于4G状态，需要发送并且没有发送过通知
            mNotificationManager.notify(0,mNotification);
            mIsNotificationSended = true;
        }

        if(current4GSubId == subId && !mNeedSendNotification && mIsNotificationSended == true) {//当前卡处于非4G状态，需要取消并且通知已经发过了。
            mNotificationManager.cancel(0);
            mIsNotificationSended = false;
        }        
    }


    NetworkTypeChangedCallback mNetworkTypeCallback = new NetworkTypeChangedCallback(){
        public void onNetworkTypeChanged(int subId, int dataNetType){
            updateSeatelNotification(subId, dataNetType);
        }
    };

    public interface NetworkTypeChangedCallback {
        public void onNetworkTypeChanged(int subId, int dataNetType);
    }
    //[BIRD][BIRD_SEATEL_4G_NOTIFICATION][Seatel运营商卡处于4G状态时通知栏中显示图片，其他运营商不显示][yangheng][20170228] END
    ...
    private void registerListeners() {
        ...
        //[BIRD][BIRD_SIM_ICON_COLORFUL][状态栏SIM卡图标着色][qianliliang][20160519] BEGIN
        if (FeatureOption.BIRD_SIM_ICON_COLORFUL) {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        /// @}
        //[BIRD][BIRD_SIM_ICON_COLORFUL][状态栏SIM卡图标着色][qianliliang][20160519] BEGIN
        else if (FeatureOption.BIRD_SIM_ICON_COLORFUL 
                && action.equals(TelephonyIntents.ACTION_SUBINFO_CONTENT_CHANGE) && mMobileSignalControllers != null) {
            Log.i(TAG, "BIRD, ACTION_SUBINFO_CONTENT_CHANGE, action = " + action);
            for (MobileSignalController controller : mMobileSignalControllers.values()) {
                controller.setTintColorForController();
            }
        }
        //[BIRD][BIRD_SIM_ICON_COLORFUL][状态栏SIM卡图标着色][qianliliang][20160519] END
        else {
    ...
    void setCurrentSubscriptions(List<SubscriptionInfo> subscriptions) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (cachedControllers.containsKey(subId)) {
                ...
                //[BIRD][BIRD_SEATEL_4G_NOTIFICATION][Seatel运营商卡处于4G状态时通知栏中显示图片，其他运营商不显示][yangheng][20170228] BEGIN
                if(FeatureOption.BIRD_SEATEL_4G_NOTIFICATION) {
                    Log.i(TAG,"!!! cachedControllers.containsKey(subId)...");
                    controller.setNetworkTypeChangedListener(mNetworkTypeCallback);
                }
                //[BIRD][BIRD_SEATEL_4G_NOTIFICATION][Seatel运营商卡处于4G状态时通知栏中显示图片，其他运营商不显示][yangheng][20170228] END
