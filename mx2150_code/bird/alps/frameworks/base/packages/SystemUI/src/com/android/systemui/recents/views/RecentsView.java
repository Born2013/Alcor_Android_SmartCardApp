...
public class RecentsView extends FrameLayout {
    ...
    public void setClear(View view) {
        ...
        mClearView.setClipToOutline(true);
    }
    //[BIRD][BIRD_MULTI_TASK][多任务功能增强][chenguangxiang][20161128] end
    ...
    private void showStackActionButton(final int duration, final boolean translate) {
        ...
        if (SystemProperties.get("ro.bird_multi_task").equals("1")) {
            if (mClearView != null) {
                if (mClearView.getVisibility() == View.INVISIBLE) {
                    mClearView.setVisibility(View.VISIBLE);
                    mClearView.setAlpha(0f);
                    postAnimationTrigger.addLastDecrementRunnable(new Runnable() {
                        @Override
                        public void run() {
                            if (translate) {
    ...
    private void hideStackActionButton(int duration, boolean translate,
        ...
        if (SystemProperties.get("ro.bird_multi_task").equals("1")) {
            if (mClearView != null) {
                if (mClearView.getVisibility() == View.VISIBLE) {
                    mClearView.animate()
                            .alpha(0f)
                            .setDuration(duration)
                            .setInterpolator(Interpolators.FAST_OUT_SLOW_IN)
