...
/*[BIRD] [BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150602 begin*/
import android.content.res.Configuration;
import android.os.Handler;
import android.os.Message;
/*[BIRD] [BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150602 end*/
...
public class StatusBarKeyguardViewManager implements RemoteInputController.Callback {
    ...
    public void dismiss(boolean authenticated) {
        ...
        if (mDeviceInteractive || mDeviceWillWakeUp) {  
            ...
            if (com.android.systemui.FeatureOption.BIRD_FACELOCK_SCREEN) { 
               boolean isFaceLockScreen = isSecure() && FaceLockManager.isRegistered();
               if (!isFaceLockScreen) {
                     showBouncer();
               }
            } else {
    ...
    public boolean onBackPressed(boolean isLongPress) {
        ...
        if (isLongPress && FeatureOption.BIRD_ONBACKPRESS_TAKE_PHOTO
            && Settings.System.getInt(mContext.getContentResolver(), Settings.System.BIRD_SUPER_SAVING_STATUS, 0) != 1) {
            mPhoneStatusBar.mKeyguardBottomArea.launchCamera(KeyguardBottomAreaView.CAMERA_LAUNCH_SOURCE_AFFORDANCE, true);
        }
    ...
    /*[BIRD] [BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150602 begin*/
    private static final int MSG_SHOW_SMALL_WINDOW = 30;
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_SHOW_SMALL_WINDOW:
                    mStatusBarWindowManager.adjustStatusBarHeight(msg.arg1 == 1);
                    break;
            }
        }
    };
    public boolean isShowLeather(){
    	if(mStatusBarWindowManager != null){
    		return mStatusBarWindowManager.isSmallWindowEnable();
    	}
    	return false;
    }
    public void showSmallWindow(boolean showing) {
    	if(mStatusBarWindowManager != null){
			if(mBouncer.isShowing()) {
				if(mShowing) {
					mBouncer.hide(false /* destroyView */);
    		...
    		if(!mShowing){
    			mStatusBarWindowManager.setKeyguardShowing(showing);
    		}
            ...
            mHandler.removeMessages(MSG_SHOW_SMALL_WINDOW);
            Log.d("aaman","ori = "+ori+",mConfiguration.ORIENTATION_LANDSCAPE = "+mConfiguration.ORIENTATION_LANDSCAPE);
            if (ori == mConfiguration.ORIENTATION_LANDSCAPE) {
                Message msg = mHandler.obtainMessage(MSG_SHOW_SMALL_WINDOW);
                msg.arg1 = showing ? 1 : 0;
                mHandler.sendMessageDelayed(msg, 500);
            } else {
                mStatusBarWindowManager.adjustStatusBarHeight(showing);
            }
    	}
    }
    /*[BIRD] [BIRD_SMALL_VIEW_WINDOW] mandy.wu 20150602 end*/
