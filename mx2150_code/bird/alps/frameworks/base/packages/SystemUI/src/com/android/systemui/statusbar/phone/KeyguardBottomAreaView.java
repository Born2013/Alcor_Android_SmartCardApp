...
public class KeyguardBottomAreaView extends FrameLayout implements View.OnClickListener,
    ...
    /// @}
    /*[BIRD][BIRD_DIRECT_WITH_PROXIMITY][智能体感][yangbo][20150605]BEGIN */
    private UnLockProxSensor mUnLockProxSensor;
    /*[BIRD][BIRD_DIRECT_WITH_PROXIMITY][智能体感][yangbo][20150605]END */
            ...
            if (action == ACTION_CLICK) {
                if (host == mLockIcon) {
                    ...
                    return true;
                     ...
                     //[BIRD_ONBACKPRESS_TAKE_PHOTO][在锁屏亮屏界面长按back打开相机并拍照][hongzhihao][20170613]
                    launchCamera(CAMERA_LAUNCH_SOURCE_AFFORDANCE, false);
                    //[BIRD_ONBACKPRESS_TAKE_PHOTO][在锁屏亮屏界面长按back打开相机并拍照][hongzhihao][20170613]
                    return true;
                } else if (host == mLeftAffordanceView) {
    ...
    private void updateCameraVisibility() {
        ...
        if (FeatureOption.BIRD_POWER_MANAGER_V2) {
            visible = visible && Settings.System.getInt(mContext.getContentResolver(),
                            Settings.System.BIRD_SUPER_SAVING_STATUS, 0) != 1;
        }
        //[BIRD][BIRD_POWER_MANAGER_V2][省电管理V2.0][qianliliang][20160527] END
    ...
    private void updateLeftAffordanceIcon() {
        /*[TASK-4827][去掉锁屏界面左下角的语音助理功能，修改为拨号][zhangaman][20170524]begin*/
        //mLeftIsVoiceAssist = canLaunchVoiceAssist();
        mLeftIsVoiceAssist = false;
        /*[TASK-4827][去掉锁屏界面左下角的语音助理功能，修改为拨号][zhangaman][20170524]end*/
        int drawableId;
    ...
    public void launchCamera(String source, boolean onback) {

        /*[BIRD][BIRD_AIMEI_STUDENT] caoyuangui 20170613 BEGIN*/
        if (FeatureOption.BIRD_AIMEI_STUDENT ) {
             if (FeatureOption.isForbid(mContext, "com.mediatek.camera")) {
                    // unlock
        ...
        Log.w("xiaowo", "onback = " + onback);
        final Intent intent = getCameraIntent();

        Bundle bundle = new Bundle();
        bundle.putBoolean("sh", onback);
        ...
        if (intent == SECURE_CAMERA_INTENT && !wouldLaunchResolverActivity) {
            ...
            if (android.os.SystemProperties.get("ro.bd_fingerprint_ex").equals("1")) {
                ...
                int appLockAble = Settings.System.getInt(mContext.getContentResolver(), Settings.System.BIRD_SYSTEM_SETTINGS_FP_APPLOCK, 0);
                if (appLockAble == 1) {
                    String lockedApps = Settings.System.getString(mContext.getContentResolver(), Settings.System.BIRD_SETTINGS_NEEDLOCK_APP_PACKAGENAMES);
                    if (lockedApps != null && lockedApps.contains("com.mediatek.camera")) {
                        isLocked = true;
                    }
                }
                if (isLocked) {
    ...
    private void launchPhone() {
        ...
        /*[BIRD][BIRD_AIMEI_STUDENT] caoyuangui 20170613 BEGIN*/
        if (FeatureOption.BIRD_AIMEI_STUDENT ) {
             if (FeatureOption.isForbid(mContext, "com.android.dialer")) {
                    // unlock
                    mPhoneStatusBar.animateCollapsePanels(CommandQueue.FLAG_EXCLUDE_NONE, true /* force */);
                    PowerManager powerManager = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
                    if (powerManager != null) {
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        ...
        if(FeatureOption.BIRD_DIRECT_WITH_PROXIMITY){
            if (changedView == this && visibility == GONE) {
                if (mUnLockProxSensor != null) {
                    mUnLockProxSensor.stop();
                    mUnLockProxSensor = null;
                }
            }
        }
        /*[BIRD][BIRD_DIRECT_WITH_PROXIMITY][智能体感][yangbo][20150605]END */
    ...
    private final KeyguardUpdateMonitorCallback mUpdateMonitorCallback =
        ...
        public void onScreenTurnedOn() {
            ...
            /*[BIRD][BIRD_DIRECT_WITH_PROXIMITY][智能体感][yangbo][20150605]BEGIN */
            if(FeatureOption.BIRD_DIRECT_WITH_PROXIMITY) {
                if(hasWindowFocus() && getProximitySwitchEnable()) {
                    if (mUnLockProxSensor == null) {
                        mUnLockProxSensor = new UnLockProxSensor();
                    }
                    mUnLockProxSensor.start();
                } else{
                    if (mUnLockProxSensor != null) {
                        mUnLockProxSensor.stop();
                        mUnLockProxSensor = null;
    ...
    /// @}
    /*[BIRD][BIRD_DIRECT_WITH_PROXIMITY][智能体感][yangbo][20150605]BEGIN */
    class UnLockProxSensor implements SensorEventListener {
        ...
        public void onSensorChanged(SensorEvent event) {
            try {
                if (event.values != null && event.values.length > 0) {
                    if ((event.timestamp - mOldTimestamp) > 1500000000.0f) {
                        if (event.values[0] < this.mMaxRange) { //被遮
                            mCount = 1;
                        } else {
                            mCount = 0;
                        }
    ...
    private boolean getProximitySwitchEnable() {
        if (FeatureOption.BIRD_DIRECT_WITH_PROXIMITY) {
            ...
            if (Settings.System.getInt(mContext.getContentResolver(), Settings.System.BIRD_DIRECT_TURN_ON, 0) == 1 && (Settings.System.getInt(mContext.getContentResolver(), Settings.System.BIRD_DIRECT_UNLOCK, 0) == 1)) {
                return true;
            } else {
                return false;
            }
