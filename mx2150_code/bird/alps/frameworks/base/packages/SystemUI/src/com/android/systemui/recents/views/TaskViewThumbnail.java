...
public class TaskViewThumbnail extends View {
    ...
    void onTaskDataLoaded(ActivityManager.TaskThumbnailInfo thumbnailInfo) {
        if (mTask.thumbnail != null) {
            ...
            ComponentName f = mTask.getTopComponent();
            if (f != null) {
                topActivityPackage = f.getPackageName();
            }
            if (FeatureOptions.BIRD_RECENTS_APP_BLUR && canBeBlur(topActivityPackage)) {
                setThumbnail(blur(mTask.thumbnail,12f), thumbnailInfo);
            } else {
    ...
    /*[BIRD][BIRD_RECENTS_APP_BLUR][模糊显示最近任务列表的应用][zhangaman][20170620]begin*/
    private Bitmap blur(Bitmap bmp,float radius) {
        float factor = 1f;
        ...
        canvas.translate((-this.getLeft()) / factor, (-this.getTop
())
                / factor);
        ...
        RenderScript mRenderScript = RenderScript.create
(mContext);
        ScriptIntrinsicBlur mScript = ScriptIntrinsicBlur.create
(mRenderScript,
    ...
    private boolean canBeBlur(String pkgName) {
        if (!getRecentsAppStatus () || TextUtils.isEmpty(pkgName)) {
            return false;
        }
        String appsListString = Settings.System.getString(mContext.getContentResolver(), Settings.System.BIRD_SETTINGS_NEEDBLUR_APP_PACKAGENAMES);
    ...
    private boolean getRecentsAppStatus() {
        int status = Settings.System.getInt(mContext.getContentResolver(), Settings.System.BIRD_SYSTEM_SETTINGS_RECENST_APP_BLUR, 0);
