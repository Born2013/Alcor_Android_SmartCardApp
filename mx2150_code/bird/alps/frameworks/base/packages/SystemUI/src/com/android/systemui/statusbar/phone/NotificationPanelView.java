...
//[BIRD][BIRD_REPLACE_UNLOCK_IMAGE][锁屏界面解锁图标替换成客制化图标][dingjiayuan][20160722] begin
import android.graphics.drawable.Drawable;
import android.graphics.drawable.InsetDrawable;
import android.util.Log;
//[BIRD][BIRD_REPLACE_UNLOCK_IMAGE][锁屏界面解锁图标替换成客制化图标][dingjiayuan][20160722] end
...
public class NotificationPanelView extends PanelView implements
    ...
    public int computeMaxKeyguardNotifications(int maximum) {
        ...
        float minPadding;
        if (com.android.systemui.FeatureOption.BIRD_CUSTOMED_KEYGUARD) {
            minPadding = mClockPositionAlgorithm.getMinStackScrollerPadding(getHeight(),
            mKeyguardStatusView.getHeight() - 150);
        } else {
            minPadding = mClockPositionAlgorithm.getMinStackScrollerPadding(getHeight(),
            mKeyguardStatusView.getHeight());
        }
    ...
    private boolean shouldQuickSettingsIntercept(float x, float y, float yDiff) {
        ...
        if (!mQsExpansionEnabled || mCollapsedOnDown 
                || (com.android.systemui.FeatureOption.BIRD_POWER_MANAGER_V2
    ...
    private void updateUnlockIcon() {
        if (mStatusBar.getBarState() == StatusBarState.KEYGUARD
            ...
            if (active && !mUnlockIconActive && mTracking) {
                //[BIRD][BIRD_REPLACE_UNLOCK_IMAGE][锁屏界面解锁图标替换成客制化图标][dingjiayuan][20160722] begin
                if (SystemProperties.get("ro.bd_replace_unlock_image").equals("1")) {
                    Drawable iconUnlock = mContext.getDrawable(R.drawable.zzz_ic_lock_24dp);
                    lockIcon.setImageDrawable(iconUnlock);
                }
                //[BIRD][BIRD_REPLACE_UNLOCK_IMAGE][锁屏界面解锁图标替换成客制化图标][dingjiayuan][20160722] begin
        ...
        if (start) {
            ...
            if (mFalsingManager.shouldEnforceBouncer()) {
				...
				//[BIRD_ONBACKPRESS_TAKE_PHOTO][在锁屏亮屏界面长按back打开相机并拍照][hongzhihao][20170613]begin
                        mKeyguardBottomArea.launchCamera(mLastCameraLaunchSource, false);
                    }
                }, null, true /* dismissShade */, false /* afterKeyguardGone */,
                    true /* deferred */);
            }
