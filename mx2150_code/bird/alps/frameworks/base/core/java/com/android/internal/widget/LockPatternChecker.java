...
public final class LockPatternChecker {
    ...
    public static AsyncTask<?, ?, ?> checkPatternFromAppLock(final LockPatternUtils utils, final String password, final int userId,
			final OnCheckCallback callback) {
		AsyncTask<Void, Void, Boolean> task = new AsyncTask<Void, Void, Boolean>() {
			private int mThrottleTimeout;
			...
			protected Boolean doInBackground(Void... args) {
				try {
					return utils.checkPatternFromAppLock(password, userId);
				} catch (RequestThrottledException ex) {
					mThrottleTimeout = ex.getTimeoutMs();
					return false;
				}
