...
//[BIRD][BIRD_OLD_PHONE] chengci added 20170825 begin
import android.widget.Toast;
import android.widget.ToastOldPhone;
//[BIRD][BIRD_OLD_PHONE] chengci added 20170825 end
...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    protected boolean onKeyDown(int featureId, int keyCode, KeyEvent event) {
         ...
         * ****************************************************************************/
        Log.d("Bird_oneStep", "enter PhoneWindow onKeyDown");
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                ...
                if (getBirdOldPhoneMode()) {
                    Log.d(TAG,"enter onKeyDown getBirdOldPhoneMode == ture");
                    if (intercepKeyEvent()) {
                        return true;
                    }
                }
                ...
                /*[BIRD][一键静音][Task6105] chengci 20171015 begin */
                if (getOneStepEnable() && getOneStepMuteMode() && !getAudioManager().isMusicActive()) {
                    return true;
                }
                /*[BIRD][一键静音][Task6105] chengci 20171015 end */
    ...
    private boolean getOneStepMuteMode() {
        if (mTelephonyMgr != null && mTelephonyMgr.getCallState() != TelephonyManager.CALL_STATE_IDLE) {
            Log.d("Bird_oneStep", "CALL_STATE_IDLE false");
            return false;
        }
        return (Settings.System.getInt(getContext().getContentResolver(), 
            "bird_onestep_volume_mute", 0) ==1);//|| BirdFlashlightNative.isFlashLightOn();
    }
    ...
    //[BIRD][BIRD_OLD_PHONE] chengci added 20170803 begin
    private boolean getBirdOldPhoneMode() {
        return SystemProperties.get("ro.bird_old_phone").equals("1") && 
            (Settings.System.getInt(getContext().getContentResolver(), 
            "is_oldphoneluancher_mode", 0) ==1);
    }

    private boolean intercepKeyEvent(){
        boolean isPeaceLock = Settings.System.getInt(getContext().getContentResolver(),
            "BIRD_PEACE_LOCK_MODE", 0) != 0;
        if (isPeaceLock) {
            Log.d(TAG,"enter intercepKeyEvent isPeaceLock");
            return true;
        }
        return false;
    }
            ...
            case KeyEvent.KEYCODE_VOLUME_DOWN: {
                ...
                if (getOneStepEnable() && getOneStepMuteMode() && !getAudioManager().isMusicActive()) {
                    Toast toast = Toast.makeText(getContext(), 
                            R.string.volume_onestep_enable_toast, Toast.LENGTH_SHORT);
                        toast.show();
                    return true;
                }
                /*[BIRD][一键静音][Task6105] chengci 20171015 end */
                ...
                //[BIRD][BIRD_OLD_PHONE] chengci added 20170803 begin
                if (getBirdOldPhoneMode()) {
                    Log.d(TAG,"enter onKeyUp getBirdOldPhoneMode == ture");
                    if (intercepKeyEvent()) {
                        ToastOldPhone toast = ToastOldPhone.makeText(getContext(), 
                            com.android.internal.R.string.volume_dialog_enable_toast, Toast.LENGTH_SHORT);
                        toast.show();
                        return true;
                    }
                }
    ...
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        //[BIRD][BIRD_MAKE_STATUSBAR_CLEAR][低内存模式，使状态栏看清][chenguangxiang][20170803] BEGIN
        if (!mIsFloating && (ActivityManager.isHighEndGfx() || SystemProperties.get("ro.bd_statusbar_clear").equals("1"))) {
        ...
        //[BIRD][BIRD_MAKE_STATUSBAR_CLEAR][低内存模式，使状态栏看清][chenguangxiang][20170803] END
