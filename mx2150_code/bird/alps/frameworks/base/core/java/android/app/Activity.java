...
public class Activity extends ContextThemeWrapper
    ...
    protected void onResume() {
        ...
        if (android.os.SystemProperties.get("ro.bd_fingerprint_ex").equals("1")) {
			int appLockAble = Settings.System.getInt(getContentResolver(), Settings.System.BIRD_SYSTEM_SETTINGS_FP_APPLOCK, 0);
			if (appLockAble == 1) {
			    KeyguardManager keyguardManager = (KeyguardManager) getSystemService(Context.KEYGUARD_SERVICE);	
			    //[bug-18475] caoyuangui add 20180302 begin 
			    boolean isLocked = keyguardManager.inKeyguardRestrictedInputMode();    
			    if (keyguardManager == null || keyguardManager.isDeviceSecure() == false || isLocked) {
		            //如果锁屏不是安全锁则不要进入应用锁 
                //[bug-18475] caoyuangui add 20180302 end 		            
		        } else {
				    String packagenameString = getPackageName();// lqiao
				    if (!packagenameString.equals("com.fingerprint.applock") && checkIntentNull(mIntent)) {
					    String lastPackage = Settings.System.getString(getContentResolver(), Settings.System.BIRD_LAST_APP_LOCK_PACKAGE_NAME);                        
					    if (lastPackage!= null && !lastPackage.equals(packagenameString)) {
						    String appString = Settings.System.getString(getContentResolver(), Settings.System.BIRD_SETTINGS_NEEDLOCK_APP_PACKAGENAMES);
						    String temp = Settings.System.getString(getContentResolver(), Settings.System.BIRD_ALREADY_UNLOCKED_PACKAGESNAME);
						    if (temp == null) {
							    temp = "";
						    }
						    if (appString != null && !temp.contains(packagenameString)) {
							    ...
							    for (int i = 0; i < appsStrings.length; i++) {
								    if (packagenameString.equals(appsStrings[i])) {
                                        //[bug-18500] caoyuangui add 20180302 begin 
									    Intent intent = new Intent();
									    intent.setClassName("com.fingerprint.applock","com.fingerprint.applock.service.AppLockWatchDogService");
									    intent.putExtra("packagename", packagenameString);
									    startService(intent);
                                        //[bug-18500] caoyuangui add 20180302 end 
								    }
							    }
    ...
    private boolean checkIntentNull(Intent intent) {
        if(intent != null) {
            ...
            if((className.equals("com.android.phone.EmergencyDialer"))
                ...
                || (className.equals("com.android.settings.ConfirmDeviceCredentialActivity"))
                || (className.equals("com.android.settings.ConfirmLockPattern"))
                || (className.equals("com.android.settings.ConfirmLockPassword"))
    ...
    public void startActivityForResult(@RequiresPermission Intent intent, int requestCode,
        ...
        if (BIRD_AIMEI_STUDENT) {
            android.util.Log.d("CAOYUANGUI", "intent = "+intent);
            if (intent !=null) {
               ...
               if ("android.intent.action.CHOOSER".equals(intent.getAction()) && "android".equals(packageName)) {
                      packageName = "com.android.documentsui";
               }
               if (isForbid(this, packageName, className)) {
                      return;
               }
            }
        }
     ...
     * @hide
     */
    private static final boolean BIRD_AIMEI_STUDENT = SystemProperties.get("ro.bd_aimei_student").equals("1");
     ...
     * @hide
     */
    private static  boolean isForbid(Context context, String packageName, String className) {
    	/*[BIRD][BIRD_AIMEI_STUDENT][gaowei][20170707]BEGIN*/
    	if ("com.android.incallui.InCallActivity".equals(className)) {
            return false;
        }
        /*[BIRD][BIRD_AIMEI_STUDENT][gaowei][20170707]END*/
        android.util.Log.d("CAOYUANGUI.isForbid", "packageName = "+packageName);
