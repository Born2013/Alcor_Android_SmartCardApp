...
public class DownloadManager {
    ...
    /*[TASK#5963][BIRD_KUSAI_DOWNLOAD_SUPPORT_PAUSE][增加下载暂停功能]xujing 20170918 begin*/
    /** @hide */
    public int pauseDownload(long... ids) {
        Log.d(XLOGTAG, "pauseDownload:" + ids);
        if (ids == null || ids.length == 0) {
            return 0;
        }
        ContentValues values = new ContentValues();
        ...
        Log.d(XLOGTAG, "pauseDownload:ids.length:" + ids.length);
        if (ids.length == 1) {
    ...
    public int resumeDownload(long... ids) {
        Log.d(XLOGTAG, "resumeDownload:" + ids);
        if (ids == null || ids.length == 0) {
            return 0;
        }
        ContentValues values = new ContentValues();
        ...
        Log.d(XLOGTAG, "resumeDownload:ids.length:" + ids.length);
        if (ids.length == 1) {
            return mResolver.update(ContentUris.withAppendedId(mBaseUri, ids[0]), values,
            null, null);
        } 
    ...
    /** @hide */
    public long getDownloadStatus(long ids) {
        long status = 0;
        Cursor cursor = null;
        try {
            cursor = mResolver.query(ContentUris.withAppendedId(mBaseUri, ids), 
                UNDERLYING_COLUMNS, null, null, null);
            
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    int columnIndex = cursor
                            .getColumnIndex(Downloads.Impl.COLUMN_STATUS);
                    status = cursor.getLong(columnIndex);
                    break;
                }
            }
        } catch (Exception e) {
            Log.e(XLOGTAG, "getDownloadStatus - Exception: ", e);
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    /** @hide */
    public long getDownloadStatusByIds(long... ids) {
        long status = 0;
        Cursor cursor = null;
        try {
            cursor = mResolver.query(mBaseUri, 
                UNDERLYING_COLUMNS, 
    ...
    /*[TASK#5963][BIRD_KUSAI_DOWNLOAD_SUPPORT_PAUSE][增加下载暂停功能]xujing 20170918 end*/
}
