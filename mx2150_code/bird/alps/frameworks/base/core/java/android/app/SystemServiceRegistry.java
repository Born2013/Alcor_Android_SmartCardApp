...
/*[BIRD][BIRD_SMALL_VIEW_WINDOW][深圳皮套功能][接口部分][pangmeizhou][20160614] BEGIN*/
import android.app.IHallcoverManager;
/*[BIRD][BIRD_SMALL_VIEW_WINDOW][深圳皮套功能][接口部分][pangmeizhou][20160614] END*/

/*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170629]BEGIN */
import com.bird.security.ContainerManager;
import com.bird.security.IContainerManager;
...
final class SystemServiceRegistry {
    ...
    static {
        ...
        /*[BIRD][BIRD_SMALL_VIEW_WINDOW][深圳皮套功能][接口部分][pangmeizhou][20160614] BEGIN*/
		if(true == SystemProperties.get("ro.bd_hall_leather").equals("1")
				|| true == SystemProperties.get("ro.bd_custom_hall_leather").equals("1")) { //[BIRD][BIRD_CUSTOM_SMALL_VIEW_WINDOW][重新客制化深圳皮套功能][dingjiayuan][20160707] modify
		    registerService(Context.HALL_COVER_SERVICE, HallcoverManager.class,
		            new CachedServiceFetcher<HallcoverManager>() {
		        @Override
		        public HallcoverManager createService(ContextImpl ctx) {
					HallcoverManager hallcover = null;
					try {
				        IBinder b = ServiceManager.getService(Context.HALL_COVER_SERVICE);
				        IHallcoverManager service = IHallcoverManager.Stub.asInterface(b);
						Log.i("HallcoverManager", "bb hallcover Service b = "+b +" service = "+service);
						hallcover = new HallcoverManager(service);
						Log.i("HallcoverManager", "bb hallcover Service b = "+b +" hallcover = "+hallcover);
		            } catch (Exception e) {
		                e.printStackTrace();
						Log.i("HallcoverManager", "Exception e: "+e);
		            }
		            return hallcover;
		        }});
		}
        ...
        /*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170629]BEGIN */
        if (SystemProperties.get("ro.bd_encrypt_space").equals("1")) {
            registerService(Context.BIRD_CONTAINER_SERVICE, ContainerManager.class,
                new CachedServiceFetcher<ContainerManager>() {
                @Override
                public ContainerManager createService(ContextImpl ctx) {
                    IBinder b = ServiceManager.getService(Context.BIRD_CONTAINER_SERVICE);
                    return new ContainerManager(ctx, IContainerManager.Stub.asInterface(b));
                }
            });
        }
        /*[BIRD][BIRD_ENCRYPT_SPACE][加密空间][yangbo][20170629]END */
