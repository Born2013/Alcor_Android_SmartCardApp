...
public class ActivityManager {
    ...
    public static class MemoryInfo implements Parcelable {
        ...
        public void readFromParcel(Parcel source) {
            ...
            long minRam = 0L;
            if (BIRD_SYSTEM_APP) {
                String maxStr = SystemProperties.get("persist.sys.tt.ram_max", "0");
                String minStr = SystemProperties.get("persist.sys.tt.ram_min", "0");
                if (!TextUtils.isEmpty(maxStr) && !"0".equals(maxStr)) {
                    maxRam = Long.valueOf(maxStr) * 1024L * 1024L;
                    totalMem = maxRam;
                } else {
                    totalMem = source.readLong();
                }

                if (!TextUtils.isEmpty(minStr) && !"0".equals(minStr)) {
                    minRam = Long.valueOf(minStr) * 1024L * 1024L;
                }
                
            } else {
                totalMem = source.readLong();
            }
            android.util.Log.i("huang1025", "readFromParcel availMem = " + availMem + ", maxRam = " + maxRam + ", minRam = " + minRam);
            if (BIRD_SYSTEM_APP && minRam > 0L && maxRam > availMem + minRam) {
                availMem = availMem + minRam;
            } 
            /*[BIRD][BIRD_SYSTEM_APP]chengshujiang 20160828 begin*/
