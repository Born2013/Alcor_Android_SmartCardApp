...
public class RemoteViews implements Parcelable, Filter {
                    ...
                    public void onClick(View v) {
                       ...
                       /*[BIRD][BIRD_AIMEI_STUDENT] caoyuangui 20170613 BEGIN*/
                        if (BIRD_AIMEI_STUDENT) {
                              Intent intent1 = pendingIntent.getIntent();
                              android.util.Log.d("CAOYUANGUI", "intent1 = "+intent1);
                              if (intent1 !=null && intent1.getComponent() !=null) {
                                   String packageName = intent1.getComponent().getPackageName();
                                   android.util.Log.d("CAOYUANGUI", "packageName = "+packageName);
                                   if (!isForbid(target.getContext(),packageName)) {
                                        final Rect rect = getSourceBounds(v);
                                        final Intent intent = new Intent();
                                        intent.setSourceBounds(rect);
   ...
   /**
     * @hide
    */
    private static  boolean isForbid(Context context, String packageName) {
        android.util.Log.d("CAOYUANGUI", "packageName = "+packageName);
        String selection = "pkgname=?";
        Cursor cursor = null;
        if (context == null) {
            ContentResolver resolver =  ActivityThread.currentApplication().getApplicationContext().getContentResolver();
            if (resolver !=null) {
                    cursor  = resolver.query(mAppUri, COL_S, selection, new String[]{packageName}, null);
            }
