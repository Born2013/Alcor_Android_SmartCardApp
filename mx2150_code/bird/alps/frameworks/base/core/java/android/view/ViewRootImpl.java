...
public final class ViewRootImpl implements ViewParent,
    ...
    boolean mNeedScreenShot = true;
    boolean mBirdScreenShotDistanceIsEnough = false;
    float mPonit1Y = 0.0f;
    float mPonit2Y = 0.0f;
    float mPonit3Y = 0.0f;
    float mPonitUp1Y = 0.0f;
    float mPonitUp2Y = 0.0f;
    float mPonitUp3Y = 0.0f;
    ...
    //[BIRD][三指截屏][BIRD_THREE_POINTER_SCREENSHOT][qianliliang][20160607] EDN

    void enqueueInputEvent(InputEvent event,
            InputEventReceiver receiver, int flags, boolean processImmediately) {
        //[BIRD][三指截屏][BIRD_THREE_POINTER_SCREENSHOT][qianliliang][20160607] BEGIN
        //有的应用无法用这个去获取当前状态Settings.System.getInt，但是SystemProperties.get可以获取
        if (SystemProperties.get("ro.bd_three_pointer_screenshot").equals("1")
                && SystemProperties.get("persist.sys.bird.screenshot").equals("1")){
            if (event instanceof MotionEvent) {
                MotionEvent motionEvent = (MotionEvent)event;
                int count = motionEvent.getPointerCount();
                Log.i(TAG, "BIRD_THREE_POINTER_SCREENSHOT, PointerCount = " +count );
                if (count == 1) {
                    switch (motionEvent.getAction()) {
                        case MotionEvent.ACTION_DOWN:
                            ...
                            break;
                        case MotionEvent.ACTION_UP:
                            if (mNeedScreenShot && mBirdScreenShotDistanceIsEnough) {
                            ...
                            mBirdScreenShotDistanceIsEnough = false;
                            mNeedScreenShot = false;
                            break;
                    }
                } else if (count == 3 && mNeedScreenShot) {

                    switch (motionEvent.getAction() & MotionEvent.ACTION_MASK) {
                        case MotionEvent.ACTION_POINTER_DOWN:
                            mPonit1Y = motionEvent.getY(0);
                            mPonit2Y = motionEvent.getY(1);
                            mPonit3Y = motionEvent.getY(2);
                            Log.i(TAG, "BIRD_THREE_POINTER_SCREENSHOT, " + mPonit1Y + ", " + mPonit2Y +", " + mPonit3Y);
                            break;
                        case MotionEvent.ACTION_MOVE:
                            motionEvent.setAction(MotionEvent.ACTION_CANCEL);
                            InputEvent cancellation = MotionEvent.obtain(motionEvent);
                            enqueueInputEvent(cancellation, receiver, 0, true);
                            Log.i(TAG, "BIRD_THREE_POINTER_SCREENSHOT, send ACTION_CANCEL");
                            break;
                        case MotionEvent.ACTION_POINTER_UP:
                            mPonitUp1Y = motionEvent.getY(0);
                            mPonitUp2Y = motionEvent.getY(1);
                            mPonitUp3Y = motionEvent.getY(2);
                            Log.i("xiaowo", mPonit1Y + ", " + mPonit2Y +", " + mPonit3Y + ", " +mPonitUp1Y + ", " + mPonitUp2Y +", " + mPonitUp3Y);
                            if (Math.abs(mPonitUp1Y - mPonit1Y) >= 100.0f 
                                    && Math.abs(mPonitUp2Y - mPonit2Y) >= 100.0f
                                    && Math.abs(mPonitUp3Y - mPonit3Y) >= 100.0f) {
                                Log.i(TAG, "BIRD_THREE_POINTER_SCREENSHOT, pointerCount = 3, MotionEvent.ACTION_POINTER_UP");
                                mBirdScreenShotDistanceIsEnough= true;
                            }
