...
//[BIRD_KUSAI_GALLETY][songwei][20170626]BEGIN
import android.os.SystemProperties;
//[BIRD_KUSAI_GALLETY][songwei][20170626]END
/**
...
public class ActivityChooserView extends ViewGroup implements ActivityChooserModelClient {
    ...
    public ActivityChooserView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        /// M: HotKnot, for recent button
        //[BIRD_KUSAI_GALLETY][songwei][20170626]BEGIN
        if(context.getPackageName().equals("com.android.gallery3d") && SystemProperties.get("ro.bird_kusai_gallery").equals("1")) {
            mRecentButtonEnabled = false;
            isGallery = true;
        } else {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            switch (itemViewType) {
                case ActivityChooserViewAdapter.ITEM_VIEW_TYPE_FOOTER: {
                    //[bug-12726][songwei][20170706]begin
                    if(isGallery) {
                        showPopupUnchecked(ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED, 200, 50);
                    } else {
                        showPopupUnchecked(ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED);
                    }
                    //[bug-12726][songwei][20170706]end
                } break;
    ...
    public void shareBlueteeth() {
        dismissPopup();
        final int index = mAdapter.getDataModel().getBluetoothIndex();
        ResolveInfo defaultActivity = mAdapter.getDataModel().getBluetoothActivity(index);
        Intent launchIntent = mAdapter.getDataModel().chooseActivity(index);
        if (launchIntent != null) {
            launchIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET);
            startActivity(launchIntent, defaultActivity);
    ...
    public void sharePopup(int x, int y) {
        showPopupUnchecked(mInitialActivityCount, x, y);
    }

    private void showPopupUnchecked(int maxActivityCount, int x, int y) {
        if (mAdapter.getDataModel() == null) {
            throw new IllegalStateException("No data model. Did you call #setDataModel?");
        }
