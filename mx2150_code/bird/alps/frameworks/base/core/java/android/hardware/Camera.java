...
public class Camera {
    ...
    private Camera(int cameraId, int halVersion) {
        ...
        /*[BIRD][BIRD_CAMERA_PIXEL_FRAUD][相机差值作假]huangzhangbin 20180306 begin*/
        if (SystemProperties.get("ro.bd_camera_piexl").equals("1")) {
            mCameraID = cameraId;
        }
        /*[BIRD][BIRD_CAMERA_PIXEL_FRAUD][相机差值作假]huangzhangbin 20180306 end*/
    ...
    */
        ...
        */
        ...
        public List<Size> getSupportedPictureSizes() {
            ...
            /*[BIRD][BIRD_CAMERA_PIXEL_FRAUD][相机差值作假]huangzhangbin 20180306 begin*/
            if (SystemProperties.get("ro.bd_camera_piexl").equals("1")) {
                String position = SystemProperties.get("persist.sys.tt.camera_set");
                if (position != null && position.length() > 0) {
                    String support = SystemProperties.get("persist.sys.tt.camera_support");
                    android.util.Log.i("huang0307", "getSupportedPictureSizes str = " + str + ", support = " + support + ", position = " + position);
                    String currentValue = support.split(",")[Integer.valueOf(position)];
                    android.util.Log.i("huang0307", "getSupportedPictureSizes currentValue = " + currentValue + ", mCameraID = " + mCameraID);
                    str = filterLimitResolution(str.split(","), currentValue);
                }
            }
            /*[BIRD][BIRD_CAMERA_PIXEL_FRAUD][相机差值作假]huangzhangbin 20180306 end*/
            return splitSize(str);
        ...
        /*[BIRD][BIRD_CAMERA_PIXEL_FRAUD][相机差值作假]huangzhangbin 20180306 begin*/
        private String filterLimitResolution(String[] pictureSizes, String currentValue) {
            String[] value = currentValue.split("x");
            int front = Integer.valueOf(value[0]) / 100;
            int back = Integer.valueOf(value[1]) / 100;
            android.util.Log.i("huang0307", "filterLimitResolution front = " + front + ", back = " + back);
            String str = "";
            for (int i = 0; i < pictureSizes.length; i++) {
                String size = pictureSizes[i];
                int index = size.indexOf('x');
                int width = Integer.parseInt(size.substring(0, index));
                int height = Integer.parseInt(size.substring(index + 1));
                int pix = width * height / 1000 / 1000;
                android.util.Log.i("huang0307", "filterLimitResolution width = " + width + ", height = " + height + ", pix = " + pix);
                if (mCameraID == 1 && Math.abs(pix - front) <= 1) {//前置摄像头
                    if (str.length() == 0) {
                        str = size;
                    } else {
                        str = str + "," + size;
                    }
                } else if (mCameraID == 0 && Math.abs(pix - back) <= 1) {//后置摄像头
                    if (str.length() == 0) {
                        str = size;
                    } else {
                        str = str + "," + size;
            ...
            android.util.Log.i("huang0307", "filterLimitResolution str = " + str);
            return str;
        }
