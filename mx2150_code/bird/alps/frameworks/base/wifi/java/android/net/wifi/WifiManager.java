...
/* [BIRD][BIRD_ZX_WIFI_SIGNAL_LEVEL][WIFI信号按中兴标准划分]xujing 20170714 begin */
import android.os.SystemProperties;
/* [BIRD][BIRD_ZX_WIFI_SIGNAL_LEVEL][WIFI信号按中兴标准划分]xujing 20170714 end */
...
public class WifiManager {
    ...
    /// M: M: Hotspot manager implementation @{
    /**
    ...
    */
    ...
    public boolean setWifiEnabled(boolean enabled) {
		//[BIRD][BIRD_CTA_WIFI_DISABLE][没有WIFI的手机CTA禁止调用wifi开关][pangmeizhou][20180726]begin
		if("1".equals(SystemProperties.get("ro.bird_wifi_diable","0"))){
			Log.d(TAG, "setWifiEnabled, aaaaa enabled = " + enabled);
			return false;
		}
		//[BIRD][BIRD_CTA_WIFI_DISABLE][没有WIFI的手机CTA禁止调用wifi开关][pangmeizhou][20180726]end
        try {
    ...
    public static int calculateSignalLevel(int rssi, int numLevels) {
        /* [BIRD][BIRD_ZX_WIFI_SIGNAL_LEVEL][WIFI信号按中兴标准划分]xujing 20170714 begin */
        if(SystemProperties.get("ro.bird_zx_wifi_signal_level").equals("1")) {
            if (rssi < -90) {
                return 0;//无信号
            } else if (rssi >= -60) {
                return numLevels - 1;//满格
            } else {
                if(rssi >= -70) {
                    return numLevels -2;//三格
                } else if(rssi >= -85) {
                    return numLevels -3;//两格
                } else if(rssi >= -90) {
                     return numLevels -4;//一格
                } else {
                    return 0;
                }
            }
   ...
   /**
    ...
    public boolean setWifiDisabled(int flag) {
		...
		//[BIRD][BIRD_CTA_WIFI_DISABLE][没有WIFI的手机CTA禁止调用wifi开关][pangmeizhou][20180726]begin
		if("1".equals(SystemProperties.get("ro.bird_wifi_diable","0"))){
			Log.d(TAG, "setWifiDisabled, aaaaa flag = " + flag);
			return false;
		}
		//[BIRD][BIRD_CTA_WIFI_DISABLE][没有WIFI的手机CTA禁止调用wifi开关][pangmeizhou][20180726]end
        if (null == mService) {
