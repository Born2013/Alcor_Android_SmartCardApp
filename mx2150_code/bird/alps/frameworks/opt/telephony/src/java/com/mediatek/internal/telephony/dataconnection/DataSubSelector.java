...
public class DataSubSelector {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
                ...
                if (simStatus.equals(IccCardConstants.INTENT_VALUE_ICC_IMSI)) {
                    ...
                    /*[BIRD][ss default sim1 data]xujing 20180309 begin*/
                    log("IMSI: FOR single sim SystemProperties.get:" + SystemProperties.get("persist.radio.multisim.config"));
                    if ("ss".equals(SystemProperties.get("persist.radio.multisim.config"))) {
                        int defaultDataSubId = SubscriptionManager.getDefaultDataSubscriptionId();
                        log("IMSI: FOR single sim defaultDataSubId:" + defaultDataSubId);
                        if (defaultDataSubId == -1) {
                            setDefaultData(slotId);
                        }
                    }
                    /*[BIRD][ss default sim1 data]xujing 20180309 end */
                ...
                if (MTK_C2K_SUPPORT) {
                    //[BIRD][BUG #13536][SIM卡插到另外一个卡槽中开机,不能切主卡][chenguangxiang][20170816]
                    if (mOperatorSpec.equals(OPERATOR_OM) || isOP09CSupport() || mOperatorSpec.equals("CUST")){
                        if (isCanSwitch()){
                            if (isOP09CSupport()){
    ...
    public DataSubSelector(Context context, int phoneNum) {
        ...
        // check op01 rule when each bootup
    ...
    private void subSelectorForOm(Intent intent) {
        ...
        log("DataSubSelector defDataSubId: " + defDataSubId + "; defDataPhoneId: " + defDataPhoneId);
        if (defDataPhoneId >= 0) {
        ...
        if (("N/A".equals(defaultIccid)) || ("".equals(defaultIccid))) {
            ...
            if (FeatureOption.BIRD_DEFAULT_DATA_ON) {
                ...
                log("BIRD_DEFAULT_DATA_ON, insertedSimCount = " + insertedSimCount);
                ...
                if (telephony != null) {
                    ...
                    log("BIRD_DEFAULT_DATA_ON, setDataEnabled, phoneId = " + phoneId);
                }
            }
            //[BIRD][BIRD_DEFAULT_DATA_ON][默认开启数据连接][qianliliang][20160622] END
            return;
    ...
    private void subSelectorForOp01(Intent intent) {
        ...
        for (int i = 0; i < mPhoneNum; i++) {
            ...
            if (!NO_SIM_VALUE.equals(currIccId[i]) && (SubscriptionManager.isValidSubscriptionId(
...
//            }
            ...
            if (isCanSwitch() == false || checkOp01CapSwitch6m() == false) {
                // need wait imsi ready
//                mIsNeedWaitImsi = true;
                setNeedWaitImsi(NEED_TO_WAIT_IMSI, Boolean.toString(true));
                if (intent != null &&
    ...
    private void handleDefaultDataOp01MultiSims(int targetPhoneId) {
        ...
        log("OP01 handle Multi SIM: targetPhoneId=" + targetPhoneId + ", preDataSub=" + preDataSub);
        if (targetPhoneId != SubscriptionManager.INVALID_PHONE_INDEX) {
            int sub = SubscriptionManager.getSubIdUsingPhoneId(targetPhoneId);
            if (preDataSub != sub && sub > SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
                setDataEnabled(targetPhoneId, getDataEnabledFromSetting(preDataSub));
            }
            setDefaultData(targetPhoneId);
            for (int i = 0; i < mPhoneNum; i++) {
                if (i != targetPhoneId) {
