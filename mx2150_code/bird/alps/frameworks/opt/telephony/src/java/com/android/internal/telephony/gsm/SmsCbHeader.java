...
public class SmsCbHeader {
    ...
    boolean isEmergencyMessage() {
        ...
        /*[BIRD][BIRD_CB_SAE_FEATURE_CUSTOM_CHANEL][客制化紧急小区广播频道][zhangaman][20170721]begin*/
        else if (isCustomSaeChanel(mMessageIdentifier)) {
                result = true;
        }
        /*[BIRD][BIRD_CB_SAE_FEATURE_CUSTOM_CHANEL][客制化紧急小区广播频道][zhangaman][20170721]end*/

		else {
            // 1. To check cell area is in specific region or not
            int region = GsmCellBroadcastHandler.isCellAreaInSpecificRegion();
            // 2. To get the special channel table for the specific region
            int[] channelTable = null;
            switch(region) {
                case SmsCbConstants.REGION_TAIWAN:
                    channelTable = SmsCbConstants.spcialChannelInTaiwan;
                    break;
                case SmsCbConstants.REGION_CHILE:
                    channelTable = SmsCbConstants.spcialChannelInChile;
                    break;
            };

            if (channelTable != null) {
                // 3. Compare message identifier
                for (int j = 0; j < channelTable.length; j++) {
                    if (mMessageIdentifier == channelTable[j]) {
                        result = true;
                        break;
                    }
                }
    ...
    /*[BIRD][BIRD_CB_SAE_FEATURE_CUSTOM_CHANEL][客制化紧急小区广播频道][zhangaman][20170721]begin*/
    private boolean isCustomSaeChanel(int Id) {
        Log.d("SmsCbHeader","isCustomSaeChanel Id : "+Id);
        if (!GsmCellBroadcastHandler.isCustomChanel()) {
            return false;
        }
        String chanel = SystemProperties.get("ro.bird_custom_chanel_value");
        Log.d("SmsCbHeader","isCustomSaeChanel chanel : "+chanel);
        if (TextUtils.isEmpty(chanel)) {
            return false;
        }
        String[] chanelStrings = chanel.split(",");
        Log.d("SmsCbHeader","isCustomSaeChanel chanelStrings : "+chanelStrings);
        for (String chanelString : chanelStrings) {
            if (Integer.parseInt(chanelString) == Id) {
                return true;
            }
        }
        return false;
