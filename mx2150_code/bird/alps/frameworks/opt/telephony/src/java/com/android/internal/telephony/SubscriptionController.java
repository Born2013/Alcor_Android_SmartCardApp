...
public class SubscriptionController extends ISub.Stub {
        ...
        try {
           ...
           // MTK-END
            ...
            if (setDisplayName) {
                ...
                if(SystemProperties.get("ro.bird_spn_custom").equals("1") && nameToSet.toString().length() > 1) {
                    nameToSet = nameToSet.toString().substring(0,1).toUpperCase()
                            + nameToSet.toString().substring(1).toLowerCase();
                    logd("BIRD_SPN_SHOW_FIRST_CAP_OTHER_LOWER sim name = " + nameToSet);
                }
                ...
                if(SystemProperties.get("ro.bird_spn_show_cap").equals("1") && nameToSet.toString().length() > 1) {
                    nameToSet = nameToSet.toString().substring(0,1).toUpperCase()
                            + nameToSet.toString().substring(1).toUpperCase();
                }
    ...
    public boolean setPlmnSpn(int slotId, boolean showPlmn, String plmn, boolean showSpn,
        ...
        synchronized (mLock) {
            ...
            for (int i = 0; i < subIds.length; i++) {
                //[BIRD][BIRD_SPN_SHOW_FIRST_CAP_OTHER_LOWER][SPN显示首字母大写,其余字母小写][qianliliang][20160603] BEGIN
                if(SystemProperties.get("ro.bird_spn_custom").equals("1") && carrierText != null && carrierText.toString().length() > 1) {
                    carrierText = carrierText.toString().substring(0,1).toUpperCase()
                            + carrierText.toString().substring(1).toLowerCase();
                    logd("BIRD_SPN_SHOW_FIRST_CAP_OTHER_LOWER sim carrierText = " + carrierText);
                }
                //[BIRD][BIRD_SPN_SHOW_FIRST_CAP_OTHER_LOWER][SPN显示首字母大写,其余字母小写][qianliliang][20160603] END
                //[BIRD][BIRD_SPN_SHOW_ALL_CAP][SPN显示全部字母大写][chenguangxiang][20170707] BEGIN
                if(SystemProperties.get("ro.bird_spn_show_cap").equals("1") && carrierText != null && carrierText.toString().length() > 1) {
                    carrierText = carrierText.toString().substring(0,1).toUpperCase()
                            + carrierText.toString().substring(1).toUpperCase();
                }
                //[BIRD][BIRD_SPN_SHOW_ALL_CAP][SPN显示全部字母大写][chenguangxiang][20170707] END
    ...
    public int getDefaultDataSubId() {
        int subId = Settings.Global.getInt(mContext.getContentResolver(),
                ...
                SubscriptionManager.INVALID_SUBSCRIPTION_ID);
        if (true) logd("[getDefaultDataSubId] amlinsan　subId= " + subId);
        return subId;
    }
    ...
    public void setDefaultDataSubId(int subId) {
        // MTK-START
        Rlog.d(LOG_TAG, "[setDefaultDataSubId] amlinsan subId= " + subId, new Exception(""));
    ...
    private void broadcastDefaultDataSubIdChanged(int subId) {
        ...
        if (DBG) logdl("[broadcastDefaultDataSubIdChanged] amlinsan subId=" + subId);
