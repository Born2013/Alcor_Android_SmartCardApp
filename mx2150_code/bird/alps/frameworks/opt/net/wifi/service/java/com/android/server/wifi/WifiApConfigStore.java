...
public class WifiApConfigStore {
    ...
    private WifiConfiguration getDefaultApConfiguration() {
        ...
        //[BIRD][BIRD_HOT_WIFI_PASSWORD][可配置wifi热点默认密码][hongzhihao][20160918] BEGIN
        String mPassWord = SystemProperties.get("ro.bd_hot_wifi_password");
        if ("1".equals(SystemProperties.get("ro.bd_hot_wifi")) && mPassWord != null) {             
            config.preSharedKey = mPassWord;
        } else {
            config.preSharedKey = randomUUID.substring(0, 8) + randomUUID.substring(9, 13);
        }    
        //[BIRD][BIRD_HOT_WIFI_PASSWORD][可配置wifi热点默认密码][hongzhihao][20160918] END
        return config;
    }
}
