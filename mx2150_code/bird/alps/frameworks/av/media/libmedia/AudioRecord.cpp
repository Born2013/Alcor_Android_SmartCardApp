        ...
        const audio_attributes_t* pAttributes)
{
    ...
    ALOGD("set(): %p, mTransfer = %d", this, mTransfer);
    ...
    if (cbf != NULL) {
        ...
        ALOGD("set: Create AudioRecordThread, tid = %d", mAudioRecordThread->getTid());
    }
...
ssize_t AudioRecord::read(void* buffer, size_t userSize, bool blocking)
{
    ...
    while (userSize >= mFrameSize) {
        ...
        if (err < 0) {
            ...
            ALOGE("AudioRecord::read: %p, err = %d", this, err);
            return ssize_t(err);
        }
    ...
    ALOGV("%s: %p, read = %zd, mFramesRead = %" PRId64, __FUNCTION__, this, read, mFramesRead);
    return read;
}
